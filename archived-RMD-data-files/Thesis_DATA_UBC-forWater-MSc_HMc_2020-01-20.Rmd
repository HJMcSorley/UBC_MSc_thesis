...
## Join results files from Shimadzu and Spectrolyser (direct and indirect measures of DOC pluss full scan data)
...
# --- WIDE SUMMARY DF --- # 
sampleresults_wide <- full_join(TOCV_results, SCAN_results, 
...
full_join(FULLSCAN_results, by = c("trip", "site", "sample_type", "sample", "dt_sampled", "fillStage_cm")) %>% 
  select(-c(vial, measurement))  # drop unneccesary variables
...
The six pirmary research location in the Leech River watershed equipped with vertical sampling racks (which combined passive (siphon) samplers and compact river stage loggers) colected continuous water level via Odyssey capacitiance water level loggers. 
...
sample_count_df <- TOCV_results %>% 
  ...
  knitr::kable(., caption = "Installation sites sample counts")
...
seasonal_count_df <- sampleresults_wide %>% 
  select("NPOC_ppm", "site", "sample_type", "sample", "two_seasons", "analysis") %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
                  site == "Cragg-crk" |
  ...
  summarise(number_of_samples = n())  %>% 
  knitr::kable(., caption = "Installation sites sample counts by season")
...
sampleresults_wide %>% 
  dplyr::filter(site == "Weeks-out" |
                  ...
                  site == "Tunnel",
         analysis == "DOC",
         sample_type == "Grab" | sample_type == "Rack", sample == "Grab") %>%
  mutate(site = factor(site, levels = c("Weeks-out", "Chris-crk", "Leech-head", "Cragg-crk", "West-Leech", "Tunnel"))) %>%
  mutate(trip = factor(trip, levels = (1:22))) %>% 
  ggplot(aes(x = trip, y = NPOC_ppm, shape = sample_type, fill = two_seasons)) +
