...
pMessages(library(knitr))      # tidy tables
...
# create a data frame, holding file names in a new column
shimadzu_samples <- data_frame(filename = dir("R-inputs_UBC-forWater-MSc_HMc/shimadzu/samples/")) %>% 
  dplyr::mutate(file_contents = purrr::map(filename, ~ readr::read_csv(file.path("R-inputs_UBC-forWater-MSc_HMc/shimadzu/samples/", .),
                                                  skip = 4,
                                                  col_names = TRUE))) %>% 
  tidyr::unnest(cols = c(file_contents))
shimadzu_samples
  
  purrr::map(~ readr::read_csv(file.path("R-inputs_UBC-forWater-MSc_HMc/shimadzu/samples/", .), skip = 4, col_names = TRUE)) %>%
  purrr::reduce(rbind)
shimadzu_samples

# hmmm try it again but different
shimadzu_samples <- dir("R-inputs_UBC-forWater-MSc_HMc/shimadzu/samples/")) %>% 
  dplyr::mutate(file_contents = purrr::map(filename, ~ readr::read_csv(file.path("R-inputs_UBC-forWater-MSc_HMc/shimadzu/samples/", .),
                                                  skip = 4,
                                                  col_names = TRUE))) %>% 
  tidyr::unnest(cols = c(file_contents))
...
# --------- you are here
# try again
shimadzu_sample_files <- list.files(path = "R-inputs_UBC-forWater-MSc_HMc/shimadzu/samples/", pattern = "*.csv", full.names = TRUE)

sampleFUN <- function(x){
    read_csv(file = x, skip = 4, col_names = TRUE, col_types = list("d", "c", "c", "c", "c", "d", "f", "f"))}
...
sampleIDmerge <-  function(mypath){
  filenames = list.files(path = mypath)
  datalist = lapply(filenames, function(x) {
    read_csv(file = x, skip = 4, col_names = TRUE,
  ...
  purrr::reduce(function(x, y) {full_join(x, y)}, datalist)
}

shimadzu_samples <- sampleIDmerge("/R-inputs_UBC-forWater-MSc_HMc/shimadzu/samples")
