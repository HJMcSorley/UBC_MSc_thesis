...
majEvents_summary <- events %>%
  mutate(duration_hrs = difftime(EndDate, StartDate, units = "hours"),
         duration_days = as.numeric(round(difftime(EndDate, StartDate, units = "days"), 1)),
         hours = as.numeric(NumberXtract(duration_hrs)),
         intensity = rain/hours,
...
events_summary <- majEvents_summary %>% 
  ...
  summarise(duration_min_days = min(duration_days),
            ...
            intensity_min_mmhr = min(intensity),
            intensity_max_mmhr = max(intensity),
            ...
            ID_intensity_min = ID[which(intensity == intensity_min_mmhr)],
            ID_intensity_max = ID[which(intensity == intensity_max_mmhr)])
...
events_summary %>% 
  ...
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/Wx_Events_min-max.csv",
            col_names = TRUE) 
...
six_wide_stage <- odyssey_data %>%
  ...
  select(c(DateTime, source, stage_cm)) %>% 
  distinct(DateTime, .keep_all = TRUE) %>%   # keep only unique time stamps
  ungroup() %>% 
  pivot_wider(names_from = source,
              values_from = stage_cm) %>% 
  unnest(cols = c(Weeks, ChrisCrk, LeechHead, CraggCrk, WestLeech, Tunnel)) %>% 
  arrange(DateTime) %>% 
  mutate(timestep = row_number(DateTime))
...
# note, I check p-value difference with nrands = 10000 and they were similar enough that I think this is fine.
# there is no difference between results of type 1 or 2 (?)
peaks_1to2 <- synchrony::peaks(six_wide_stage_Weeks, six_wide_stage_Chris, type = 2, nrands = 1000)
peaks_1to3 <- synchrony::peaks(six_wide_stage_Weeks, six_wide_stage_Head, type = 2, nrands = 1000)
peaks_2to3 <- synchrony::peaks(six_wide_stage_Chris, six_wide_stage_Head, type = 2, nrands = 1000)
peaks_3to4 <- synchrony::peaks(six_wide_stage_Head, six_wide_stage_Cragg, type = 2, nrands = 1000)
peaks_3to5 <- synchrony::peaks(six_wide_stage_Head, six_wide_stage_West, type = 2, nrands = 1000)
peaks_4to5 <- synchrony::peaks(six_wide_stage_Cragg, six_wide_stage_West, type = 2, nrands = 1000)
peaks_3to6 <- synchrony::peaks(six_wide_stage_Head, six_wide_stage_Tunnel, type = 2, nrands = 1000)
peaks_4to6 <- synchrony::peaks(six_wide_stage_Cragg, six_wide_stage_Tunnel, type = 2, nrands = 1000)
peaks_5to6 <- synchrony::peaks(six_wide_stage_West, six_wide_stage_Tunnel, type = 2, nrands = 1000)
...
synch_PeaksSumm_stage <- tibble(
  ...
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/Stage-stats_synchrony-peaks.csv", col_names = T)

# where proportion of coincident peaks = a fraction of the maximum number of peaks in the two series
```


## Normalized stage + samples
...
* identify the max DOC/SaC254/SUVA for each site & event sampled
* find the corresponding sample stage for max[]
* identify the max sample-stage for each site & event sampled
* count the number of samples collected in that event 
* was the max[] stage the same as the max sample-stage? i.e. did the highest bottle collect the max[]?
* in case of sampling multiple stage peaks or falling limb, also look at timestep for sample-stage
...
stepped_stg_samples <- stage_samples_norm %>% 
  filter(DateTime >= "2018-12-06 19:30:00", DateTime <= "2020-02-01 02:30:00") %>%  # tunnel logger life
  select(c(site, event_ID, corr_stage_cm, norm_stg, DateTime, NPOC_ppm, SAC254_Abs.m, SUVA)) %>% 
...
# find peak for stage
pks_stage_DOC <- stepped_stg_samples %>% 
  filter(!is.na(NPOC_ppm)) %>%
  group_by(site, event_ID) %>% 
  summarise(DateTime_peak_stage = DateTime[which.max(corr_stage_cm)],
            timestep_peak_stage = timestep[which.max(corr_stage_cm)],
            peak_stage = max(corr_stage_cm))
            
# isolate & find peaks for NPOC and keep the same timesteps
pks_DOC <- stepped_stg_samples %>% 
  filter(!is.na(NPOC_ppm)) %>% 
  group_by(site, event_ID) %>% 
  summarise(DateTime_peak_DOC = DateTime[which.max(NPOC_ppm)],
            timestep_peak_DOC = timestep[which.max(NPOC_ppm)],
            peak_DOC = max(NPOC_ppm))
       
# isolate & find peaks for SAC254 and keep the same timesteps  
pks_SAC254 <- stepped_stg_samples %>% 
  filter(!is.na(SAC254_Abs.m)) %>%
  group_by(site, event_ID) %>%
  summarise(DateTime_peak_SAC254 = DateTime[which.max(SAC254_Abs.m)],
            timestep_peak_SAC254 = timestep[which.max(SAC254_Abs.m)],
            peak_SAC254 = max(SAC254_Abs.m)) 

# isolate & find peaks for SUVA254 and keep the same timesteps  
pks_SUVA <- stepped_stg_samples %>% 
  filter(!is.na(SUVA)) %>%
  group_by(site, event_ID) %>% 
  summarise(DateTime_peak_SUVA = DateTime[which.max(SUVA)],
            timestep_peak_SUVA = timestep[which.max(SUVA)],
            peak_SUVA = max(SUVA)) 
  
 
# now - compare these group summaries to see if the timesteps were the same between groups
...
##### isolate max for each site
```{r}
# isolate data related to max stage at each site
...
# HERE
## ??Slice: DOC subbasin MAX DOC and UV254 
```{r}

# isolate data related to max DOC at each site
max_DOC <- sixfilter %>%
  mutate(year = lubridate::year(DateTime_sampled)) %>% 
  filter(sample_type == "Grab" | sample_type == "Rack") %>% 
  select(c(site, year, trip, DateTime_sampled, sample, NPOC_ppm, SAC254_Abs.m, corr_stage_cm,  event_ID, rain_seasons, HoldTime_days, QAQC_flag)) %>% 
  group_by(year, site) %>% 
  slice(which.max(NPOC_ppm)) 

# isolate data related to max UV-254 at each site
max_UV254 <- sixfilter %>%
  mutate(year = lubridate::year(DateTime_sampled)) %>% 
  filter(sample_type == "Grab" | sample_type == "Rack") %>% 
  select(c(site, year, trip, DateTime_sampled, sample, NPOC_ppm, SAC254_Abs.m, corr_stage_cm,  event_ID, rain_seasons, HoldTime_days, QAQC_flag)) %>% 
  group_by(site, year) %>% 
  slice(which.max(SAC254_Abs.m))
...
peaks_1to2 <- synchrony::peaks(six_wide_NPOC_Weeks, six_wide_NPOC_Chris, type = 2)
peaks_1to3 <- synchrony::peaks(six_wide_NPOC_Weeks, six_wide_NPOC_Head, type = 2)
peaks_2to3 <- synchrony::peaks(six_wide_NPOC_Chris, six_wide_NPOC_Head, type = 2)
peaks_3to4 <- synchrony::peaks(six_wide_NPOC_Head, six_wide_NPOC_Cragg, type = 2)
peaks_3to5 <- synchrony::peaks(six_wide_NPOC_Head, six_wide_NPOC_West, type = 2)
peaks_4to5 <- synchrony::peaks(six_wide_NPOC_Cragg, six_wide_NPOC_West, type = 2)
peaks_3to6 <- synchrony::peaks(six_wide_NPOC_Head, six_wide_NPOC_Tunnel, type = 2)
peaks_4to6 <- synchrony::peaks(six_wide_NPOC_Cragg, six_wide_NPOC_Tunnel, type = 2)
peaks_5to6 <- synchrony::peaks(six_wide_NPOC_West, six_wide_NPOC_Tunnel, type = 2)

# combine
synch_PeaksSumm <- tibble(
       "Comparison Group" = c(rep("headwaters", 3), rep("mainstems", 3), rep("mainstem to outlet", 3)),
       "Site Comparison" = c("Weeks & ChrisCrk", "LeechHead & Weeks", "LeechHead & ChrisCrk", 
                             "LeechHead & CraggCrk", "LeechHead & WestLeech", "CraggCrk & WestLeech",
                             "LeechHead & Tunnel", "CraggCrk & Tunnel", "WestLeech & Tunnel"),
       "Proportion of common peaks" = c(peaks_1to2$obs, peaks_1to3$obs, peaks_2to3$obs, peaks_3to4$obs, peaks_3to5$obs, peaks_4to5$obs, peaks_3to6$obs, peaks_4to6$obs, peaks_5to6$obs)) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/DOC-stats_synchrony-peaks.csv", col_names = T)


## Synchrony
# tests by site (number)
synch_1to2 <- synchrony::meancorr(six_wide_NPOC_Weeks, six_wide_NPOC_Chris, type = 2)
synch_1to3 <- synchrony::peaks(six_wide_NPOC_Weeks, six_wide_NPOC_Head, type = 2)
synch_2to3 <- synchrony::peaks(six_wide_NPOC_Chris, six_wide_NPOC_Head, type = 2)
synch_3to4 <- synchrony::peaks(six_wide_NPOC_Head, six_wide_NPOC_Cragg, type = 2)
synch_3to5 <- synchrony::peaks(six_wide_NPOC_Head, six_wide_NPOC_West, type = 2)
synch_4to5 <- synchrony::peaks(six_wide_NPOC_Cragg, six_wide_NPOC_West, type = 2)
synch_3to6 <- synchrony::peaks(six_wide_NPOC_Head, six_wide_NPOC_Tunnel, type = 2)
synch_4to6 <- synchrony::peaks(six_wide_NPOC_Cragg, six_wide_NPOC_Tunnel, type = 2)
synch_5to6 <- synchrony::peaks(six_wide_NPOC_West, six_wide_NPOC_Tunnel, type = 2)

```

## Temporal trends 

### *plot: DOC~time subbasin geom_smooth -- site & subbasin type
