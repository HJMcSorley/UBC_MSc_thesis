...
Hobo <- read_csv("R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/Hobo_trip-event.csv", 
                    col_names = TRUE) %>% 
...
Hobo %>% 
  ...
  geom_line(aes(colour = site), size = 1.25)+
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/TidbiTs_daily-temps.png", 
       width = 6, height = 7, units = "in")
```

## table: Hobo temp VS mean LWSA
```{r}
# join the air temp from subbasins with the mean LWSA temperature data to compare
Hobo_WxLWSA <- LWSA_meanWx %>%
  select(c(DateTime, Temp_mean, TMin_mean, TMax_mean)) %>% 
  right_join(
    (Hobo %>% filter(TidbiT_location == "air") %>%  group_by(site) ), by = "DateTime") %>% 
  rename(LWSA_Tmean = "Temp_mean", LWSA_Tmin = "TMin_mean", LWSA_Tmax = "TMax_mean", Hobo_Temp = Temp_C) %>% 
  select(c(site, Date, DateTime, rain_season, trip, Hobo_Temp, LWSA_Tmean, LWSA_Tmin, LWSA_Tmax)) %>% 
  distinct()
# temp on the half hour and min/max on the hour

# ---
# find and isolate overlapping ranges
Hobo_Fwx %>% 
  group_by(site) %>% 
  summarize(count = n(),
            earliest = first(Date),
...
# isolate
Hobo_WxLWSA <- Hobo_WxLWSA %>%
  filter(Date > "2019-08-23", Date < "2020-02-18") %>% 
  rename(Hobo = "Hobo_Temp", FWx = "LWSA_Tmean")

# pivot longer
Hobo_Fwx <- Hobo_WxLWSA %>%
  select(-c(LWSA_Tmin, LWSA_Tmax)) %>% 
  group_by(site) %>% 
...
# density by TidbiT site -- not normal
Hobo_Fwx %>% 
  ggplot(aes(Temp_C))+
  geom_density(aes(colour = data_source))+
  scale_color_brewer(palette = "Dark2")+ 
  theme_bw()+
  theme(legend.position = "top")+
  facet_wrap(~ site,
...
#### Relate subbasins to FWx
# YOU ARE HERE

use linear regression to fill in temperature estimates at each subbasin 

```{r}
# 1:1 scatter of temperatures
Hobo_WxLWSA %>% 
  ggplot(aes(x = FWx, y = Hobo))+
  geom_point(alpha = 0.5)+
  theme_bw()+
  facet_wrap(~ site)+
  labs(y = "Subbasin temperature (°C)", x = "Mean LWSA temperature (°C)") +
  theme(text = element_text(size = 12))+
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/TidbiTs-FWx_scatter_lm.png", 
       width = 5, height = 4, units = "in")

```


## Hobo with subbasin stage_sample results
```{r}

## Below is old

# use subbasin_dailyHobo
subbasin_dailyHobo

# --- OLD --- 
a <- Hobo %>% 
  mutate(hr_DateTime = round_date(DateTime)) %>% 
  group_by(site, location, hr_DateTime) %>% 
  summarise(hr_Temp = mean(Temp_C))

b <- stage_samples %>% 
  mutate(hr_DateTime = round_date(DateTime),
         hr_DT_sampled = round_date(DateTime_sampled)) %>% 
  group_by(site, trip, NPOC_ppm, hr_DateTime, hr_DT_sampled) %>% 
  summarise(hr_Stage = mean(corr_stage_cm))
  
subbasin_df <- full_join(a, b, by = c("site", "hr_DateTime"))

# plot?
subbasin_df %>% 
  filter(!is.na(hr_Temp)) %>% 
  group_by(site, location, hr_DateTime) %>% 
  ggplot(aes(x = hr_DateTime, y = hr_Temp))+
...
subbasin_df %>% 
  filter(!is.na(hr_Temp)) %>% 
  group_by(site, hr_DateTime) %>% 
  ggplot(aes(x = hr_DateTime, y = hr_Stage))+
  geom_line(colour = forWater_colours2[["DeepBlue"]])+
  theme_bw()+
  facet_wrap(~site, ncol = 1,
             strip.position = "right")+
  labs(x = "", y = "Mean Hourly Stage (cm)")+
  theme(legend.position = "top", 
        text = element_text(size = 12))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
wut <- subbasin_df %>% 
  group_by(site, trip) %>% 
  summarise(T_mean = mean(Temp_C, na.rm = T),
            T_sd = sd(Temp_C, na.rm = T),
            S_mean = mean(corr_stage_cm, na.rm = T),
            S_sd = sd(corr_stage_cm, na.rm = T))
