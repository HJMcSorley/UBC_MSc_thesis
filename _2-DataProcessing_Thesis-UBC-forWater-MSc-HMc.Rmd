...
synopticfilter %>% 
  ...
  theme(legend.position = "none",
        ...
        axis.text.x = element_text(angle = 60, hjust = 1)) +
  labs(caption = 
         glue::glue("DOC concentrations in grab samples across 13 sites over 16 months (", {n_SynopticGrabs}, " samples)"), 
       x = "", y = "DOC (mg/L)")
# save
...
synopticfilter %>% 
  ...
  theme(legend.position = "none",
        text = element_text(size = 12)) +
  labs(caption = 
         glue::glue("DOC concentrations in grab samples across thirteen sites over 16 months (", {n_SynopticGrabs}, " samples)"), 
       x = "DOC (mg/L)", y = "")
# save
...
sixfilter %>% 
  ...
  summarize(DOC_mean = mean(NPOC_ppm, na.rm = T),
            ...
            DOC_max = max(NPOC_ppm, na.rm = T)) %>% 
  ungroup() %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/DOC_subbasin-SampleType_MeanMinMax_table.csv", col_names = TRUE)
