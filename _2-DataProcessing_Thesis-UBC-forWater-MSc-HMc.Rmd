...
stage_sample_plot <- stage_samples %>% 
  ...
  geom_vline(xintercept = c(trip_starts), size = 0.7, 
             colour = forWater_colours2["MyOrange"])+ 
  geom_vline(xintercept = c(event_starts), size = 0.7, linetype = "twodash", 
             colour = forWater_colours1["AccentBlue"])+ 
  geom_line(aes(y = corr_stage_cm), size = 0.7)+
  theme_bw()+
  geom_point(aes(y = sampleStage_cm, shape = sample_type), fill = "white", na.rm = TRUE)+
  scale_shape_manual(breaks = c("Grab", "Rack"),
                     values = c("Grab" = 24, "Rack" = 21))+
  ...
  theme(text = element_text(size = 12),
        axis.text.x = element_text(angle = 60, hjust = 1),
        legend.position = "top")+
  guides(shape = guide_legend("Sample Type:"))
...
* group by site and event ID
* identify max DOC
* find corresponding sample stage for max DOC
* count number of bottles (levels) that were collected on that event surge


oh no... there's something fucky going on for trip 22-23... why are rack samples on the falling limbs?
```{r}
         
# select trips/events that were well represented by samples for each site

# DOC peaks 
pks_DOC <- sixfilter %>% 
  filter(trip == 4 | trip == 5 | trip == 17 | trip == 18 | trip == 19 | trip == 23) %>%  # 18 + 23 were falling limb, 4 is good for all eaxcept Tunnel & 19 is a gooder for all
  group_by(site, trip) %>% 
  summarise(#peak_DOC = max(NPOC_ppm),
            #DateTime_peak_DOC = DateTime_sampled[which.max(NPOC_ppm)],
            peak_DOC_stage = sampleStage_cm[which.max(NPOC_ppm)],
            peak_DOC_sample = sample[which.max(NPOC_ppm)],
            peak_DOC_event_ID = event_ID[which.max(NPOC_ppm)],
            #DateTime_peak_stage = DateTime_sampled[first(which.max(sampleStage_cm))],
            peak_sampleStage = max(sampleStage_cm),
            peak_stage_sample = sample[first(which.max(sampleStage_cm))],
            peak_stage_event_ID = event_ID[first(which.max(sampleStage_cm))],
            n_bottles = n(),
