...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/HT-exp_boxplots.png")

```

### tests: Wilcoxon
```{r}
# Tests NPOC for normality 
HT %>% 
  ggplot(aes(NPOC_ppm))+
...
# t-tests --> Wilcoxon tests (not normally distributed + small sample size)
...
HT_Wilcoxon_tests <- bind_rows(
  ...
  mutate(sig.diff = case_when(p.value < 0.01 ~ "at 99%",
                                ...
                                p.value < 0.1 ~ "at 90%",
                                p.value > 0.1 ~ "NA")) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/HoldTime_Wilcoxon-tests.csv")
