...
### trash plots: air temp over time & space
```
...
#### baby mega plot
```{r}
# plot stage, rain events, samples (by sample type) all sites
stage_samples %>% 
  ggplot(aes(x = DateTime))+
...
# save megaplot
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/hydro-samples_events_plot.png",
         width = 8.5, height = 11, units = "in")
```
...
### tables: DOC subbasin sites, Mean/Min/Max 
...
### variance: Levene's test for homoscedasticity in DOC:
...
TRASH: temporal NPOC Synchrony tests
```
# this isn't useful --- see manual checks of DOC extrema with stage 
# -- synchrony::peaks WET
