...
# --- drop two headwater creeks -- not sure this is useful...

# change colour order for clarity (drop 1 and 2)
# check colours with > brewer.pal(n = 6, name = "Set2")
mainstemcolours <- c(brewer.pal(n = 6, name = "Set2")[[3]],
                      brewer.pal(n = 6, name = "Set2")[[4]], 
...
site_mainstem_box <- a %>% 
  ...
  geom_boxplot(alpha = 0.5) +
  geom_jitter(aes(fill = site), alpha = 0.8, shape = 21) +
  scale_colour_manual(values = mainstemcolours, aesthetics = "fill") +
  theme_bw() +
  theme(legend.position = "none",
        text = element_text(size = 12)) +
  labs(x = "", y = "DOC (mg/L)")
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/DOC_mainstem_boxplots.png")

# stack
cowplot::plot_grid(site_subbasin_box, site_mainstem_box, ncol = 1, rel_widths = c(2,1))

```

##### ?inferential/inductive stats

Levene's test: An alternative to the Bartlett’s test that is less sensitive to departures from normality. Test for Ho, homoscedasticity (homogeneity of variance). If p-value is less than significance level (e.g. 0.05 for 95% confidence) the differences are unlikely to have been caused randomly and it's concluded that there is a difference between variances.
...
# change colour order for clarity
# check colours with > brewer.pal(n = 6, name = "Dark2")
# new order = [1,4,2,5,3,6]
timetrendcolours <- c(brewer.pal(n = 6, name = "Dark2")[[1]],
                      brewer.pal(n = 6, name = "Dark2")[[4]], 
                      brewer.pal(n = 6, name = "Dark2")[[2]], 
                      brewer.pal(n = 6, name = "Dark2")[[5]], 
                      brewer.pal(n = 6, name = "Dark2")[[3]], 
                      brewer.pal(n = 6, name = "Dark2")[[6]])
  
# at each of the six sites (DOC over time)
sixfilter %>%
  # change site order for clarity in legend (1,4,2,5,3,6)
  ...
  scale_colour_manual(values = timetrendcolours, aesthetics = c("colour", "fill"))+
...
# 1
six_wide_NPOC_Weeks <- six_wide_NPOC %>% select(c(DateTime_sampled, Weeks)) %>% 
  mutate(timestep = row_number(DateTime_sampled)) %>% 
  select(c(timestep, NPOC_Weeks = "Weeks")) %>% 
  as.matrix()
# 2
six_wide_NPOC_Chris <- six_wide_NPOC %>% select(c(DateTime_sampled, ChrisCrk)) %>% 
  mutate(timestep = row_number(DateTime_sampled)) %>% 
  select(c(timestep, NPOC_ChrisCrk = "ChrisCrk")) %>% 
  as.matrix()
# 3
six_wide_NPOC_Head <- six_wide_NPOC %>% select(c(DateTime_sampled, LeechHead)) %>% 
  mutate(timestep = row_number(DateTime_sampled)) %>% 
  select(c(timestep, NPOC_LeechHead = "LeechHead")) %>% 
  as.matrix()
# 4
six_wide_NPOC_Cragg <- six_wide_NPOC %>% select(c(DateTime_sampled, CraggCrk)) %>% 
  mutate(timestep = row_number(DateTime_sampled)) %>% 
  select(c(timestep, NPOC_CraggCrk = "CraggCrk")) %>% 
  as.matrix()
# 5
six_wide_NPOC_West <- six_wide_NPOC %>% select(c(DateTime_sampled, WestLeech)) %>% 
  mutate(timestep = row_number(DateTime_sampled)) %>% 
  select(c(timestep, NPOC_WestLeech = "WestLeech")) %>%
  as.matrix()
# 6
six_wide_NPOC_Tunnel <- six_wide_NPOC %>% select(c(DateTime_sampled, Tunnel))%>% 
  mutate(timestep = row_number(DateTime_sampled)) %>% 
  select(c(timestep, NPOC_Tunnel = "Tunnel")) %>%
  as.matrix()

# tests by site (number)
peaks_1to2 <- synchrony::peaks(six_wide_NPOC_Weeks, six_wide_NPOC_Chris)
peaks_1to3 <- synchrony::peaks(six_wide_NPOC_Weeks, six_wide_NPOC_Head)
peaks_2to3 <- synchrony::peaks(six_wide_NPOC_Chris, six_wide_NPOC_Head)
peaks_3to4 <- synchrony::peaks(six_wide_NPOC_Head, six_wide_NPOC_Cragg)
peaks_3to5 <- synchrony::peaks(six_wide_NPOC_Head, six_wide_NPOC_West)
peaks_3to6 <- synchrony::peaks(six_wide_NPOC_Head, six_wide_NPOC_Tunnel)
peaks_4to5 <- synchrony::peaks(six_wide_NPOC_Cragg, six_wide_NPOC_West)
peaks_4to6 <- synchrony::peaks(six_wide_NPOC_Cragg, six_wide_NPOC_Tunnel)
peaks_5to6 <- synchrony::peaks(six_wide_NPOC_West, six_wide_NPOC_Tunnel)
