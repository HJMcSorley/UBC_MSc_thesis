...
Hobo_WxLWSA <- LWSA_meanWx %>%
  ...
  right_join((Hobo %>% filter(TidbiT_location == "air")), by = "DateTime") %>% 
...
Hobo_WxLWSA %>% 
  mutate(Date = as_date(DateTime)) %>%
  group_by(site, Date) %>% 
  summarise(Hobo_Tdaily = mean(Hobo_Temp),
            FWx_Tdaily = mean(LWSA_Tmean)) %>% 
  ggplot(aes(x = Date))+
  geom_line(aes(y = Hobo_Tdaily, colour = "Sub-basin TidbiTs"), size = 0.8)+
  geom_line(aes(y = FWx_Tdaily, colour = "CRD FWx"), size = 0.8)+
  scale_color_brewer(palette = "Dark2")+ 
  theme_bw()+
...
Hobo_WxLWSA %>%
  mutate(month = lubridate::month(DateTime, label = TRUE),
         year = lubridate::year(DateTime)) %>% 
  group_by(site, TidbiT_location, year, month) %>% 
