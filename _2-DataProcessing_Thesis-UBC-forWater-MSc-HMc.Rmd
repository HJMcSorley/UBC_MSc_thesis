...
LWSA_meanWx <- read_csv("R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/FWx-Mean-LWSA_PrecipTemp.csv", col_names = TRUE, col_types = list("T", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d")) %>% 
  ...
  distinct(DateTime, .keep_all = TRUE) # I guess I ended up with duplicate rows, woops.
# resave this one:
#  write_csv("R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/FWx-Mean-LWSA_PrecipTemp.csv", col_names = TRUE)
...
* fix snow data -- SR50a noise creates weird 3m error
```{r, FWx-plots}
...
LWSA_meanWx <- precip_data %>% 
  select(DateTime, rain_season) %>%  # just to get rainseason from precip_data df
...
# LWSA Wx -- stack precip and temp with cowplot
cowplot::plot_grid(mean_rainplot, mean_snowplot, mean_tempplot, ncol = 1, rel_heights = c(3,3,4))
...
n_subbbasinSamples <- sixfilter %>% 
  dplyr::filter(sample_type == "Rack" & QAQC_flag == "OK" | sample_type == "Grab") %>%
...
n_installRackSamples <- sixfilter %>% 
  dplyr::filter(sample_type == "Rack" & QAQC_flag == "OK") %>% 
  summarise(number_of_samples = n()) %>% pull() 
...
n_installRackSamples_OK <- sixfilter %>% 
  dplyr::filter(sample_type == "Rack" & QAQC_flag == "OK") %>% 
  summarise(number_of_samples = n()) %>% pull() 
...
tibble::tibble(
  "synoptic Grab samples outside of monitoring sites" = n_SynopticGrabs - n_installGrabSamples,
  "opportunistic Grab samples" = (n_Grabs)-(n_SynopticGrabs),
  "monitoring sites synoptic Grab samples" = n_installGrabSamples,
  "monitoring sites vertical Rack samples" = n_installRackSamples,
  "total samples analyzed for results" = n_totalSamples) %>% 
  pivot_longer(everything(),
               names_to = "sample category", values_to = "total count") %>% 
