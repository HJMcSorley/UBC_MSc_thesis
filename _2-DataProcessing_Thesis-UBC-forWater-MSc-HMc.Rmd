...
### table: DOC subbasin sites, Mean/Min/Max 
...
b <- sixfilter %>%
  ...
  summarize(site = "Summary (total)",
            ...
            DOCmedian = median(NPOC_ppm, na.rm = T), 
            DOCmax = max(NPOC_ppm, na.rm = T)) %>% 
  select(site, count, DOCmean, DOCsd, RSD, DOCmin, DOCmedian, DOCmax) %>%  # reorder
  ungroup()

# tag totals summary onto site summary
rbind(a, b) %>% 
  write_csv(path = "R-outputs_UBC-forWater-MSc_HMc/tables/DOC-Subbasin_summary.csv", col_names = T)
