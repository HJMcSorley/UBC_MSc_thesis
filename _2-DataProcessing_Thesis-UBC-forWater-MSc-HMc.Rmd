...
Hobo_WxLWSA_full <- LWSA_meanWx %>%
  ...
  mutate(Date = as_date(DateTime)) %>% 
  filter(Date >= "2018-10-23") %>% 
  full_join(Hobo %>% filter(TidbiT_location == "air") %>%  group_by(site), 
            by = "DateTime") %>% 
  rename(LWSA_Tmean = "Temp_mean", Hobo_Temp = "Temp_C") %>%
  mutate(Date = as_date(DateTime)) %>% 
  select(c(site, Date, DateTime, Hobo_Temp, LWSA_Tmean)) %>% 
  distinct() %>% 
...
Hobo_WxLWSA_full %>% 
  tidyr::expand(., nesting(site), Hobo, FWx, DateTime)
...
Hobo_WxLWSA <- Hobo_WxLWSA_full %>% 
  filter(!is.na(site),
         Date > "2019-08-23", Date < "2020-02-18") 
...
sites_lm <- Hobo_WxLWSA %>% 
  filter(Date > "2019-08-23", Date < "2020-02-18") %>% # overlapping range
  group_by(site) %>% 
