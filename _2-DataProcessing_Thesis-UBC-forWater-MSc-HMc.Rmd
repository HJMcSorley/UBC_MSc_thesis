...
max_stage_df <- sixfilter %>%
  ...
  rename(max_stage = corr_stage_cm,
         max_stage_sample = sample) %>% 
  arrange(trip) %>% 
  ungroup()

# combine and create a comparison column
right_join(max_DOC_df, max_stage_df, 
          by = c("site", "trip", "event_ID", "DateTime_sampled", "HoldTime_days", "QAQC_flag", "Tmean_airDaily")) %>% 
  group_by(site, trip) %>% 
  mutate(same_MAXsample = isTRUE(max_stage_sample == max_DOC_sample),
         same_MAXsample_DOC = isTRUE(max_DOC == NPOC_ppm),
         same_MAXsample_stage = isTRUE(max_stage == corr_stage_cm)) %>% 
...
peaksss %>% 
  ...
  scale_fill_manual(values = c(forWater_colours2["MyOrange"], "grey")) +
  facet_wrap(~site, ncol = 1, scales = "free_y", strip.position = "right") +
  labs(y = "River Stage (cm)", x = "", caption = "Trip 17-21, Events 9-12")+
...
# save 
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/narrow_events10-12_peakDOC-stage.png",
...
# ? events$StartDate[10], events$StartDate[11]

narrow_trips <- trip_df %>% filter(!is.na(trip)) %>% 
  filter(trip == 17 | trip == 18 | trip == 19 | trip == 20 | trip == 21) %>% 
  summarise(start = as.POSIXct(.[[1,2]], tz = TZ), end = as.POSIXct(.[[5,3]], tz = TZ))

# big plot with lines
stage_sample_plot +
  geom_vline(xintercept = narrow_trips$start, size = 0.7)+
  geom_vline(xintercept = narrow_trips$end, size = 0.7)
