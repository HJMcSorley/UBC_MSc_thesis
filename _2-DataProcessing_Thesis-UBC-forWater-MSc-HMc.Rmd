...
#FIX THESE -- Remake the seaosnal boxplot (Fig 17 in draft)
...
sixfilter %>% 
  ...
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~rain_season, labeller = labeller(rain_season = label),
             ncol = 2, nrow = 1) +  
  labs(x = "", y = "DOC (mg/L)")
...
# combine
sixfilter_physiographic <- left_join(sixfilter, physiographic, by = c("site")) %>%
  filter(QAQC_flag == "OK") %>% 
  # filter so all that's left is NPOC and any predictor variables
  select(c(NPOC_ppm, DateTime_sampled, rain_season, site_type, sample_type, sample, sampleStage_cm, antecedent_7day_rain, antecedent_7day_temp, latitude, longitude, strahler_order, drainage_area_km2, elevation_masl, forest_percent, wetland_percent, openWater_percent, geo_WarkGneiss_percent, geo_ArgilliteMetagreywacke_percent, geo_ChertArgilliteVolcanic_percent, geo_Metagreywacke_percent, geo_MetchosinVolcanics_percent, geo_SookeGabbro_percent, slope_med_degrees, slope_max_degrees, "1980s_logging_percent", "1990s_logging_percent", "2000s_logging_percent", "2010and2011_logging_percent"))

# feed it into RandomForests with NPOC_ppm as the dependent variable
RandoFrst_NPOC <- randomForest::randomForest(y = NPOC_ppm, data = sixfilter_physiographic, na.action=na.roughfix)
