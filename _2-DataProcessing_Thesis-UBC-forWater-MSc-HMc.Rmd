...
sampleresults %>% 
  ...
  filter(trip > 11,
         ...
         sample != "QA-QC") %>%  
  group_by(count) %>%
  dplyr::summarize(n = length(count))
...
sixfilter %>% 
  ...
  ggplot(aes(x = DateTime_sampled, y = sampleStage_cm)) +
  geom_jitter(aes(colour = NPOC_ppm)) +
...
stg_sample <- left_join(odyssey_data, (sixfilter %>% 
                                         filter(sample_type =="Grab" | sample_type == "Rack")), 
                        by = c("DateTime" = "DateTime_sampled", 
                               "source" = "site")) %>% 
  dplyr::rename(site = source) # new_name = old_name

# try to do a combo
ggplot(data = stg_sample)+
  geom_line(aes(x = DateTime, y = corr_stage_cm))+
  geom_point(aes(x = DateTime, y = sampleStage_cm, colour = sample_type))+
  #geom_point(aes(x = DateTime, y = fillStage_cm  , colour = NPOC_ppm))+
  #scale_colour_viridis(direction = -1)+
...
sixfilter %>% 
  ...
  mutate(RisingLimb = NumberXtract(sample),
         RisingLimb = factor(RisingLimb, levels = c(1:15))) %>%
  ggplot(aes(x = DateTime_sampled, y = rackstage_cm)) +
  geom_jitter(aes(colour = NPOC_ppm)) +
  scale_colour_viridis(direction = -1) +
  facet_wrap(~site, nrow = 6,
             strip.position = "right")
...
sampleresults %>% 
  ...
  mutate(site = factor(site, exclude = "Lab"),
         site = factor(site)) %>% 
  ggplot(aes(x = DateTime_sampled, y = pseudoSUVA, fill = two_seasons)) +  
  geom_jitter(shape = 21, size = 3, alpha = 0.6) +
  scale_fill_manual(values = c("#f4AB0E", "#09A4D2")) +
  theme_bw() +
  ylim(0, 20) +
...
plot rain, snow and temp at the two FWx Stns:
```{r, FWx-plots}
...
Make a table for annual precip, max snow and mean temp at the two FWx stns.
```{r, FWx-table}
# precip_data summary
precip_data %>%
...
# water-year rainfalls (Oct 1 - Sept 30) --- this one's farked
rn_wateryr <- precip_data %>%
  ...
  dplyr::summarise(rain = sum(Rn_1, na.rm = TRUE),
                   ...
                   annual_mean_temp = mean(Temp, na.rm = TRUE),
                   temp_sd = sd(Temp, na.rm = TRUE)) %>%  
   write_csv(., "R-outputs_UBC-forWater-MSc_HMc/tables/Wx_LWSA_WaterYear-summary.csv",
             col_names = c("year", "station name", "annual precip. (mm)", "max snow (m)", "mean air temp. (°C)", "stdev air temp. (±°C)"))
...
stage_plot <- odyssey_data %>% 
  ...
  scale_x_datetime(date_breaks = "1.5 months", date_labels = "%Y-%m") + #,
                   #limits = ymd_hms(c("2018-10-24 00:00:00", "2020-02-01 00:00:00")))+
