...
HT %>% 
  ...
  mutate(# DOC
    fresh_DOC_ppm = paste(c(round(HT_summ$mean_DOC[1], 2), 
    ...
    held_DOC_ppm = paste(c(round(HT_summ$mean_DOC[2], 2), 
    ...
    DOC_pValue = c(A_DOC_pvalue, B_DOC_pvalue, C_DOC_pvalue),
    sig_DOC = c(A_DOC_sig, B_DOC_sig, C_DOC_sig),
    # DOC_eq
    fresh_DOCeq_ppm = paste(c(round(HT_summ$mean_DOCeq[1], 2),
                              round(HT_summ$mean_DOCeq[3], 2), 
                              round(HT_summ$mean_DOCeq[5], 2)),
                            "±",
                            c(round(HT_summ$sd_DOCeq[1], 2), 
                              round(HT_summ$sd_DOCeq[3], 2), 
                              round(HT_summ$sd_DOCeq[5], 2))),
    held_DOCeq_ppm = paste(c(round(HT_summ$mean_DOCeq[2], 2),
                             round(HT_summ$mean_DOCeq[4], 2), 
                             round(HT_summ$mean_DOCeq[6], 2)),
                           "±",
                           c(round(HT_summ$sd_DOCeq[2], 2), 
                             round(HT_summ$sd_DOCeq[4], 2), 
                             round(HT_summ$sd_DOCeq[6], 2))),
    DOCeq_pValues = c(A_DOCeq_pvalue, B_DOCeq_pvalue, C_DOCeq_pvalue),
    sig_DOCeq = c(A_DOCeq_sig, B_DOCeq_sig, C_DOCeq_sig),
    # UV254
    fresh_UV254_ppm = paste(c(round(HT_summ$mean_UV254[1], 2), 
    ...
    held_UV254_ppm = paste(c(round(HT_summ$mean_UV254[2], 2), 
  ...
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/HoldTime_tTests.csv")
...
HT_Hobo %>% 
  ...
  mutate(date = as_date(DateTime)) %>%
  ggplot()+
  geom_line(aes(x = date, y = Temp_C), colour = forWater_colours2[["DeepBlue"]], size = 1.5)+
  geom_line(aes(x = HT_Hobo_DailyT$date, y = HT_Hobo_DailyT$T_daily), colour = forWater_colours2[["SkyBlue"]], size = 1)+
  theme_bw()+
  # plot "refrigerator" range (0-7 C)
  geom_hline(yintercept = 0, linetype = "dashed", colour = forWater_colours2[["Gray"]], size = 0.7)+
  geom_hline(yintercept = 7, linetype = "dashed", colour = forWater_colours2[["Gray"]], size = 0.7)+
  labs(x = "", y = "Mean Daily Temperature (°C)",
       colour = "Temperature measurement:")+
  theme(legend.position = "top", 
        text = element_text(size = 12))+
