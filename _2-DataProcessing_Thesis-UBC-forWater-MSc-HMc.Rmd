...
# SUMMARY FROM HT CHUNK -- summarize = HT_summ  
HT_Hobo %>% 
  group_by(group) %>% 
  summarise(lapse = first(lapse),
            mean_DOC = mean(NPOC_ppm),
...
wut <- stage_samples %>% 
  group_by(site, event_ID) %>% 
  summarise(type = first(sample_type),
            stage_min = min(corr_stage_cm),
            stage_max = max(corr_stage_cm))
#,
            #peaks_g = ggpmisc::find_peaks(corr_stage_cm))
...
trip_bysite <- stage_samples %>%
  group_by(site, trip) %>% 
  summarise(trip_start = first(Date),
            trip_end = last(Date))
...
stage_samples %>% 
  ggplot()+
  geom_line(aes(x = DateTime, y = corr_stage_cm), na.rm = TRUE, colour = "#0072B2")+  ## stage
  geom_point(aes(x = DateTime, y = sampleStage_cm, shape = sample_type), na.rm = TRUE)+ ## samples 
  scale_shape_manual(values = c("Grab" = 16, "Rack" = 17), na.translate = FALSE)+
