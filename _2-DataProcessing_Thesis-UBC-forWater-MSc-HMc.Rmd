...
sampleresults <- read_csv("R-outputs_UBC-forWater-MSc_HMc/Results_complete.csv",
  ...
  mutate(trip = factor(trip, levels = c(0:23)),
         ...
         site = factor(site),
         event_ID = factor(event_ID, levels = c(1:16)))
...
events <- read_csv("R-outputs_UBC-forWater-MSc_HMc/Wx-RainEvents.csv", col_names = TRUE) %>% 
  mutate(ID = factor(ID, levels = c(1:16)))
...
synopticfilter %>% 
  ...
  labs(caption = 
         glue::glue("DOC concentrations across fifteen sites over 16 months (", {n_SynopticGrabs+n_installRackSamples}, " samples)"), 
       x = "", y = "DOC (mg/L)")
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/DOC_15sites_boxplots.png")
```

## table: subbasin summary DOC
```{r, summaryDOC-table}
...
## table: rack/grab subbasin summary DOC
```{r, summaryDOC-table}
...
#  mutate(site = forcats::fct_recode(site, 
 #                                   LeechBeach = "Leech-downstreamconf",
  ...
  write_csv(path = "R-outputs_UBC-forWater-MSc_HMc/tables/DOC-GrabSynoptic_site-conc-summary.csv", 
            col_names = T)
```

## table: grab samples DOC con 
```{r, Grab-synoptic-table}
# all together
synopticfilter %>%
  filter(sample_type == "Grab") %>% 
...
sixfilter %>% 
  ...
  ggplot(aes(x = sampleStage_cm, y = NPOC_ppm)) +
  geom_jitter(aes(colour = event_ID),
              size = 3) +
...
sixfilter %>% 
  ...
  theme(legend.position = "none") 
#ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/DOC_ridgeplot-seasonal.png")

```

### DOC scatter 1:1 plot
```{r, DOC-vs-surrogate}
...
sixfilter %>% 
  ...
  geom_point(aes(fill = rain_season), shape = 21, size = 3, alpha = 0.6)+
  scale_fill_manual(values = c("wet" = "#62ACC8", "dry" = "#f4AB0E"))+
  geom_abline(slope = 1, intercept = 0, linetype = 2)+
  theme_bw()+
  ylim(NA, 19) + 
  xlim(NA, 19)+
  labs(x = "DOC (ppm, as NPOC)",
       y = "DOC surrogate (ppm eqv., from UV abs.)",
