...
### table: synoptic grabs by site Mean/Min/Max  
...
synopticfilter %>%
  ...
  write_csv(path = "R-outputs_UBC-forWater-MSc_HMc/tables/DOC-GrabSynoptic_site-summary.csv", 
            col_names = T)
```

### table: synoptic grabs allover Mean/Min/Max  
```{r, Grab-synoptic-table}
# all together
synopticfilter %>%
  filter(sample_type == "Grab") %>% 
  summarize(count = n(), 
            ...
            DOCmedian = median(NPOC_ppm, na.rm = T), 
            DOCmax = max(NPOC_ppm, na.rm = T)) %>% 
  write_csv(path = "R-outputs_UBC-forWater-MSc_HMc/tables/DOC-GrabSynoptic_all-summary.csv", 
            col_names = T)
```

### table: all synoptic sites, all samples, together
...
sixfilter %>% 
  ...
  labs(x = "DOC (ppm, as NPOC)",
       y = "DOC surrogate (ppm eqv.)",
       fill = "Sampling Season",
       caption = glue::glue("Plot of DOC (measured as NPOC) versus aromatic DOC (measured via UV-Vis spectroscopy). \n The dotted line show equivalent measurements. The seasonal separation indicates samples \n generally had more aromatic character in the wet season than during the dry season \n (n = {n_DOCCDOM}: wet = {n_wet_DOCCDOM}, dry = {n_dry_DOCCDOM})"))+
  theme(text = element_text(size = 12),
        legend.position = "top")
