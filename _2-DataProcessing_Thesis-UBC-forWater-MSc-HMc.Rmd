...
full_join(
  ...
  full_join(synch_PeaksSumm_all%>% rename(`Proportion of total common DOC peaks` = `Proportion of common DOC peaks`)) %>% write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/DOC-stats_synchrony-peaks_all.csv", col_names = T)
...
max_stage_df <- sixfilter %>%
  ...
  select(c(site, trip, event_ID, DateTime_sampled, sample, NPOC_ppm, sampleStage_cm, HoldTime_days, QAQC_flag, Tmean_airDaily)) %>% 
  group_by(site, trip) %>% 
  slice(which.max(sampleStage_cm)) %>%  # subsets the data set for max values of DOC :) 
  rename(max_stage = sampleStage_cm,
         max_stage_sample = sample) %>% 
  arrange(trip) %>% 
  ungroup()

# combine and create a comparison column
maxDOCstage_df <- right_join(max_DOC_df, max_stage_df, 
          by = c("site", "trip", "event_ID", "DateTime_sampled", "HoldTime_days", "QAQC_flag", "Tmean_airDaily")) %>% 
  group_by(site, trip) %>% 
  mutate(same_MAXsample = (max_stage_sample == max_DOC_sample),
         same_MAXsample_DOC = (max_DOC == NPOC_ppm),
         same_MAXsample_stage = (max_stage == sampleStage_cm)) %>% 
  select(site,  trip,  event_ID, same_MAXsample, same_MAXsample_DOC, same_MAXsample_stage) %>% 
  ungroup()
...
maxDOCstage_df %>% 
  #group_by(trip) %>% 
  summarise(n = n(),
            count = length(same_MAXsample),
            count_true = length(same_MAXsample[TRUE]),
            count_false = length(same_MAXsample[FALSE]))
...
# I'm not sure slice is working the way I want it to
...
coincident_peaks <- sixfilter %>%
  ...
  select(c(site, trip, event_ID, DateTime_sampled, sample, NPOC_ppm, sampleStage_cm, HoldTime_days, QAQC_flag, Tmean_airDaily)) %>% 
  group_by(site, trip) %>% 
  summarise(max_stage = max(sampleStage_cm),
            sample_max_stage = as.character(sample[first(which.max(sampleStage_cm))]),
            npoc_max_stage = NPOC_ppm[first(which.max(sampleStage_cm))],
            #eventID_max_stage = as.character(event_ID[first(which.max(sampleStage_cm))]),
            max_DOC = max(NPOC_ppm),
            sample_max_DOC = as.character(sample[first(which.max(NPOC_ppm))]),
            stage_max_DOC = sampleStage_cm[which.max(NPOC_ppm)],
            #eventID_max_DOC = as.character(event_ID[first(which.max(NPOC_ppm))]) 
            ) %>% 
  mutate(coincident_sample = (sample_max_stage == sample_max_DOC),
         coincident_DOC = (npoc_max_stage == max_DOC),
         coincident_stage = (max_stage == stage_max_DOC)) %>% 
  ungroup()

# subset for narrower window
coincident_peaks %>% 
  filter(trip == 17 | trip == 18 | trip == 19 | trip == 20 | trip == 21) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/narrow_peaks_DOCvsStage.csv")
...
##### test: how did these events compare
```{r}
