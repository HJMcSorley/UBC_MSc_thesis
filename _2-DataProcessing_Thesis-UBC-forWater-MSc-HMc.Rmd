...
### plot: DOC/time x site & subbasin type
...
synopticfilter %>% 
  ...
  geom_point(aes(fill = rain_seasons, shape = rain_seasons), size = 3, alpha = 0.6)+
...
sixfilter %>% 
  ...
  summarize(DOC_mean = mean(NPOC_ppm),
            ...
            RSD = (DOC_sd/DOC_mean)*100) %>% 
  ungroup() %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/table_subbasin-seasonal-DOC.csv")
...
sixfilter %>% filter(sample_type == "Rack" | 
                       sample_type == "Grab" & 
                       sample == "Grab") %>%
  ggplot(aes(x = NPOC_ppm, y = DOCeq_ppm, fill = two_seasons)) +
