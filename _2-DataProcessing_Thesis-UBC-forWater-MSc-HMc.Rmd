...
Malahat_Wx_ttest <- malahat_data %>% 
  ...
  do(tidy(t.test(malahat_data$Value[malahat_data$Year == .$Year & malahat_data$Analysis == .$Analysis],
                 malahat_data$Value[malahat_data$Year == .$lag_year & malahat_data$Analysis == .$Analysis])))

# look at 2013 - 2017 compared to 2018-2019
pre_Msc <- malahat_data %>% 
...
Malahat_Wx_ttest <-  bind_rows(
  # rain
  tidy(t.test(pre_Msc$Value[pre_Msc$Analysis == "Precipitation Amount (mm)"], 
              MSc$Value[MSc$Analysis == "Precipitation Amount (mm)"])) %>% 
    mutate(Parameter = "rain"),
  # snow
  tidy(t.test(pre_Msc$Value[pre_Msc$Analysis == "Surface Snow Depth (Point) (cm)"], 
              MSc$Value[MSc$Analysis == "Surface Snow Depth (Point) (cm)"])) %>% 
    mutate(Parameter = "snow"),
  # air temp
  tidy(t.test(pre_Msc$Value[pre_Msc$Analysis == "Temperature (Mean) (celsius)"], 
...
HT %>% 
  ...
  mutate(
    ...
    # DOC
    DOC_ppm_fresh_held = paste(c(round(HT_summ$mean_DOC[1], 2), 
    ...
    # DOC_eq
    #DOCeq_ppm_fresh_held = paste(c(round(HT_summ$mean_DOCeq[1], 2),
        ...
        #                    c(round(HT_summ$sd_DOCeq[1], 2), 
         #                     round(HT_summ$sd_DOCeq[3], 2), 
          #                    round(HT_summ$sd_DOCeq[5], 2)),
           #                 " // ",
            #                c(round(HT_summ$mean_DOCeq[2], 2),
    ...
    # UV254
    UV254_fresh_held = paste(c(round(HT_summ$mean_UV254[1], 2), 
