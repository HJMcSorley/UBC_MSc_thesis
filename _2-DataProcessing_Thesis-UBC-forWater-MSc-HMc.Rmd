...
# add samples to this and then calculate the total rain for week up to sample
sixfilter_RF <- sixfilter %>%
  # add normalized stage
  group_by(site) %>% 
  left_join(., (odyssey_norm %>% group_by(source)), 
            by = c("DateTime_sampled" = "DateTime", "site" = "source", "trip" = "interval", 
                   "corr_stage_cm", "rain_season", "event_ID"))
  
  filter(!is.na(analysis)) %>% # drop enply TOC samples (unfiltered on shimadzu)
  select(-c(Date, two_seasons, corr_stage_cm)) %>% # drop un-needed items 
  group_by(site, trip, sample) %>% 
  left_join(., antecedent_7day_Wx, by = c("Date_sampled" = "Date")) %>% ungroup() %>%  group_by(site)

# add all the watershed characteristics for each site
physiographic <- read_csv("R-inputs_UBC-forWater-MSc_HMc/subbasin_characteristics_RandomForests.csv", 
                          col_names = TRUE) %>% 
  mutate(site = factor(site, levels = c("Weeks", "ChrisCrk", "LeechHead", "CraggCrk", "WestLeech", "Tunnel")))
...
sixfilter_physiographic <- left_join(sixfilter_RF, physiographic, by = c("site")) %>%
  ...
  filter(!is.na(DateTime_sampled),
         !is.na(sampleStage_cm)) %>%
  select(c(site, DateTime_sampled, NPOC_ppm, rain_season, sampleStage_cm, antecedent_7day_rain, antecedent_7day_temp, latitude, longitude, strahler_order, drainage_area_km2, elevation_masl, forest_percent, wetland_percent, openWater_percent, geo_WarkGneiss_percent, geo_ArgilliteMetagreywacke_percent, geo_ChertArgilliteVolcanic_percent, geo_Metagreywacke_percent, geo_MetchosinVolcanics_percent, geo_SookeGabbro_percent, slope_med_degrees, slope_max_degrees, 
           eightiesLogging = "1980s_logging_percent", 
           ninetiesLogging = "1990s_logging_percent", 
           twothousandsLogging = "2000s_logging_percent", 
           twentytenElevenLogging = "2010and2011_logging_percent")) #removed: c(trip, sample_type) # remove features that seems to disrupt the algorithm by overshadowing 
...
# mtry = (length(sixfilter_physiographic)-1)/3 = number of features/3 =~= 10
RF_NPOC <- randomForest::randomForest(NPOC_ppm ~ ., 
                                             data = RF_sixPhysiog, 
                                             na.action = randomForest::na.roughfix,
                                             mtry = 10, ntree = 500,  # ntree default = 500
                                             importance = TRUE,
                                             proximity = TRUE) 

# use functions of randomForestExplainer on obtained results
# measure_importance() == each row corresponds to a predictor
RF_NPOC_importance <- randomForestExplainer::measure_importance(RF_NPOC)
