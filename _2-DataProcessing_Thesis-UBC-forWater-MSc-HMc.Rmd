...
event_starts <- as.POSIXct(events$StartDate)
  
# stage at each of the subbasins
stage_sample_plot <- stage_samples %>% 
  ggplot(aes(x = DateTime))+
  geom_vline(xintercept = c(trip_starts), size = 0.7, colour = forWater_colours2["MyOrange"])+ 
  geom_vline(colour = forWater_colours1["MainBlue"], linetype = "twodash", 
             xintercept = c(event_starts), size = 0.65, show.legend = TRUE)+ 
...
#### Slice: DOC subbasin MAX DOC and stage 

```{r}
# trips and sampling coverage:
# 4 was good for all except Tunnel (and 5 was good for Tunnel)
# 17 & 18* collected racks for all and 2 grabs on falling limb (18)
# 19 & 20 " "
...
max_DOC <- sixfilter %>%
  filter(sample_type == "Grab" | sample_type == "Rack",
         trip == 4 | trip == 5 | trip == 17 | trip == 18 | trip == 19 | trip == 20,
         sample != "Algae", # remove sample containing whole algae (high DOC)
         QAQC_flag == "OK") %>% # remove samples with extended hold-time  
  select(c(site, trip, event_ID, DateTime_sampled, sample, NPOC_ppm, corr_stage_cm, HoldTime_days, QAQC_flag, Tmean_airDaily)) %>% 
  group_by(site, trip) %>% 
  slice(which.max(NPOC_ppm)) %>%  # subsets the data set for max values of DOC :) 
  arrange(trip) %>% 
  ungroup()

# isolate data related to max stage at each site (highest bottle on rack, or peak grab) 
max_stage <- sixfilter %>%
  filter(sample_type == "Grab" | sample_type == "Rack",
         trip == 4 | trip == 5 | trip == 17 | trip == 18 | trip == 19 | trip == 20,
         sample != "Algae", # remove sample containing whole algae (high DOC)
  ...
  select(c(site, trip, event_ID, DateTime_sampled, sample, NPOC_ppm, corr_stage_cm, HoldTime_days, QAQC_flag, Tmean_airDaily)) %>% 
  group_by(site, trip) %>% 
  slice(which.max(corr_stage_cm)) %>%  # subsets the data set for max values of DOC :) 
  arrange(trip) %>% 
  ungroup()

# column bind and create a comparison column
# compare: DateTime_sampled, sample
bind_cols(max_DOC, max_stage) %>% 
  group_by(site, trip) %>% 
  mutate(same_MAXsample = isTRUE(sample == sample1)) %>% 
  select(site,  trip,  event_ID, same_MAXsample,
         "DOCmax_stage" = corr_stage_cm, "DOCmax_sample" = sample,
         "max_stage" = corr_stage_cm1, "stageMax_sample" = sample1) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/peaks_DOCvsStage.csv")
```

##### stage samples narrower plot windows
# YOU ARE HERE
repeat this for the other isolated trips and summarize interesting findings
```{r}

# first, add max_DOC stage to dataframe
peaksss <- stage_samples %>% 
  ...
  full_join(stage_samples %>% group_by(site, trip), by = c("site", "trip", "sample", "sampleStage_cm"))

# stage and samples at each site by sampling event
four <- peaksss %>% 
  filter(interval == 4) %>%
  ggplot(aes(x = DateTime))+
  #geom_vline(xintercept = c(trip_starts), size = 0.7, colour = forWater_colours2["MyOrange"])+ 
  #geom_vline(colour = forWater_colours1["MainBlue"], linetype = "twodash", xintercept = c(event_starts), size = 0.65, show.legend = TRUE)+ 
  geom_line(aes(y = corr_stage_cm), size = 0.6, colour = forWater_colours2["DeepBlue"])+
  theme_bw()+
  geom_point(aes(y = sampleStage_cm, colour = "sample"), 
             fill = "grey", shape = 21, size = 2, na.rm = TRUE) +
  geom_point(aes(y = maxDOC_stage, colour = "peakDOC"), 
             fill = forWater_colours2["MyOrange"], shape = 24, size = 2, na.rm = TRUE) +  
  facet_wrap(~site, ncol = 1, scales = "free_y", strip.position = "right") +
  labs(y = "River Stage (cm)", x = "", colour = "Sample:")+
  scale_x_datetime(breaks = "1 day")+
  theme(text = element_text(size = 12),
        axis.text.x = element_text(angle = 60, hjust = 1),
        legend.position = "top") +
  #scale_color_manual(breaks = c("sample", "peakDOC",
   #                  values = c("black", forWater_colours2["MyOrange"])))+
  guides(colour = guide_legend("sample:"))


# other trips
# trip == 5 | trip == 17 | trip == 18 | trip == 19 | trip == 20) %>% 





# --- do something similiar for UV absorbance at 254 nm --- 
# isolate data related to max UV-254 at each site 
# incomplete dataset (not all samples analyzed by spectrophotometry) 
max_UV254 <- sixfilter %>%
  filter(sample_type == "Grab" | sample_type == "Rack",
         trip == 4 | trip == 5 | trip == 17 | trip == 18 | trip == 19 | trip == 20,
         sample != "Algae", # remove sample containing whole algae (high DOC)
         QAQC_flag == "OK") %>% # remove samples with extended hold-time  
  select(c(site, trip, event_ID, DateTime_sampled, sample, SAC254_Abs.m, HoldTime_days, QAQC_flag, Tmean_airDaily)) %>% 
  group_by(site, trip) %>% 
  slice(which.max(SAC254_Abs.m)) %>%  # subsets the data set for max values of DOC :) 
  arrange(trip)
