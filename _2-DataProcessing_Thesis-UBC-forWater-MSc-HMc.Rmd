...
stage_samples_dat <- read_csv("R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/subbasins_matched-chemohydro-samples.csv", col_names = TRUE) %>% 
  ...
  mutate(site = factor(site, levels = c("Weeks", "ChrisCrk", "LeechHead", "CraggCrk", "WestLeech", "Tunnel")),
         ...
         pseudo_SUVA = as.numeric(pseudo_SUVA)) # sample collection
...
cowplot::plot_grid(HoboFwx_lm, HoboFwx_lm_means, ncol = 1,
                   labels="AUTO")
# save
...
tibble::tibble(samples = c("synoptic Grabs outside of monitoring sites", "opportunistic Grabs", "monitoring sites synoptic Grabs", "monitoring sites vertical Rack", "total"),
  total = c((n_SynopticGrabs - n_installGrabSamples), (n_Grabs)-(n_SynopticGrabs), n_installGrabSamples, n_installRackSamples, n_totalSamples),
  QAQC = c((n_SynopticGrabs_ok - n_installGrabSamples_ok), (n_Grabs_ok)-(n_SynopticGrabs_ok), n_installGrabSamples_ok, n_installRackSamples_ok, n_totalSamples_ok)) %>% 
  mutate("reduction (%)" = round( (-1*((QAQC-total)/total)*100 ), digits = 1))  %>% 
  dplyr::rename("total count" = total,
                "analyzed (QA/QC count)" = QAQC) %>% 
...
## Variance:  Levene's test for homoscedasticity in stage
...
Boxplot: DOC overview boxplot by site
```{r}
...
## DOC variance: Levene's test for homoscedasticity in DOC:
...
synopticfilter <- synopticfilter %>% 
  mutate(description = case_when(
    ...
    site == "Deception-res" ~ "outlet of Leech Tunnel, SWSA",
    site == "Boneyard" ~ "downstream from Leech Tunnel"
  ))
