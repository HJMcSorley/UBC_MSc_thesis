...
sixfilter_physiographic <- left_join(sixfilter_RF, physiographic, by = c("site")) %>%
  ...
  select(c(site, DateTime_sampled, HoldTime_days, NPOC_ppm, rain_season, sampleStage_normalized, antecedent_7day_rain, antecedent_7day_temp, latitude, longitude, strahler_order, drainage_area_km2, elevation_masl, forest_percent, wetland_percent, openWater_percent, geo_WarkGneiss_percent, geo_ArgilliteMetagreywacke_percent, geo_ChertArgilliteVolcanic_percent, geo_Metagreywacke_percent, geo_MetchosinVolcanics_percent, geo_SookeGabbro_percent, slope_med_degrees, slope_max_degrees, logging_percent_FROMnineteeneightyTOtwentyeleven = "logging_percent_nineteeneighty-twentyeleven")) 
...
RF_sixPhysiog <- sixfilter_physiographic %>% 
  select(-c(site, DateTime_sampled)) %>% 
  mutate(rain_season = factor(rain_season)) %>% 
  # add a column of random values as as QA -- this should not be found to be important
  mutate(QA_randomNumbers = sample(100, size = nrow(.), replace = TRUE)) #,
...
RF_sixPhysiog_clean <- RF_sixPhysiog %>% 
  select(-c())

# ---  
# feed it into RandomForests with NPOC_ppm as the dependent variable
...
RF_NPOC <- randomForest::randomForest(NPOC_ppm ~ ., 
                                             ...
                                             mtry = length(RF_sixPhysiog)-1, ntree = 500,  # ntree default = 500
                                             importance = TRUE,
                                             proximity = TRUE) 
...
# RF_NPOC_explained <- randomForestExplainer::explain_forest(RF_NPOC, interactions = TRUE, 
                                                           data = RF_sixPhysiog)
...
RF_NPOC_relimp_1 <- randomForest::importance(RF_NPOC, type = 1) %>% 
  as.data.frame() %>%  # rather than matrix
  rownames_to_column() %>% 
  select(predictor = rowname,
         MSE = "%IncMSE") %>% # percent increase in mean square error
  mutate(predictor = fct_reorder(predictor, MSE, .desc=TRUE),
         MSE_percent = MSE/sum(MSE)*100) # add a percent MSE variable

# plot relative importance of MSE_percent
RF_plot_relimp_MSE <- RF_NPOC_relimp_1 %>%
  ggplot(aes(reorder(predictor, MSE_percent), MSE_percent)) +
  coord_flip()+
  geom_bar(stat = "identity", aes(), position="dodge", color="black", fill="navy") +
  theme_bw() +
  labs(y = "Percent contribution to MSE", x = "") +
  ggtitle("Relative importance in predicting DOC from watershed data (type 1, mean square errors)")
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/RF_rel-imp_type1-MSE")

## relative importance of features in predicting DOC
# type 2 = mean decrease in node impurity
RF_NPOC_relimp_2 <- randomForest::importance(RF_NPOC, type = 2) %>% 
  as.data.frame() %>% 
  rownames_to_column() %>% 
  select(predictor = rowname,
         RSS = IncNodePurity) %>% # residual sum of squares
  mutate(predictor = fct_reorder(predictor, RSS, .desc=TRUE),
...
RF_plot_relimp_RSE <- RF_NPOC_relimp_2 %>%
  ggplot(aes(reorder(predictor, RSS_percent), RSS_percent)) +
  coord_flip()+
  geom_bar(stat = "identity", aes(), position="dodge", color="black", fill="navy") +
  theme_bw() +
  labs(y = "Percent contribution to RSE", x = "") +
  ggtitle("Relative importance in predicting DOC from watershed data (type 2, residual sum of squares)")
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/RF_rel-imp_type1-MSE")
