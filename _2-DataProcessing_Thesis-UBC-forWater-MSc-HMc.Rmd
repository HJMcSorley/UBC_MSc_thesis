...
need to use nesting or tidyr::complete() or expand() to get a range of DateTimes for each site prior to when TidbiTs were deployed...
...
project_start <- as.POSIXct("2018-10-23 00:00:00", tz = TZ) # first overlapping field day
Hobo_start <- as.POSIXct("2019-08-24 00:00:00", tz = TZ)  # all TidbiTs deployed
project_end <- as.POSIXct("2020-02-18 00:00:00", tz = TZ) # last overlapping field day
...
Hobo_WxLWSA_full <- LWSA_meanWx %>%
  ...
  filter(as_date(DateTime) >= as_date(project_start),
         as_date(DateTime) >= as_date(project_end)) %>% 
  full_join(Hobo_FWx_joiner, by = c("DateTime")) %>% 
  rename(FWx = "Temp_mean", Hobo = "Temp_C") %>%
  #select(c(site, DateTime, Hobo, FWx)) %>%  # reorganize for viewing
  dplyr::filter(!is.na(site)) %>%  # drops 15 minute nulls
  group_by(site) %>% 
  distinct() %>% 
  mutate(Date = as_date(DateTime))
# temp on the half hour 

# isolate overlapping date ranges
...
#### Visualize overlapping estimates
```{r}
...
### Estimate earlier subbasin temps
```{r}

subbasinHoboFWx <- Hobo_WxLWSA_full %>%
  #filter(Date > "2019-08-23", Date < "2020-02-18") %>%  # for 'Hobo_WxLWSA'
  pivot_longer(cols = c("Hobo", "FWx"),
               names_to = "Temp_source",
               values_to = "Temp_C") %>% 
  mutate(Temp_source = factor(Temp_source)) %>% 
  left_join(HoboFWx_filled_L)
...
HoboFWx_filled_L %>% 
  ...
  #geom_hline(yintercept = 0, linetype = "dashed", colour = "darkgrey", size = 0.7)+
  #geom_hline(yintercept = 7, linetype = "dashed", colour = "darkgrey", size = 0.7)+
  #facet_wrap(~TidbiT_location, ncol = 1, scales = "free_y", strip.position = "right")+
  labs(x = "", y = "Mean Daily Temperature (°C)",
       colour = "Temperature:")+
  theme(legend.position = "top", 
        text = element_text(size = 12))+
