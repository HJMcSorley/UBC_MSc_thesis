...
event_starts <- as.POSIXct(round_date(events$StartDate, "10 min"), tz = TZ) 
...
```{r}
# ---
# again with simple sample points
stage_sample_plot <- stage_samples %>% 
  ggplot(aes(x = DateTime))+
  geom_line(aes(y = corr_stage_cm), size = 0.6)+ 
  theme_bw()+
  geom_point(aes(y = sampleStage_cm), shape = 21, fill = forWater_colours2["MyOrange"], na.rm = TRUE)+
  facet_wrap(~site, ncol = 1, scales = "free_y", strip.position = "right")+
  labs(y = "River Stage (cm)", x = "", shape = "Sample Type:")+
  scale_x_datetime(date_labels = "%Y %b %d",
                   date_breaks = "2 months",
...
# stack snow, temp, precip and stage with cowplot::plot_grid
cowplot::plot_grid(subasin_snow_plot, subasin_meantemp_plot, subbasin_meanrain_plot, stage_sample_plot, 
                   ncol = 1, 
                   axis = "l", 
                   align = "v",
...
peaksss <- stage_samples %>% 
  ...
  full_join(stage_samples %>% group_by(site, trip), by = c("site", "trip", "event_ID", "sample", "sampleStage_cm")) %>% 
  arrange(DateTime) %>% 
  ungroup()
...
# trip 5 -- stage and samples at each site by sampling event
five <- peaksss %>% 
  filter(interval == 5) %>%
  ggplot(aes(x = DateTime))+
  ...
  scale_fill_manual(values = c(forWater_colours2["MyOrange"], "grey")) +
  facet_wrap(~site, ncol = 1, scales = "free_y", strip.position = "right") +
  labs(y = "River Stage (cm)", x = "", caption = "Trip 5, Events 4-7")+
  scale_x_datetime(breaks = "1 day")+
  theme(text = element_text(size = 12),
        axis.text.x = element_text(angle = 60, hjust = 1),
        legend.position = "top") +
...
svntn21 <- peaksss %>% 
  ...
  scale_fill_manual(values = c(forWater_colours2["MyOrange"], "grey")) +
  facet_wrap(~site, ncol = 1, scales = "free_y", strip.position = "right") +
  labs(y = "River Stage (cm)", x = "", caption = "Trip 17-21, Events 10-12")+
