...
TOCV_results %>% 
  ...
  geom_jitter(aes(fill = site), alpha = 0.5, shape = 21) +
  theme_light() +
...
# --- Density ridge plots ---
# Grab samples -- density ridge plots
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
         ...
         sample == "Grab") %>% 
  mutate(site = factor(site, levels = c("Weeks-out", "Chris-crk", "Leech-head", "Cragg-crk", "West-Leech", "Tunnel"))) %>% 
  ggplot(aes(x = NPOC_ppm, y = fct_rev(site), colour = site)) +
  ggridges::geom_density_ridges(aes(fill = site), alpha = 0.2) +
  theme_light() +
  labs(caption = "Grab samples: density plots of DOC concentration")

# Rack sampes -- density ridge plots
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
  ...
  ggridges::geom_density_ridges(aes(fill = site), alpha = 0.2) +
  theme_light() +
  labs(caption = "Rack samples: density plots of DOC concentration")
