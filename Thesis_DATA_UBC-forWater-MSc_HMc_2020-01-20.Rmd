...
# import as two separate blocks (from two subdirectories), then alter and then merge
...
shimadzu_results1 <- list.files(path = resultsdata_path1_TOCV, pattern = "*.txt") %>% 
...
shimadzu_results2 <- list.files(path = resultsdata_path2_TOCV, pattern = "*.txt") %>% 
  set_names(str_extract(., "([a-z]{4,}+[0-9]*+)")) %>%
...
# check structure
str(shimadzu_results2)

# combine the two blocks into one shimadzu results dataframe
shimadzu_results <- bind_rows(shimadzu_results1, shimadzu_results2)
...
# the first few batched measured unfiltered samples for TOC


# add a seasonal ID (wet or dry)
# 2 seasons wide TOCV
TOCV_wetdry_results <- TOCV_results %>% 
...
```{r Spectrolyser samples and results}
...
# trips c(10, 11, 13, 15, 16)
# these measurements are counted as unique samples
# create an average for those measured in triplicate
t <- SCAN_results %>%  filter(trip == "10"|
                          trip == "11"|
                          trip == "13"|
                          trip == "15"|
                          trip == "16") %>% 
...
s <- SCAN_results %>% filter(trip != "10",
                          trip != "11",
                          trip != "13",
                          trip != "15",
                          trip != "16") %>%
  select(-c(source, measurement))

SCAN_results <- full_join(s, t)
```
...
# use long format for easy plotting later
...
# --- WIDE SUMMARY DF --- # 
sampleresults_wide <- full_join(TOCV_results, SCAN_results, by = c("trip", "site", "sample_type", "sample", "dt_sampled", "fillStage_cm")) %>% 
  mutate(site = as_factor(site),
         sample_type = as_factor(sample_type),
         sample = as_factor(sample),
         analysis = factor(analysis, exclude = "TOC"), 
         analysis = factor(analysis),   # drop residual NA from removing "TOC"
         site = factor(site, exclude = "Mystery-bottle"),
...
sampleresults_wide <- sampleresults_wide %>% 
...
# --- LONG --- # 
sampleresults_long <- full_join(TOCV_results, SCAN_results, by = c("trip", "site", "sample_type", "sample", "dt_sampled", "fillStage_cm")) %>% 
  mutate(site = as_factor(site),
         sample_type = as_factor(sample_type),
         sample = as_factor(sample),
         analysis = factor(analysis, exclude = "TOC"), 
         analysis = factor(analysis)) %>% # drop residual NA from removing "TOC"
  select(-c(vial, source, measurement)) %>%   # drop unneccesary variables
  tidyr::pivot_longer(cols = c(NPOC_ppm, Turbid.FTUeq, NO3.Neq_ppm, TOCeq_ppm, DOCeq_ppm, SAC254_Abs.m, SAC436_Abs.m),
                      names_to = "analyte",
                      values_to = "result") %>% 
  mutate(analyte = as_factor(analyte))
  
```


Six pirmary research location in the Leech River watershed were equipped with vertical sampling racks which combined passive (siphon) samplers and compact river stage loggers (Odyssey capacitiance water level loggers).
...
# note that interval (stage_df) is the same as (trip_df) trip number of analysis/collection 
  
# check out the head tail and middle
head(stage_df)
tail(stage_df, - 0.2*length(stage_df$Date))
tail(stage_df, - 0.5*length(stage_df$Date))
tail(stage_df, - 0.7*length(stage_df$Date))
tail(stage_df)
...
save the reformatted results files as .csv files:
``` 
...
write_csv(TOCV_results, path = "R-outputs_UBC-forWater-MSc_HMc/TOCV_results.csv", na = "NA")               # Shimadzu sample analyses results
write_csv(SCAN_results, path = "R-outputs_UBC-forWater-MSc_HMc/SCAN_results.csv", na = "NA")               # Spectrolyser sample analyses results
write_csv(odyssey_data, path = "R-outputs_UBC-forWater-MSc_HMc/odyssey-data_compiled.csv", na = "NA")      # stage data compiled 
write_csv(stage_df, path = "R-outputs_UBC-forWater-MSc_HMc/stage_results.csv", na = "NA")                  # stage (cm) compiled with interval/trip
write_csv(sampleresults_wide, path = "R-outputs_UBC-forWater-MSc_HMc/sample-results_wide.csv", na = "NA")  # compiled sample analyses results (wide)
write_csv(seasonal_wideresults, path = "R-outputs_UBC-forWater-MSc_HMc/seasonal3-results_wide.csv", na = "NA")  # seasonal sample analyses results (wide)
write_csv(sampleresults_long, path = "R-outputs_UBC-forWater-MSc_HMc/sample-results_long.csv", na = "NA")  # compiled sample analyses results (long)
...
# write_csv(sample_count_df, path = "R-outputs_UBC-forWater-MSc_HMc/summ_sample-DOCcount-df.csv", na = "NA")  

# how many samples by season?
seasonal_count_df <- TOCV_wetdry_results %>% 
  select("NPOC_ppm", "site", "sample_type", "sample", "season", "analysis") %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
                  site == "Cragg-crk" |
  ...
  summarise(number_of_samples = n())
# write_csv(seasonal_count_df, path = "R-outputs_UBC-forWater-MSc_HMc/summ_seasonal-DOCcount-df.csv", na = "NA")  
...
# ----
# Boxplot with jitter scatter (DOC by sample_type)
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
  ...
  #stat_summary(fun.y = min, geom = "point", shape = 24, size = 2, inherit.aes = TRUE) +
  #stat_summary(fun.y = max, geom = "point", shape = 25, size = 2, inherit.aes = TRUE) +
  facet_wrap(~sample_type, ncol = 2, nrow = 1) +  # hash this out to plot all samples together
  labs(caption = "DOC concentration by sample type (grab samples vs. rising limb rack sample)", x = "", y = "DOC (mg/L)")
#ggsave(path = "R-outputs_UBC-forWater-MSc_HMc/", filename = "DOC_boxplot-Grab-vs-Rack.png") 
# ----

# ----
# Boxplot with jitter scatter (grab and rack combined)
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
  ...
  #stat_summary(fun.y = min, geom = "point", shape = 24, size = 2, inherit.aes = TRUE) +
  #stat_summary(fun.y = max, geom = "point", shape = 25, size = 2, inherit.aes = TRUE) +
  labs(caption = "DOC concentration across six sites over 15 months (342 samples)", x = "", y = "DOC (mg/L)")
#ggsave(path = "R-outputs_UBC-forWater-MSc_HMc/", filename = "DOC_boxplot-alltogether.png")
# ----


# --- Density ridge plots ---
# Grab samples -- density ridge plots (grab samples)
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
  ...
  scale_fill_brewer(palette="Accent") +
  theme_bw() +
  labs(y = "", x = "Grab sample DOC (mg/L)") +
  theme(legend.position = "none") 


# Rack sampes -- density ridge plots (rack samples)
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
  ...
  scale_fill_brewer(palette="Accent") +
  theme_bw() +
  labs(y = "", x = "Rack sample DOC (mg/L)") +
  theme(legend.position = "none") 

# density ridge plots (facet wrap by sample_type)
# Grab: n = 114
# Rack: n = 171
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
  ...
  theme_bw() +
  labs(y = "", x = "DOC (mg/L)", 
       caption = "Density distribution plots of DOC concentration at six sites based on sampling method (Grab: n = 114, Rack: n = 171)") +
  theme(legend.position = "none") 
...
TOCV_wetdry_results %>% 
  dplyr::filter(site == "Weeks-out" |
                ...
                analysis == "DOC", 
                sample_type == "Grab" & sample == "Grab") %>% 
  mutate(site = factor(site, levels = c("Weeks-out", "Chris-crk", "Leech-head", "Cragg-crk", "West-Leech", "Tunnel", "Rithet", "Judge-crk"))) %>% 
  ggplot(aes(x = site, y = NPOC_ppm, fill = site)) +
  geom_boxplot(alpha = 0.5) +
  geom_jitter(aes(fill = site), alpha = 0.8, shape = 21) +
  scale_fill_brewer(palette="Accent") +
...
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                ...
                sample_type == "Grab" & sample == "Grab") %>% 
  mutate(site = factor(site, levels = c("Weeks-out", "Chris-crk", "Leech-head", "Cragg-crk", "West-Leech", "Tunnel", "Rithet", "Judge-crk"))) %>% 
  ggplot(aes(x = dt_sampled, y = NPOC_ppm, fill = site)) +
...
# --- facet wrap scatter plots of conc ---
...
TOCV_wetdry_results %>% 
  ...
  ggplot(aes(x = trip, y = NPOC_ppm, shape = sample_type, fill = season)) +
  scale_fill_manual(values = c("#f4AB0E", "#09A4D2")) +
  geom_jitter(shape = 21, size = 2) +
  theme_bw() +
  labs(y = "DOC (mg/L)", x = "field trip", colour = "sample type:") +
  facet_wrap(~site, ncol = 2, nrow = 3) +
  theme(legend.position = "top",
        axis.text.x = element_text(angle = 90))   # rotate tick labels for clarity
#ggsave(path = "R-outputs_UBC-forWater-MSc_HMc/", filename = "DOC_facet-scatter-seasonal.png") 


# --- scatter plot bunched ---
# DOC over time (by trip) -- try mutating in a month-year for clarity
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
  ...
  geom_jitter(aes(fill = sample_type), size = 2.5, shape = 23, alpha = 0.8) +
  scale_fill_manual(values = c("#09A4D2", "#f4AB0E")) +
  #  scale_colour_brewer(palette="Accent") +
  theme_bw() +
  labs(y = "DOC (mg/L)", x = "field trip", fill = "sample type")
#ggsave(path = "R-outputs_UBC-forWater-MSc_HMc/", filename = "DOC_scatter-sampletype.png") 
...
seasonal_wideresults %>% 
  ...
  mutate(site = factor(site, exclude = "Lab"),
         site = factor(site)) %>% 
  ggplot(aes(x = NPOC_ppm, y = DOCeq_ppm, fill = season)) +  # exchange NPOC_ppm with DOCeq_ppm -- similar pattern but split seasonally
  geom_point(shape = 21, size = 3) +
  geom_abline(lty = "dotted", slope = 1, intercept = 0) +
  scale_fill_manual(values = c("#5B99CC", "#3B3838", "#f4AB0E")) +
  theme_bw() +
  theme(legend.position = "bottom") +
  labs(x = "DOC (mg/L as NPOC)", y = "CDOM (mg/L)", fill = "sampling season:",
...
seasonal_wideresults %>% 
  filter(sample_type == "Grab" & sample == "Grab",
         site == "Weeks-out" |
                  ...
                  site == "Tunnel") %>%
  mutate(site = factor(site, levels = c("Weeks-out", "Chris-crk", "Leech-head", "Cragg-crk", "West-Leech", "Tunnel"))) %>%
  ggplot(aes(x = NPOC_ppm, y = DOCeq_ppm, fill = season)) +
  ...
  theme_bw() +
  theme(legend.position = "bottom") +
  labs(x = "DOC (mg/L as NPOC)", y = "Proxy (mg/L as CDOM)", fill = "sampling season:",
