...
# 3 seasons wide 
seasonal_wideresults <- sampleresults_wide %>% 
   mutate(trip = factor(trip, levels = 0:22),
          season = case_when(
    trip == 0 ~ "SCAN QA-QC", 
...
# 2 seasons wide 
twoseasonal_wideresults <- sampleresults_wide %>% 
   mutate(trip = factor(trip, levels = 0:22),
          season = case_when(
    trip == 0 ~ "SCAN QA-QC", 
    trip %in% 1:9 ~ "wet",   # mid-Oct 2018 to May 2019
    trip %in% 10:16 ~ "summer",  # June to mid-Oct 2019
    trip %in% 17:22 ~ "wet")) # mid-Oct 2019 to Jan 2020
...
# ----
# Boxplot with jitter scatter (DOC by site)
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
...
ggsave(path = "R-outputs_UBC-forWater-MSc_HMc/", filename = "DOC_boxplot-Grab-vs-Rack.png") Boxplot
# ----

# ----
# Boxplot with jitter scatter (DOC all together)
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
...
# --- Density ridge plots ---
# Grab samples -- density ridge plots (individual sample_type)
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
...
# Rack sampes -- density ridge plots (individual sample_type)
TOCV_results %>% 
  dplyr::filter(site == "Weeks-out" |
                  site == "Leech-head" |
                  site == "Chris-crk" |
