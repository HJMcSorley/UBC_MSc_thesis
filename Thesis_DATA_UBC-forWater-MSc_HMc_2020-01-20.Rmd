...
sample_results <- full_join(TOCV_results, SCAN_results, by = c("trip", "site", "sample_type", "sample", "dt_sampled", "fillStage_cm")) %>% 
  ...
  select(-c(vial, source, measurement, analysis)) %>%   # drop unneccesary variables
...
# long format will be easier to plot from and nicer for comprehensive analyses & stats
...
stage_df <- odyssey_data %>%
  ...
  mutate(interval = case_when(  
    Date %within% interval(trip_df$trip.start[1], trip_df$trip.start[2]) ~ trip_df$trip[2],
    Date %within% interval(trip_df$trip.start[2], trip_df$trip.start[3]) ~ trip_df$trip[3],
    Date %within% interval(trip_df$trip.start[3], trip_df$trip.start[4]) ~ trip_df$trip[4],
    Date %within% interval(trip_df$trip.start[4], trip_df$trip.start[5]) ~ trip_df$trip[5],
    Date %within% interval(trip_df$trip.start[5], trip_df$trip.start[6]) ~ trip_df$trip[6],
    Date %within% interval(trip_df$trip.start[6], trip_df$trip.start[7]) ~ trip_df$trip[7],
    Date %within% interval(trip_df$trip.start[7], trip_df$trip.start[8]) ~ trip_df$trip[8],
    Date %within% interval(trip_df$trip.start[8], trip_df$trip.start[9]) ~ trip_df$trip[9],
    Date %within% interval(trip_df$trip.start[9], trip_df$trip.start[10]) ~ trip_df$trip[10],
    Date %within% interval(trip_df$trip.start[10], trip_df$trip.start[11]) ~ trip_df$trip[11],
    Date %within% interval(trip_df$trip.start[11], trip_df$trip.start[12]) ~ trip_df$trip[12],
    Date %within% interval(trip_df$trip.start[12], trip_df$trip.start[13]) ~ trip_df$trip[13],
    Date %within% interval(trip_df$trip.start[13], trip_df$trip.start[14]) ~ trip_df$trip[14],
    Date %within% interval(trip_df$trip.start[14], trip_df$trip.start[15]) ~ trip_df$trip[15],
    Date %within% interval(trip_df$trip.start[15], trip_df$trip.start[16]) ~ trip_df$trip[16],
    Date %within% interval(trip_df$trip.start[16], trip_df$trip.start[17]) ~ trip_df$trip[17],
    Date %within% interval(trip_df$trip.start[17], trip_df$trip.start[18]) ~ trip_df$trip[18],
    Date %within% interval(trip_df$trip.start[18], trip_df$trip.start[19]) ~ trip_df$trip[19],
    Date %within% interval(trip_df$trip.start[19], trip_df$trip.start[20]) ~ trip_df$trip[20],
    Date %within% interval(trip_df$trip.start[20], trip_df$trip.start[21]) ~ trip_df$trip[21],
    Date %within% interval(trip_df$trip.start[21], trip_df$trip.start[22]) ~ trip_df$trip[22],
    Date %within% interval(trip_df$trip.start[22], trip_df$trip.start[23]) ~ trip_df$trip[23] ) )
...
sample_count_df <- TOCV_results %>% 
  filter(analysis == "DOC", sample_type == "Rack" | sample_type == "Grab") %>% 
  group_by(site, sample_type) %>% 
  summarise(number_of_samples = n())
# write_csv(sample_count_df, path = "R-outputs_UBC-forWater-MSc_HMc/sample-count-df_2020-01-19.csv", na = "NA")  

#  ---- PLOTS! ---- #
# colour-blind friendly colours:
# "#D55E00" = "red", "#009E73" = "green", "#CC79A7" = "pink", "#999999" = "grey", "#F0E442" = "yellow", "#0072B2" = "blue")
cbPalette <- c("#D55E00", "#009E73", "#CC79A7", "#999999",  "#F0E442", "#0072B2")
...
TOCV_results %>% 
  ...
  labs(caption = "DOC concentration overall", x = "", y = "DOC (mg/L)")
...
TOCV_results %>% 
  ...
  ggplot(aes(x = trip, y = NPOC_ppm, shape = sample_type, colour = sample_type)) +
...
TOCV_results %>% 
  ...
  #  scale_colour_brewer(palette="Accent") +
  theme_bw() +
  labs(y = "DOC (mg/L)", x = "time span", fill = "sample type") 
