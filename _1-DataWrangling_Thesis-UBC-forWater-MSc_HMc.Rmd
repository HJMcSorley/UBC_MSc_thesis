...
knitr::opts_chunk$set(include = FALSE, package.startup.message = FALSE, message = FALSE, warning = FALSE)
```
...
```{r, function}
# time-zone note: all Odyssey level loggers recorded in standard time (no daylight savings shift)
TZ <- "America/Los_Angeles"

# create a function: for Alpha-numeric number extraction
...
```{r, tracking, message = FALSE}

# read in file used to track trips
trip_df <- read.csv(file = "R-inputs_UBC-forWater-MSc_HMc/Leech-FieldTrip-tracking_forWater-MSc_HMc.csv", header = TRUE)
str(trip_df)
...
# no "Mean Area" variable in trips 9-23
...
```{r, Spectrolyser-par, message = FALSE}
...
The Spectrolyser outputs a fingerprint file containing absorbance values at all of the wavelengths monitored. 
...
# ---
# now join the full scan with the results data
spectral_results_all <- dplyr::full_join(FULLSCAN_results, SCAN_results,
                                     by = c("trip", "site", "sample_type", "sample", 
                                            "dt_sampled", "fillStage_cm"))
```

### Spectral indices

The .fp files can provide information about NOM structure. For example, the ratio of the slope between 275-295nm and the slope from 350-400nm ("slope ratio (SR)"), commonly used as an indicator of molecular weight. Or more tidy is the ratio of SAC250/SAC365, which provides the same info as SR but is WAY less confusing to calculate. Spectral slopes are weird and I don't understand them or trust I'm calculating them appropriately. An alternative is A250:A365 ("E2:E3") (below)
...
# first, check that SAC254_Abs/m is ABS/m @254nm (or are the fp values AU absorbances?)
spectral_results_all %>%
  filter(site != "Lab") %>% 
  group_by(site, sample_type, sample, SAC254_Abs.m) %>% 
  summarise(twofiftyfour_abs = mean(c(Abs_252.5, Abs_255)), # actually abs at 253.75
            twofiftyfour_SAC = mean(c(Abs_252.5/0.035, Abs_255/0.035)))  
# Looks like the .fp files contain specific absorbance coefficients rather than absorbance data

# remove samples with turbidity
# how many samples will I lose by filtering out those with turbidity>0?
100*(nrow(spectral_results_all %>% filter(Turbid.FTUeq > 0.000)) / nrow(spectral_results_all))
# 10.7% loss in data

# save only samples with no turbidity
spectral_results <- spectral_results_all %>% filter(Turbid.FTUeq == 0.00000)


# Spectral slopes -- from linear regression of log~e~-transformed SAC
# read Helms, et.al (2008). Absorption spectral slopes and slope ratios as indicators of molecular weight, source and photobleaching of chromophoric dissolved organic matter. Limnol. Oceanogr, 53(3), 955-969. 
...
spectral_summary <- spectral_results %>%
  ...
  select(trip:fillStage_cm, 
         Abs_250, Abs_365, Abs_465, Abs_665, 
         Turbid.FTUeq, NO3.Neq_ppm, TOCeq_ppm, DOCeq_ppm, SAC254_Abs.m, pseudo254) %>% 
  group_by(trip, site, sample_type, sample) %>% 
  full_join(S1, by = c("trip", "site", "sample_type", "sample")) %>% 
  full_join(S2, by = c("trip", "site", "sample_type", "sample")) %>% 
  mutate(SlopeRatio = sample_slope1/sample_slope2,
         E2E3 = Abs_250/Abs_365, # aka A~250:A~365
         E4E6 = Abs_465/Abs_665, # humification indicator --  for soils (may not be appropriate in limnology)
         trip = as_factor(trip)) %>% 
  select(-c(sample_slope1, sample_slope2, Abs_250, Abs_365, Abs_465, Abs_665)) %>% 
  ungroup()
...
```{r, compile-analyses-results, message = FALSE}
...
# --- WIDE SUMMARY DF --- # 
sampleresults <- full_join(TOCV_results, spectral_summary, by = c(
  "trip", "site", "sample_type", "sample", "dt_sampled", "fillStage_cm")  ) %>% 
  mutate(trip = as_factor(trip),
         site = as_factor(site),
         ...
         site = factor(site)) %>% 
  select(-c(vial, Turbid.FTUeq)) %>%    # drop unnecessary variables
  filter(site != "Mystery-bottle", # drop "mystery bottle"
         analysis != "TOC")  # drop TOC (unfiltered on Shimadzu)

# check it out
#str(sampleresults)

# add a variable to estimate season (based on month of year)
# two seasons (wet/dry))    
...
# SUVA ------ 
# add a variable for SUVA = SAC254/[DOC] (L/mg-m)
sampleresults <- sampleresults %>% 
...
sampleresults <- within(sampleresults, 
                      SUVA <- ifelse(
                        is.na(SUVA), pseudo_SUVA, SUVA)) 

```


## Metals data
```{r, CRD-metals, message = FALSE}
...
odyssey_data <- odyssey_data %>%
  ...
  mutate(interval = case_when(  
    ...
    source == "CraggCrk" & Date %within% interval(trip_df$trip.end[21], trip_df$trip.end[22]) ~ trip_df$trip[22], # trip 21
    #Date %within% interval(trip_df$trip.end[22], trip_df$trip.end[24]) ~ trip_df$trip[24], 
    Date %within% interval(trip_df$trip.end[21], trip_df$trip.end[24]) ~ trip_df$trip[24]) ) 
...
# -------
# offsets based on data downloads and observed rack stage
...
stage_racks <- corr_stage %>% 
  group_by(source, interval) %>%  # site & trip
...
sampleresults_grabstage <- left_join(grab_sample_df, stage_grabs,  
                                     by = c("site", "trip", "DateTime_R"))  #### added trip
# fill any missing stage values with observed stage
...
results3 <- within(results2, 
  ...
  mutate(DateTime_sampled = lubridate::as_datetime(DateTime_sampled, tz = TZ),
         Date = lubridate::as_date(ifelse(
           is.na(DateTime_sampled), as_date(DateTime), as_date(DateTime_sampled))))
# great jorb!

# save as a new dataframe
results <- results3 %>% 
  mutate(DateTime = dt_sampled) %>% 
  select(-c(dt_sampled, ID, fillStage_cm)) %>%  # not needed
  mutate(two_seasons = factor(two_seasons)) %>% 
  dplyr::distinct(site, trip, sample_type, sample, DateTime_sampled, .keep_all = TRUE)  
# there were some rows full of NA (ghosts imported from Excel)
...
trip_starts <- trip_df %>% filter(!is.na(trip)) %>% pull('trip.start') %>% as.POSIXct()
trip_ends <- trip_df %>% filter(!is.na(trip)) %>% pull('trip.end') %>% as.POSIXct()

stg_sample %>% 
  ggplot()+
  geom_vline(xintercept = c(trip_starts), size = 0.7)+ 
  geom_vline(xintercept = c(trip_ends), size = 0.7, linetype = "dashed", colour = "red")+ 
...
# check it out (likely noise in snow depth)
precip_data_raw %>% 
  filter(StationName == "FWx Chris Creek" | StationName == "FWx Martins Gulch") %>% 
  ggplot(aes(x = DateTime, y = SnowDep))+
  geom_point(na.rm = TRUE)+
  theme_bw()+
  facet_wrap(~StationName, ncol = 1)

# for both Martin's Gulch & Chris Creek, remove all SnowDep values > max snow in 2020
maxSnow_2020 <- precip_data_raw %>% 
  filter(year(DateTime) == 2020,
         SnowDep < 0.75) %>% 
  group_by(StationName) %>%  
  summarise(max_snow_2020 = max(SnowDep, na.rm = TRUE)) 
  
# filter data
precip_data <- precip_data_raw %>% 
  mutate(SnowDepth = case_when(
    StationName == maxSnow_2020$StationName[[1]] & SnowDep > maxSnow_2020$max_snow_2020[[1]] ~ "NA",
    StationName == maxSnow_2020$StationName[[2]] & SnowDep > maxSnow_2020$max_snow_2020[[2]] ~ "NA",
    StationName == maxSnow_2020$StationName[[3]] & SnowDep > maxSnow_2020$max_snow_2020[[3]] ~ "NA",                               TRUE ~ as.character(SnowDep)
    ),
         SnowDep = as.numeric(SnowDep),
         SnowDepth = as.numeric(SnowDepth))

# check again
precip_data %>% 
  filter(StationName == "FWx Chris Creek" | StationName == "FWx Martins Gulch") %>% 
  ggplot(aes(x = DateTime, y = SnowDepth))+
  geom_point(na.rm = TRUE)+
  theme_bw()+
  facet_wrap(~StationName, ncol = 1)
# woot! got that fixed :)
...
#### Rain Events defined based on 15-minute rain using Rainmaker::RMevents
...
stage_sample_plot <-  stg_sample %>% 
  ggplot(aes(x = as_date(Date)))+
  geom_line(aes(y = corr_stage_cm), na.rm = TRUE, colour = "#0072B2")+
  geom_point(aes(y = sampleStage_cm), fill = "white", colour = "black", na.rm = TRUE, shape = 21)+
  ...
  labs(y = "River stage (cm)", x = "", fill = "Sample Type")+
...
# use Rainmaker to delineate storms
...
### update stg_sample with rain events
```{r}
# ugly code below -----
...
stg_sample <- stg_sample %>%
  mutate(event_ID =
           case_when(
             Date %within% interval(events$StartDate[1], events$StartDate[2]) ~ events$ID[1],
             Date %within% interval(events$StartDate[2], events$StartDate[3]) ~ events$ID[2],
             Date %within% interval(events$StartDate[3], events$StartDate[4]) ~ events$ID[3],
             Date %within% interval(events$StartDate[4], events$StartDate[5]) ~ events$ID[4],
             Date %within% interval(events$StartDate[5], events$StartDate[6]) ~ events$ID[5],
             Date %within% interval(events$StartDate[6], events$StartDate[7]) ~ events$ID[6],
             Date %within% interval(events$StartDate[7], events$StartDate[8]) ~ events$ID[7],
             Date %within% interval(events$StartDate[8], "2019-01-25") ~ events$ID[8],
             Date %within% interval(events$StartDate[9]-10^5.5, events$StartDate[10]) ~ events$ID[9],
             Date %within% interval(events$StartDate[10], events$StartDate[11]) ~ events$ID[10],
             Date %within% interval(events$StartDate[11], events$StartDate[12]) ~ events$ID[11],
             Date %within% interval(events$StartDate[12], events$StartDate[13]) ~ events$ID[12],
             Date %within% interval(events$StartDate[13], events$StartDate[14]) ~ events$ID[13],
             Date %within% interval(events$StartDate[14], events$StartDate[15]) ~ events$ID[14],
             Date %within% interval(events$StartDate[15], events$StartDate[16]) ~ events$ID[15],
             Date %within% interval(events$StartDate[16], events$StartDate[17]) ~ events$ID[16],
             Date %within% interval(events$StartDate[17], events$StartDate[18]) ~ events$ID[17],
             Date %within% interval(events$StartDate[18], "2020-03-01") ~ events$ID[18]
           )  ) %>% 
  # add seasons
...
### update corr_stage with rain events
```{r}
...
### update precip data with rain events
```{r update}
...
# fill in any gaps
precip_data <- within(precip_data, 
                      rain_season <- ifelse(
                        is.na(rain_season), "dry", rain_season)) 

```

# Add Hold-Time span for each sample
```{r}

# I am sure this could be a loop!?!?!?!?!

# add a HoldTime_span for period each rack sample sat on the rack between collection & retrieval
results <- results %>% 
  ...
  mutate(Date_sampled = as_date(DateTime_sampled),
         Date_collected = as_date(collection)) %>% 
  mutate(HoldTime_days = Date_collected - Date_sampled) 
...
Hobo <- Hobo_dat %>% 
  ...
  filter(Temp_C < 35.0) %>% 
  mutate(DateTime = lubridate::mdy_hms(DateTime, tz = TZ),
         location = stringr::str_sub(source, -3, -1),  # last three characters (wet/dry)
...
## Add trips
# I don't know how to loop or map between dataframes (only within one)...
#### MAYBE lapply() with a function!?!?!?!
# continuing with this janky method:
Hobo_Trip <- Hobo %>%
  select(-c(source, num)) %>%
  mutate(Date = as_date(DateTime),
         trip = case_when(  
...
## Join Hobo with sixfilter

```{r}
# subset updated results
# rack samples
sixfilter_Rack_join <- results %>% 
  filter(site == "Weeks" | 
           site == "ChrisCrk" | 
           site == "LeechHead" |
           site == "CraggCrk" |
           ...
           site == "Tunnel",
         sample_type == "Rack") %>% 
  mutate(Date = as_date(DateTime_sampled)) %>% 
  filter(Date >= "2019-08-21",
         !is.na(site), !is.na(sample_type)) %>% 
  # summarize
  group_by(site, sample_type, sample, trip, Date) %>% 
  summarise(join_date = first(Date_sampled),  # for joining
            Date_sampled = first(Date_sampled),
            Date_collected = first(Date_collected),
            sample_HT_days = first(HoldTime_days),
            NPOC_ppm = first(NPOC_ppm),
            SAC254_Abs.m = first(SAC254_Abs.m),
            "DOCeq_ppm" = first("DOCeq_ppm")) %>% 
  ungroup() %>% 
  # replicate dates for the full range between date_sampled and date_collected
  group_by(site, sample_type, sample, trip, NPOC_ppm, sample_HT_days) %>% 
  # create sequence of daily dates between start & end
  mutate(join_date = map2(Date_sampled, Date_collected, seq, by = "1 day")) %>% 
  select(-Date) %>%
  unnest(cols = c(join_date)) %>%    # unnest the list column
  ungroup

# grab samples
sixfilter_Grab_join <- results %>% 
  filter(site == "Weeks" | 
           site == "ChrisCrk" | 
           site == "LeechHead" |
           site == "CraggCrk" |
         ...
         sample_type == "Grab") %>% 
  mutate(Date = as_date(DateTime_sampled)) %>% 
  filter(Date >= "2019-08-21", !is.na(site), !is.na(sample_type)) %>% 
  # summarize
  group_by(site, sample_type, sample, trip, Date) %>% 
  summarise(join_date = first(Date_sampled),  # for joining (no hold-time for grab samples)
            Date_sampled = first(Date_sampled),
            Date_collected = first(Date_collected),
            sample_HT_days = first(HoldTime_days),
            NPOC_ppm = first(NPOC_ppm),
...
# join all daily summaries
Hobo_RacknGrab <- Hobo_Trip %>% 
  filter(!is.na(location)) %>% 
  full_join(sixfilter_Rack_join, by = c("site" , "trip", "Date" = "join_date")) %>% 
  full_join(sixfilter_Grab_join, by = c("site", "sample_type", "sample", "trip", 
                                        "Date" = "join_date", 
                                        "Date_sampled", "Date_collected", "sample_HT_days", "NPOC_ppm", "SAC254_Abs.m", "DOCeq_ppm")) %>% 
...
# now, summarize the temperature for each Rack sample based on it's hold-time
subbasin_Hobos <- Hobo_RacknGrab %>% 
  filter(sample_type == "Grab" | sample_type == "Rack") %>%  
  group_by(site, location, sample_type, sample, NPOC_ppm, SAC254_Abs.m, DOCeq_ppm) %>% 
  summarise(event_ID = first(event_ID),
            rain_season = first(rain_season),
            sample_HT_days = first(sample_HT_days),
            trip = first(trip),
            Date_sampled = first(Date_sampled),
            Date_collected = first(Date_collected),
            Temp_daily = mean(Temp_C, na.rm = TRUE),
            Temp_daily_sd = sd(Temp_C, na.rm = TRUE)) %>% 
  ungroup() %>% 
  rename(TidbiT_location = "location") %>% 
  # rearrange
  select(trip, event_ID, rain_season, site, sample_type, sample, NPOC_ppm, SAC254_Abs.m, DOCeq_ppm, Date_sampled, Date_collected, sample_HT_days, TidbiT_location, Temp_daily, Temp_daily_sd)


# visualize
subbasin_Hobos %>% 
  ggplot(aes(x = Date_sampled, y = NPOC_ppm))+
  geom_point(aes(colour = trip, shape = sample_type, size = sample_HT_days))+
  theme_bw()+
  facet_wrap(~site, ncol = 1)
...
# --- compiled and formatted data:
# TOCV_results df
# Shimadzu sample analyses results
write_csv(TOCV_results, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/data_summaries/TOCV_results.csv", na = "NA")   

# SCAN_results df
# Spectrolyser sample analyses results
write_csv(SCAN_results, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/data_summaries/SCAN-par_results.csv", na = "NA")  

# FULLSCAN_results df
# Spectrolyser fullscan (.fp) analyses results
write_csv(FULLSCAN_results, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/data_summaries/SCAN-fp_results.csv", na = "NA")

# spectral_results df
# Spectrolyser fullscan (.fp) analyses results
write_csv(spectral_results,
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/data_summaries/spectral_results.csv", na = "NA")

# sampleresults df
# compiled sample analyses results (wide)
write_csv(sampleresults, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/data_summaries/samples-lab-analyses_results.csv", na = "NA")  
# odyssey_data df
# stage data compiled with interval/trip
write_csv(odyssey_data, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/data_summaries/Odyssey-stage_compiled.csv", na = "NA")



# --- to be read into 2-DataProcessing RMD:

# results
# Complete sample results dataframe with sample DateTimes
write_csv(results, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/Results_complete.csv", na = "NA")

# Hobo_Trip
# Rain events from Rainmaker
write_csv(Hobo_Trip, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/Hobo_trip-event.csv", na = "NA")

# subbasin_Hobos
write_csv(subbasin_Hobos, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/Subbasin_Results_complete.csv", na = "NA")

# corr_stage df
# baseflow and rack corrected Odyssey data
write_csv(corr_stage, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/Odyssey-RackCorrected-stage.csv", na = "NA")

# precip_data df
# 2018-2020 weather station data compiled and formatted
write_csv(precip_data, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/FWx-PrecipTemp_compiled.csv", na = "NA")

# wx_mean df
# 2018-2020 weather station data averages from FWx stations Chris Creek and Martins Gulch
write_csv(wx_mean, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/FWx-Mean-LWSA_PrecipTemp.csv", na = "NA")

# metalslab df
# metals sample analyses results with OC (long)
write_csv(metalslab, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/metals-DOCgrab-sample_results-long.csv", na = "NA") 

# stg_sample
# subbasin sample results with stage, timestamps, and precip event ID
write_csv(stg_sample, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/Subbasins_matched-chemohydro-samples.csv", na = "NA")

# events
# Rain events from Rainmaker
write_csv(events, 
"R-outputs_UBC-forWater-MSc_HMc/wrangled_dataframes/Wx-RainEvents.csv", na = "NA")
