...
```
#r, QA-QC}
# check stages of missing samples
corr_stage %>% 
  filter(interval == 23,
         source == "ChrisCrk") %>% 
  summarise(min_stage = min(corr_stage_cm),
            max_stage = max(corr_stage_cm))
```
...
stg_sample <- stg_sample %>%
  mutate(event_ID =
    ...
    DateTime %within% interval(events$StartDate[16], "2019-02-30") ~ events$ID[16]
             )  )
...
results <- results %>% 
  ...
  mutate(event_ID =
    ...
    Date %within% interval(events$StartDate[16], "2019-02-30") ~ events$ID[16]
             )  ) %>% 
  # add seasons
