...
results <- results %>% 
  ...
  mutate(Date_sampled = date(DateTime_sampled),
         Date_collected = date(collection)) %>% 
...
sixfilter_join <- results %>% 
  ...
  # summarize
  group_by(site, sample, trip, Date) %>% 
  ...
  group_by(site, sample, trip, NPOC_ppm, sample_HT_days) %>% 
...
Hobo_Rack_joined <- Hobo_Trip %>% 
  filter(!is.na(location)) %>% 
  full_join(sixfilter_join, by = c("site", "trip", "Date" = "join_date"))

# yes!

# now, summarize the temperature for each sample based on it's time on the rack
subbasin_Hobos <- Hobo_Rack_joined %>% 
  group_by(site, location, sample, rain_season, trip, NPOC_ppm, sample_HT_days) %>% 
  summarise(Date_sampled = first(Date_sampled),
            Date_collected = first(Date_collected),
...
Hobo_plot <- Hobo_full %>% 
  ...
  summarise(daily_temp = mean(Temp_C, na.rm = TRUE),
            Date_sampled = first(Date_sampled)) %>% 
  ggplot(aes(x = Date, y = daily_temp))+
  geom_line(aes(colour = location), size = 0.8)+
  scale_color_manual(values = c("water" = "#62ACC8", # forWater "SkyBlue"
                                "air" = "#f4AB0E"))+ # my orange
  facet_wrap(~site, ncol = 1, 
             strip.position = "right")+
  labs(y = "Temperature (°C)", x = "", location = "Temperature of:") +
  scale_x_date(date_labels = "%Y %b %d", date_breaks = "1 months", date_minor_breaks = "1 week")+
  theme_bw()+
  theme(#axis.text.x = element_text(angle = 60, hjust=1),
        text = element_text(size=12),
        legend.position = "top") 
...
Hobo_plot +
  geom_vline(xintercept = Hobo_full$Date_sampled, colour = "#CC79A7")+  
  geom_vline(xintercept = Hobo_full$Date_collected, colour = "#648326", size = 0.75)+
  scale_linetype(name = "Rack Samples")+
  guides(linetype = guide_legend(override.aes = c(colour = c("#CC79A7", "#648326"))))
