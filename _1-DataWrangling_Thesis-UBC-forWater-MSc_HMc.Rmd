...
sampleresults_grab <- sampleresults_grabstage %>% 
  ...
  select(-c(sampleStage_cm.x, sampleStage_cm.y, DateTime_R))
...
results1 <- full_join(sampleresults_grab, sampleresults_rack, 
                 by = c("site", "trip", "sample_type", "sample", "DateTime", "dt_sampled", "sampleStage_cm", "NPOC_ppm", "analysis"))
...
results <- results %>% 
  ungroup() %>% 
  select(-c("trip_end", "three_seasons", "dt_sampled", "corr_stage_cm.x", "corr_stage_cm.y")) %>% 
  mutate(trip = factor(trip), 
         site = factor(site), 
         sample_type = factor(sample_type),
...
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/hydro-samples_subbasins.png",
...
# stg_sample
# subbasin sample results with stage and timestamps
