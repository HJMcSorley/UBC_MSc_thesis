...
rack_sample_df <- sixfilter %>% 
  select(c(site, trip, sample_type, sample, dt_sampled, sampleStage_cm, analysis, NPOC_ppm)) %>%
...
stage_grabs <- corr_stage %>% 
  ...
  #mutate(sampleStage_cm = round(corr_stage_cm)) %>%   # first occurrence of stage
  #droplevels() %>% 
  group_by(site, trip) #%>% 
  #dplyr::distinct(DateTime_R, .keep_all = TRUE)    
  # first stage each hour
...
results1 <- full_join(sampleresults_grab, sampleresults_rack, 
                 by = c("site", "trip", "sample_type", "sample", "DateTime", "dt_sampled", "sampleStage_cm", "NPOC_ppm", "analysis", "corr_stage_cm"))
...
results <- full_join(results1, sampleresults, 
...
stg_sample %>% 
  ...
  geom_point(aes(x = DateTime, y = sampleStage_cm, fill = sample_type),
             na.rm = TRUE, 
             shape = 21, colour = "black")+
  scale_fill_manual(values = c("Grab" = "#E69F00", "Rack" = "#009E73"),
                     na.translate = FALSE)+
...
stg_sample <- stg_sample %>%
  mutate(event_ID =
             case_when(
    lubridate::round_date(DateTime, unit = "minute")  %within% interval(lubridate::round_date(events$StartDate[1], unit = "minute"), lubridate::round_date(events$EndDate[1], unit = "minute")) ~ events$ID[1],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[2], unit = "minute"), round_date(events$EndDate[2], unit = "minute")) ~ events$ID[2],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[3], unit = "minute"), round_date(events$EndDate[3], unit = "minute")) ~ events$ID[3],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[4], unit = "minute"), round_date(events$EndDate[4], unit = "minute")) ~ events$ID[4],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[5], unit = "minute"), round_date(events$EndDate[5], unit = "minute")) ~ events$ID[5],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[6], unit = "minute"), round_date(events$EndDate[6], unit = "minute")) ~ events$ID[6],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[7], unit = "minute"), round_date(events$EndDate[7], unit = "minute")) ~ events$ID[7],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[8], unit = "minute"), round_date(events$EndDate[8], unit = "minute")) ~ events$ID[8],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[9], unit = "minute"), round_date(events$EndDate[9], unit = "minute")) ~ events$ID[9],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[10], unit = "minute"), round_date(events$EndDate[10], unit = "minute")) ~ events$ID[10],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[11], unit = "minute"), round_date(events$EndDate[11], unit = "minute")) ~ events$ID[11],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[12], unit = "minute"), round_date(events$EndDate[12], unit = "minute")) ~ events$ID[12],
  
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[13], unit = "minute"), round_date(events$EndDate[13], unit = "minute")) ~ events$ID[13],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[14], unit = "minute"), round_date(events$EndDate[14], unit = "minute")) ~ events$ID[14],
           
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[15], unit = "minute"), round_date(events$EndDate[15], unit = "minute")) ~ events$ID[15],
    
    round_date(DateTime, unit = "minute")  %within% interval(round_date(events$StartDate[16], unit = "minute"), round_date(events$EndDate[16], unit = "minute")) ~ events$ID[16]
             )
  )
...
path = "R-outputs_UBC-forWater-MSc_HMc/subbasins_matched-chemohydro-samples.csv", na = "NA")
