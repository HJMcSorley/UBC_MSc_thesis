...
grab_sample_df <- sampleresults %>% 
  select(c(site, trip, sample_type, dt_sampled, fillStage_cm, analysis, NPOC_ppm, DOCeq_ppm)) %>% 
...
stage_grabs <- corr_stage %>% 
  ...
  rename(site = source, trip = interval) %>% 
  #mutate(sampleStage_cm = round(corr_stage_cm)) %>% 
  mutate(trip = factor(trip, levels = c(1:23))) %>% 
  mutate(site = factor(site, levels = c("Weeks", "ChrisCrk", "LeechHead", "CraggCrk", "WestLeech", "Tunnel"))) %>% 
  group_by(site, trip) %>% 
  dplyr::distinct(DateTime, .keep_all = TRUE)    
  # first stage at the top of the hour
  
# Join grab-stage and grab-sample dataframes
sampleresults_grabstage <- fuzzyjoin::fuzzy_left_join(grab_sample_df, stage_grabs,  
                 by = c("site" = "site", 
                        "trip" = "trip", 
                        "dt_sampled" = "DateTime"),
                 match_fun = list(`==`, `==`, `>=`)) #%>% 
...
sampleresults_grab <- sampleresults_grabstage %>% 
  dplyr::rename(sampleStage_cm = corr_stage_cm,
                ...
                trip = "trip.x") %>% 
  group_by(site, trip) %>% 
  dplyr::distinct(DateTime, NPOC_ppm, .keep_all = TRUE) %>% 
  select(-c(fillStage_cm, ID, site.y, trip.y))
