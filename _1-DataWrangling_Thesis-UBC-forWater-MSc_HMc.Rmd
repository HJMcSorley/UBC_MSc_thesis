...
# isolate subbasin rack samples 
rack_sample_df <- sampleresults %>% 
  select(c(site, trip, sample_type, sample, dt_sampled, fillStage_cm, analysis, NPOC_ppm)) %>% 
  rename(sampleStage_cm = fillStage_cm) %>% 
  mutate(sampleStage_cm = round(sampleStage_cm)) %>% 
  filter(site == "Weeks" |
         site == "ChrisCrk" | 
         site == "LeechHead" | 
         site == "CraggCrk" | 
...
stage_racks <- corr_stage %>% 
  ...
  rename(sampleStage_cm = corr_stage_cm,
         site = source,
         trip = interval) %>% 
  mutate(sampleStage_cm = round(sampleStage_cm)) %>% 
...
sampleresults_rack <- left_join(stage_racks, rack_sample_df, 
...
grab_sample_df <- sampleresults %>% 
  select(c(site, trip, sample_type, sample, dt_sampled, fillStage_cm, analysis, NPOC_ppm, DOCeq_ppm)) %>% 
  dplyr::rename(DateTime = dt_sampled) %>%
  mutate(DateTime = lubridate::round_date(DateTime, unit = "minutes")) %>%
  mutate(ID = row_number(NPOC_ppm)) %>% 
  filter(site == "Weeks" |
         site == "ChrisCrk" | 
         site == "LeechHead" | 
         site == "CraggCrk" | 
...
# isolate unique DateTime (top of the hour) to match to sample times  
stage_grabs <- corr_stage %>% 
  ...
  rename(site = source,
         trip = interval) %>% 
  mutate(corr_stage_cm = round(corr_stage_cm)) %>% 
  dplyr::distinct(DateTime, corr_stage_cm, .keep_all = TRUE)  # first stage  at the top of the hour
# first occurrence of rack stage

# Join grab-stage and grab-sample dataframes
sampleresults_grab <- left_join(grab_sample_df, stage_grabs,  
                 by = c("site", "trip", "DateTime")) %>% 
  dplyr::rename(sampleStage_cm = corr_stage_cm) 
# if there are missing values in stage, fill them with observations
sampleresults_grab <- sampleresults_grab %>% 
  within(sampleresults_grab, 
                 sampleStage_cm <- ifelse(
                   is.na(sampleStage_cm), fillStage_cm, sampleStage_cm))  

# good!

# Finally, join these matched sample dataframes with the original sampleresults 
# result is a full dataset with time-stamped, stage, Grab, Rack & QA-QC results
# first, join rack and grabs
results <- full_join(sampleresults_grab, sampleresults_rack, 
                 by = c("site", "trip", "sample_type", "sample", "DateTime", "sampleStage_cm", "NPOC_ppm", "analysis")) %>% 
  select(-c(ID, dt_sampled, fillStage_cm))

# join to full results 
results <- full_join(results, sampleresults, 
                 by = c("site", "trip", "sample_type", "sample", "NPOC_ppm", "analysis", "DOCeq_ppm")) %>% 
  dplyr::filter(!is.na(NPOC_ppm) | !is.na(DOCeq_ppm))  
# there were some ghost rows full of NA (likely imported from Excel)

# create DateTime_sampled variable
