...
sixfilter %>% 
  dplyr::filter(analysis == "DOC", 
                sample_type == "Rack" | sample_type == "Grab") %>% 
  group_by(site) %>% 
  summarise(meanDOC = mean(NPOC_ppm),
            stdevDOC = sd(NPOC_ppm))
...
```{r, DOC-space-time-boxplot, fig.width=8, fig.height=6, echo = FALSE}
# Boxplot with jitter scatter 
# site vs DOC
