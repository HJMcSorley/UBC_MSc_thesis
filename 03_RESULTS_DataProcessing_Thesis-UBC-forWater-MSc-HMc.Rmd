...
## Sample Counts and data table summaries
...
write_csv(n_subbbasinSamples, 
          path = "R-outputs_UBC-forWater-MSc_HMc/sampleCount_subbasins.csv", na = "NA") 
...
tibble::tibble(
  ...
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/sampleCount_allSamples.csv")
...
sixfilter %>% 
  ...
  labs(caption = "Fig.1 DOC concentration across six sites over 16 months (372 samples)", 
       x = "", y = "DOC (mg/L)")
...
```{r, conc-table}
 %>% 
  ...
  group_by(site) %>% 
  summarize(count = n(), 
            DOCmean = mean(NPOC_ppm), 
            DOCsd = sd(NPOC_ppm), 
            DOCmedian = median(NPOC_ppm), 
            DOCmin = min(NPOC_ppm), 
            DOCmax = max(NPOC_ppm))
