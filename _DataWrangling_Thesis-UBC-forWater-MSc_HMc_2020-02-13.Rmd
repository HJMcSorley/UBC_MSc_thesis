...
knitr::opts_chunk$set(echo = TRUE, package.startup.message = FALSE)
```
...
FULLSCAN_results <- FULLSCAN_results %>% 
  mutate(trip = case_when(trip == "20" & site =="Cragg-crk" & 
...
# bring in WxStn data 
# Note: Chris Crk date-time was messing with the parse.
# treat crhis crk separately, then join

# object of directory and files
Wx_files <- paste0("R-inputs_UBC-forWater-MSc_HMc/CRD_FWx-Data/", 
                  list.files(path = "R-inputs_UBC-forWater-MSc_HMc/CRD_FWx-Data/", pattern = "*.CSV"))
# alphabetical: 1 = chris, 2 = martinsgulch, 3 = surveymnt

# read the date, fix chris crk datetimes and join
precip_data <- rbind(
  read_csv(Wx_files[1], col_names = TRUE,
           col_types = list("c", "c", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d")) %>%
  mutate(StationName = forcats::as_factor(StationName),
         DateTime = lubridate::parse_date_time(DateTime, "%d/%m/%Y %H:%M", exact = TRUE)),
  read_csv(Wx_files[2], col_names = TRUE,
           col_types = list("c", "c", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d")),
  read_csv(Wx_files[3], col_names = TRUE,
           col_types = list("c", "c", "d", "d", "d", "d", "d", "d", "d", "d", "d", "d"))) %>% 
...
# determine what stage was during summertime baseflow (day I added TidbiTs to racks)
# correct all water level values with offset calculated based on observed and recorded stage 
bf_offsets <- 
  odyssey_data %>% 
  filter(as.character(Date) == "2019-08-09 08:00:00" |
           as.character(Date) == "2019-08-12 08:00:00"|
           as.character(Date) == "2019-08-16 08:00:00") %>% 
  group_by(forcats::fct_explicit_na(source)) %>% 
  summarise(bf = mean(stage_cm, na.rm = TRUE)) %>% 
  ungroup()
...
corr_stage <- odyssey_data %>% 
  ...
  mutate(corr_stage_cm = case_when(
    source == "LeechHead" ~ (stage_cm - bf_offsets[[1,2]]),
    source == "ChrisCrk" ~ (stage_cm - bf_offsets[[2,2]]),  
    source == "CraggCrk" ~ (stage_cm - bf_offsets[[3,2]]),  
    source == "Tunnel" ~ (stage_cm - bf_offsets[[4,2]]),    
    source == "Weeks" ~ (stage_cm - bf_offsets[[5,2]]),     
    source == "WestLeech" ~ (stage_cm - bf_offsets[[6,2]])
  )) %>% 
  ungroup() %>% 
...
write_csv(sampleresults, 
  path = "R-outputs_UBC-forWater-MSc_HMc/Lab-analyses_sample-results-compiled.csv", na = "NA")  

# metalslab df
# metals sample analyses results with OC (long)
write_csv(metalslab, 
  path = "R-outputs_UBC-forWater-MSc_HMc/metals-OC_sample-results_long.csv", na = "NA")  
...
# first, rename sites in sample df so they match odyssey data
# filter for sub-basin sites
subbasins <- sampleresults %>% 
  ...
  mutate(site = factor(site, levels = 
         ...
         site = forcats::fct_recode(site, 
                                    ...
                                    WestLeech = "West-Leech",
                                    Tunnel = "Tunnel"))
# try new: ---- YOU ARE HERE
# previously (janky) I subset the stage df for each site
# try group_by instead
# use the df with corrected stage -- note that these offsets are likely not right
corr_stage # input with DateTimes
# use corr_stage_cm
subbasins # input that is missing Rack DateTimes



# ---- trying to modify old code
# ... old
# Inputs:
...
stg <- odyssey_data$stage_cm
# this is the time series you're targeting
t.series <- odyssey_data$DateTime  
# this is the stage you need a date-time for
fill <- sampleresults$fillStage_cm  

# assign empty vectors to hold the outputs from the loop
occur_fill <- vector(length = length(fill))
stg_fill   <- vector(length = length(fill))
time_fill  <- vector(length = length(fill))
