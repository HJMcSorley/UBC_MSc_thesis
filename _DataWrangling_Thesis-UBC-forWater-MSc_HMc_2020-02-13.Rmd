...
# tidyverse includes: dplyr, ggplot2, purrr, readr, forcats
suppressPackageStartupMessages(library(knitr))      # tidy tables, knitting docs
suppressPackageStartupMessages(library(lubridate))  # dates, times, ranges
suppressPackageStartupMessages(library(broom))      # tidy stats
...
bf_offsets <- 
  odyssey_data %>% 
  filter(as.character(Date) == "2019-08-09") %>% 
  group_by(source) %>% 
  summarise(bf = mean(stage_cm))
...
# plot it - lines over time
corr_stage %>% 
  ggplot(aes(x = DateTime, y = corr_stage_cm))+
  geom_line(aes(colour = source))

# plot it - density ridges -- could be cool -- fix order and aesthetics (move to results)
corr_stage %>% 
