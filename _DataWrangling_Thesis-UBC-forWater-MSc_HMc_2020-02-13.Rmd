...
sampleresults <- full_join(TOCV_results, SCAN_results, 
  ...
  full_join(spectral_summary, by = c("trip", "site", "sample_type", "sample")) %>% 
  mutate(trip = as_factor(trip),
         site = as_factor(site),
...
# LWSA Wx -- stack precip and temp with cowplot
cowplot::plot_grid(mean_rainplot, mean_tempplot, ncol = 1, align = "v")
# save plot
...
# save plot
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Stage-plot_subbasins.png")
                     
# plot 
# LWSA temp for the same time span
subasin_meantemp_plot <- wx_mean %>% 
  mutate(date = lubridate::as_date(DateTime)) %>% 
  group_by(date) %>% 
  dplyr::summarise(daily_Tmean = mean(Temp_mean, na.rm = TRUE)) %>% 
  ungroup()  %>% 
  ...
  labs(x = "", y = expression('Mean daily air temp ('*~degree*C*')')) +
  theme_bw() +
  scale_x_date(date_breaks = "1.5 months", date_labels = "%Y-%m")+
  theme(legend.position = "none",
        strip.background = element_blank(),
        strip.text.x = element_blank(),
        axis.text.x = element_text(angle = 90))
...
subbasin_meanrain_plot <- wx_mean %>% 
  mutate(date = lubridate::as_date(DateTime),
         Rain = factor("Rain")) %>% 
  group_by(date, Rain) %>% 
  dplyr::summarise(daily_rn = sum(Rn_1_mean, na.rm = TRUE)) %>% 
  ...
  filter(date >= "2018-10-24") %>% 
  ggplot(aes(x = date, y = daily_rn)) +
  geom_col(aes(colour = Rain), colour = "#56B4E9") +
  scale_y_reverse() +
  labs(x = "", y = "mm/day") +
  theme_bw() +
...
cowplot::plot_grid(subbasin_meanrain_plot, stage_plot, ncol = 1, 
                   axis = "l", 
                   align = "v",
                   rel_heights = c(1,4))
# save plot
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/subbasin_rain-stage_plots.png")
