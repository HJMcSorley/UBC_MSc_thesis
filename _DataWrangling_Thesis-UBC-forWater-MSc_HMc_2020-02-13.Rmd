...
precip_data %>%
  ...
  summarise(total_precip = sum(Rn15, na.rm = TRUE),
  ...
  dplyr::rename("year" = Year, 
                ...
                "sd air temp. (± °C)" = temp_sd) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/LWSA_wx-summary.csv")
...
## stream level data (second try - bring them in, add a column with site ID combine them into one then adjust date-times)
...
bf_offsets <- 
  odyssey_data %>% 
  filter(Date == as.POSIXct("2019-08-12", tz = TZ) |
          Date == as.POSIXct("2019-08-16", tz = TZ) ) %>% 
...
Wx_CC_storms <- precip_data %>% 
  filter(StationName == "FWx Chris Creek",
         DateTime >= "2018-10-23") %>% 
  as.data.frame() %>% 
  Rainmaker::RMevents(., ieHr = 8, rainthresh = 15, rain = "Rn15", time = "DateTime")
...
occur_fill <- vector(length = length(samp_df$sample))
