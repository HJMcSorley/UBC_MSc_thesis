...
# *add a hash after writing to avoid accidental over-writing later*
...
# what I want to do:
# create a subset of odyssey_data and add a column of interval numbers based on whether the logger date falls within an interval of trip_df
stage_df <- odyssey_data %>% select(Date, source, stage_cm, DateTime)  # subset
interval <- trip_df$trip.start %>% 
  purrr::map_dbl(.l = stage_df, 
                 .f = for (i in seq_along(trip_df)) {
        interval <- case_when(stage_df$Date %within% interval(trip_df$trip.start[i], trip_df$trip.start[i+1]) ~ i)
      })

# try with map_
interval <- map_dbl(
  odyssey_data, case_when(Date %within% interval(trip_df$trip.start[1], trip_df$trip.start[2]) ~ "1"
))...
...
stage_df <- stage_df %>% mutate(interval =   
                     for (i in seq_along(nrow(trip_df))) {
                       interval <- case_when(odyssey_data$Date %within% interval(trip_df$trip.start[i], trip_df$trip.start[i+1]) ~ trip_df$trip[i+1])
                     }  
)
interval

# meta programming and map?
# learn from "https://stackoverflow.com/questions/57861055/how-can-i-use-mutate-and-case-when-in-a-for-loop"
