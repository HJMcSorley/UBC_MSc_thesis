...
Water samples were collected and transported via coolers (on ice) to UBC's EcoHydrology Lab for analysis of dissolved organic carbon (DOC) concentrations. Samples were filtered (0.45-micron PES filter), acidified to bring pH below 2, and analyzed to quantify DOC (as non-purgeable organic carbon, NPOC) using a Shimadzu TOC-V auto-analyzer (High-Temperature COmbustion Method, 5310-B [@EatonA.D.ClesceriL.S.GreenbergA.E.Franson2000a]).

In addition, samples were analyzed using a Spectrolyser spectrophotometer (s::can, Vienna, Austria) which measures turbidity and the chromophoric portion of dissolved organic matter to estimate concentrations of total organic carbon (TOC), DOC as well as nitrate-nitrogen (NO^-3^-N). Samples were also measured for phosphate concentration using a field portable colouremetric test kit (HACH); all samples resulted in phosphate concentrations below detectable limits (XXXXXX mg/L).   
...
sampledata_path_TOCV <-  "R-inputs_UBC-forWater-MSc_HMc/shimadzu/samples/"

shimadzu_samples <- list.files(sampledata_path_TOCV, pattern = "*.csv") %>% 
  map(~ read_csv(file.path(sampledata_path_TOCV, .), 
                 skip = 4, col_names = TRUE)) %>% # read in all the files
  reduce(rbind)

str(shimadzu_samples)

# get the sructure correct
...
shimadzu_sample_files <- list.files(path = "R-inputs_UBC-forWater-MSc_HMc/shimadzu/samples/", 
                                    pattern = "*.csv", 
                                    full.names = TRUE)
# specify read_csv details 
# skip first 4 rows, update headers (no dashes)
sampleFUN <- function(x){
    read_csv(file = x, skip = 5, 
             #col_names = c("vial", "site", "sample_type", "sample", "dt_sampled", "fillStage_cm", "analysis", "trip"),
             #col_types = c("d", "f", "f", "c", "d", "d", "f", "f")
             )
  }

# read and bind all sample ID files -----------
# try with map_dfr()
shimadzu_samples <- purrr::map_dfr(shimadzu_sample_files, sampleFUN) 
# try with reduce() and full_join -- don't think this will work unless each is an individual dataframe already

# Load everything into the Global Environment
df_list_read2 # Assign to a list
<- shimadzu_sample_files %>%
  purrr::map(function(file_name){ # iterate through each file name
    read_csv(paste0(file_path, file_name))})


# reduce() and full_join -when each is an individual dataframe already
 shimadzu_samples <- purrr::reduce(shimadzu_sample_files, sampleFUN full_join(by = "trip"))
# ---------------------------------------------not working
...
# ---------
# try again
sampleID_filemerge <-  function(mypath){
  filenames = list.files(path = mypath, full.names = TRUE)
  datalist = lapply(filenames, function(x)
    { read_csv(file = x, skip = 4, 
               #col_names = c("vial", "site", "sample_type", "sample", "dt_sampled", "fillStage_cm", "analysis", "trip"),
               #col_types = c("d", "f", "f", "c", "T", "n", "f", "f") 
               )})
    purrr::reduce(f = function(x, y) {merge(x, y)}, x = datalist) }

shimadzu_samples <- sampleID_filemerge("R-inputs_UBC-forWater-MSc_HMc/shimadzu/samples")
