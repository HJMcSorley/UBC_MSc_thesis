...
shimadzu_results1 <- list.files(path = resultsdata_path1_TOCV, pattern = "*.txt") %>% 
  map(~ read.table(file = file.path(resultsdata_path1_TOCV, .), 
                   row.names = NULL, skip = 14, header = FALSE,
                   col.names = nineteencolumns)) %>% 
  reduce(rbind) %>% 
  select(-Mean.Area)  # drop mean area
...
shimadzu_results2 <- list.files(path = resultsdata_path2_TOCV, pattern = "*.txt") %>% 
  map(~ read.table(file = file.path(resultsdata_path2_TOCV, .), 
                   row.names = NULL, skip = 14, header = FALSE,
