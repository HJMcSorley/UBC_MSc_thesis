...
# try to add an ID column to each file
filenames <-  list.files(path = "R-inputs_UBC-forWater-MSc_HMc/shimadzu/results_TOC/block1_19columns/", pattern = "*.txt") 
fileIDs <- str_extract(filenames, "([a-z]{4,}+[0-9]*+)")  

# 1 example:  "  ans <- map2(filenames, sites, ~read_csv(.x) %>% mutate(id = .y)) "
head(
  purrr::map2_dfr(.x = filenames, .y = fileIDs, 
                  ~read.table(.x, row.names = NULL, skip = 14, header = FALSE, col.names = nineteencolumns) %>%  
                    mutate(id = .y))  
  )

# got it! 
# read in multiple files and add a column indicating trip based on course file name
# set directory path
...
# check that there are 18 columns now
str(shimadzu_results1)
...
shimadzu_results2 <- list.files(path = resultsdata_path2_TOCV, pattern = "*.txt") %>% 
  purrr::map(~ read.table(file = file.path(resultsdata_path2_TOCV, .), 
                   row.names = NULL, skip = 14, header = FALSE,
                   col.names = eighteencolumns)) %>% 
  reduce(rbind)
...
shimadzu_results <- rbind(shimadzu_results1, shimadzu_results2)
str(shimadzu_results)



# ------------ SHIMADZU ANALYSIS RESULTS ------------ # 

# combine 'shimadzu_samples' with 'shimadzu_results'

# isolate required data from results files (vial, trip, )
# the NPOC results are in column called "Result"
