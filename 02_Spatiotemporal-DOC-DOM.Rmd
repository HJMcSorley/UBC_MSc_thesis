...
While a discrete sample can not tell a complete story of water quality dynamics in a hydrologic system, it can provide information about specific attributes of the water in a certain place at a given time. 

Collection of water samples must be done consistently and carefully to avoid sample contamination or sampling errors which would generate unreliable analytical results [@CCME2011]. 

A discrete water sample should accurately represent the body from which it was collected. Non-representative sampling techniques will lead to non-representative analytical results and ultimately to erroneous conclusions [@CCME2011].

In a carbon-based world, sample contamination must be a crucial and constant consideration when sampling for NOM [@CCME2011; @Cory2011]. 

The basis of a water quality monitoring network is the collection of representative quantitative data for physical, chemical or biological parameters that help to characterize a hydrologic system over time [@Strobl2008a]. The sampling program design of a monitoring network should reflect research objectives and physical realities (e.g., spatiotemporal heterogeneity) of the system while being cost-effective and practical [@Strobl2008a; @Kirchner2006]. 

Grab-sampling is a standard method of collecting whole-water samples for laboratory or stream-side analysis [@CCME2011; @Ruhala2017]. Grab sampling can be completed at a number of sites over a short period of time (i.e., synoptic sampling) to assess spatial variations, however there is often a lack of temporal resolution due to scheduled, convenient or opportunistic sampling at relatively low frequencies [@Ruhala2017].  

It can be difficult to schedule a synoptic sampling campaign for specific weather and flow conditions, especially for short duration of rapid runoff (@Harmel2003). Flow changes in response to precipitation or melt events are of interest for NOM studies as the rising limb of the hydrograph have been shown to have higher DOC concentration that non-storm flow [@Yang2015; @Raymond2016; @Raymond2010]. It is logistically challenging and potentially dangerous to manually collect standard grab samples during high flows across multiple sites in order to capture the dynamic changes that occur during event (@Graczyk2000; @Mackay2012).
...
fig_cap = "\\label{fig:abcDOCbox} *Comparison of DOC in synoptic Grab samples colelcted at upstream locations to samples collected by Rack and Grab below the confluence of upstrea sites. A and B show grab samples from headwater (HW) locations compared to downstream (DS) monitoring sites; C shows upstream river sites (US) compared to downstream at the mainstem monitoring.*"
...
* mathched sample filling-stage with continuous logger stage could be combined with rating curve to determine mass transport or loading (future)
