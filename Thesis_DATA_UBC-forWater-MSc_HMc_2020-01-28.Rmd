...
# and how many in each season? 
sample_count_df <- sixfilter %>% 
  dplyr::filter(analysis == "DOC", 
                sample_type == "Rack" | sample_type == "Grab" & sample == "Grab") %>%
  group_by(site, two_seasons, sample_type) %>% 
  summarise(number_of_samples = n()) %>% 
  knitr::kable(., caption = "Installation sites sample counts",
               col.names = c("installation site", "season", "sample type", "number of samples"))
...
# n > 1
synoptic_count_df <- sampleresults %>% 
  dplyr::filter(analysis == "DOC", 
                sample_type == "Grab") %>%
...
sampleresults %>% 
  dplyr::filter(site == "Weeks-out" |
                  ...
                  site == "Judge-crk", 
                analysis == "DOC", 
                sample_type == "Grab" & sample == "Grab", sample_type == "Rack") %>% 
  mutate(site = factor(site, 
                       levels = c("Weeks-out", "Chris-crk", "Leech-head", "Cragg-crk", "West-Leech", "Tunnel", "Rithet", "Judge-crk"))) %>% 
...
# DOC over time (including seasons)
sixfilter %>% 
  dplyr::filter(analysis == "DOC",
                sample_type == "Rack" | sample_type == "Grab") %>%
  ggplot(aes(x = trip_end, y = NPOC_ppm, colour = sample_type, shape = two_seasons)) +
  geom_jitter(size = 2) +
  scale_colour_manual(values = c("#f4AB0E", "#09A4D2")) +
  theme_bw() +
  labs(y = "DOC (mg/L)", x = "", colour = "sample type:", shape = "season:") +
