...
# Precip data (CRD fire weather stations)
```{r Wx-precip}

# bring in WxStn data and add a id column from the site file name
...
list.files(path = Wx_path, pattern = "*.CSV") %>%
  str_extract("([A-Za-z]{8,}+)")

# read in all the files as one dataframe
# add source (ID) based on file names
# adjust date-time
precip_data <- list.files(path = Wx_path, pattern = "*.CSV") %>% 
  set_names(str_extract("([A-Za-z]{8,}+)")) %>%
  purrr::map_dfr(~ readr::read_CSV(file.path(Wx_path, .), skip = 0,
                            col_names = TRUE, .id = "source")) %>% 
...
write_csv(SCAN_results, path = "R-outputs_UBC-forWater-MSc_HMc/SCAN_results.csv", na = "NA")               

# Spectrolyser fullscan (.fp) analyses results
write_csv(FULLSCAN_results, path = "R-outputs_UBC-forWater-MSc_HMc/FINGERPRINT-scan_results.csv", na = "NA")               

# compiled sample analyses results (wide)
write_csv(sampleresults, path = "R-outputs_UBC-forWater-MSc_HMc/sample-results_wide.csv", na = "NA")  

# stage data compiled 
write_csv(odyssey_data, path = "R-outputs_UBC-forWater-MSc_HMc/odyssey-data_compiled.csv", na = "NA")

# stage (cm) compiled with interval/trip
write_csv(stage_df, path = "R-outputs_UBC-forWater-MSc_HMc/stage_results.csv", na = "NA")                  
...
sample_count_df <- sixfilter %>% 
  ...
  group_by(site, sample_type) %>%  # add 'two_seasons' for wet/dry breakdown 
  summarise(number_of_samples = n()) %>% 
  knitr::kable(., caption = "Installation sites sample counts",
               col.names = c("installation site", "sample type", "number of samples"))
...
# write_csv(synoptic_count_df, path = "R-outputs_UBC-forWater-MSc_HMc/summ_synoptic-DOCcount-df.csv", na = "NA")  
...
# count samples
synopticfilter %>% 
  dplyr::filter(analysis == "DOC", sample_type == "Grab"| sample_type == "Rack") %>%
  summarise(number_of_samples = n())
...
synopticfilter %>% 
  ...
  geom_jitter(aes(fill = sample_type), size = 2.5, shape = 21, alpha = 0.6) +
