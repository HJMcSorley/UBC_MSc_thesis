...
# assign column names
fullscan_colnames <- c("Date", "Time", "Status", paste("fp", seq(200.0, 750.0, by = 2.5)))
## read in files and add a column indicating trip #, based on source file name
...
seasonal_count_df <- sampleresults %>% 
...
# what were the primary sysnoptic sampling sites
# n > 1
synpotic_count_df <- sampleresults %>% 
  dplyr::filter(analysis == "DOC", 
                sample_type == "Grab") %>%
  group_by(forcats::fct_explicit_na(site)) %>% 
  summarise(number_of_samples = n()) %>% 
...
sampleresults %>% 
  ...
  mutate(site = factor(site, exclude = "Lab"),
         site = factor(site)) %>% 
  ggplot(aes(x = NPOC_ppm, y = DOCeq_ppm, fill = two_seasons)) +  # exchange NPOC_ppm with DOCeq_ppm -- similar pattern but split seasonally
