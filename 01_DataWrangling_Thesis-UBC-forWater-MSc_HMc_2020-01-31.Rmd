...
metals <- read_csv("R-inputs_UBC-forWater-MSc_HMc/Metals_CRD-forWaterMSc_HMc/CRD-metals-data_collated-trips3-8_nocharacternulls.csv", 
  ...
  tidyr::pivot_longer(cols = WESTLEECH:BONEYARD,
                      names_to = "Site",
                      values_to = "Value") 
...
metalssixfilter <- sampleresults %>% filter(site == "Weeks-out" |
                                        site == "Leech-head" |
                                        site == "Chris-crk" |
                                        site == "Cragg-crk" |
                                        site == "West-Leech"| 
                                        ...
                                        analysis == "DOC") %>% 
  mutate(site = fct_recode(site, WEEKSOUT = "Weeks-out", CHRISCREEK = "Chris-crk", LEECHHEAD = "Leech-head", CRAGGCREEK = "Cragg-crk", WESTLEECH = "West-Leech", TUNNEL = "Tunnel"))
  
    

metalslab <- left_join(x = metals, 
                       y = (sampleresults %>% filter(site == "Weeks-out" |
                                                       site == "Leech-head" |
                                                       site == "Chris-crk" |
                                                       site == "Cragg-crk" |
                                                       site == "West-Leech"| 
  ...
  mutate(Site = factor(Site, exclude = "BONEYARD"),
         Parameters = factor(Parameters, exclude = c("Total Silicon (Si)",
                                                     "Total Antimony (Sb)",
                                                     "Total Beryllium (Be)",
                                                     "Total Bismuth (Bi)",
                                                     "Total Boron (B)",
                                                     ...
                                                     "Total Zinc (Zn)",
                                                     "Total Zirconium (Zr)"))) %>% 
  drop_na(Site, Parameters) %>% 
  mutate(Trip = factor(Trip))

# ----- metals plots?
# limited metals with NPOC
metalslab %>% filter(Parameters == "Total Aluminum (Al)"|
                       Parameters == "Total Barium (Ba)"|
                       Parameters == "Total Copper (Cu)"|
                       Parameters == "Total Iron (Fe)"|
                       Parameters == "Total Manganese (Mn)") %>% 
