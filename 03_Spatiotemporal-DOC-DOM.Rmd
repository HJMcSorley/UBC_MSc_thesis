...
The first objective for the surface water sampling methods defined in Chapter 2 was to describe spatial and temporal patterns and variation of DOC concentrations and NOM character across the Greater Victoria water supply area (GVWSA), and to identify the influence of seasonality on concentration of DOC and character of NOM.
...
Several comparisons can be made among the twelve synoptically sampled river sites (refer to Chapter 2, Figure \@ref(fig:sampleSitesMap)). Presented here are comparisons between the Leech WSA and Sooke WSA (cross-basin), upstream to downstream, and evaluation of sampling methods with respect to nested catchments and DOC ranges. 

\  

Mean DOC concentrations were similar (4.8 - 5.7 mg/L) when comparing across basins from the current source water tributaries, Rithet and Judge Creeks, to the future supplemental supply, Leech River at the Tunnel (Table \@ref(tab:synopticDOCdata)). 
...
From upstream to downstream sites there was an overall decreasing trend for DOC concentrations and NOM aromaticity and/or molecular size  (Figure \@ref(fig:synopticBox)). 

\ 

```{r synopticBox, fig.cap = fig_cap}
fig_cap = "\\label{fig:synopticBox}*Synoptic sampling results of dissolved organic carbon concentrations from 12 sites from Oct 2018 to Feb 2020. Symbol type indicates samples were collected by siphon samplers on Vertical Racks (triangle) or synoptic Grab sample (circle), outliers are indicated with an x.*"
