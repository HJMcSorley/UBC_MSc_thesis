...
The methods defined in Chapter 2 were used to collect and analyze surface water samples with the objective of describing spatial and temporal patterns and variation of dissolved organic carbon (DOC) concentrations and natural organic matter (NOM) character across the Greater Victoria water supply area (GVWSA, refer to Figure \@ref(fig:sampleSitesMap) for map). Additionally, sampling results were interpreted with the goal of clarifying the influence of seasonality on concentration of DOC and character of NOM. 
...
Despite DOC changes over time, there was almost no difference between mean DOC concentrations during the wet and dry seasons (6.13 mg/L compared to 6.14 mg/L, Table \@ref(tab:seasonDOCcount)). However, there were far fewer DOC samples collected in the dry season (55) than during the wet season (311).  

\  

```{r seasonDOCcount}
read_csv("R-outputs_UBC-forWater-MSc_HMc/tables/Ch3_seasonal-samples-DOC-summary.csv", col_names = TRUE) %>%
  knitr::kable(digits = c(0,0,0,2,2,2,2),
               ...
               col.names = c("Season", "Sample count", "Mean DOC (mg/L) ± RSD (%)", "Min. DOC (mg/L)", "Median DOC (mg/L)", "Max. DOC (mg/L)"))
```

\  

Similarly, there were more spectral NOM characteristic samples colelcted in the wet season than the dry (Table \@ref(tab:seasonNOMcount)). 

```{r seasonNOMcount}
read_csv("R-outputs_UBC-forWater-MSc_HMc/tables/Ch3_seasonal-samples-NOM-summary.csv", col_names = TRUE) %>%
  knitr::kable(digits = c(0,0,0, 2,2,2, 0,2,2,2),
               caption = "Seasonal Sample Collection and Summary of NOM Character Indices from Twelve Synoptic Sampling Sites Across the Greater Victoria Water Supply Area",
               col.names = c("Season", "Sample count", 
                             "Mean SAC~254~ (m^-1^) ± RSD (%)", "Min. SAC~254~ (m^-1^)", "Median SAC~254~ (m^-1^)", "Max. SAC~254~ (m^-1^)", 
                             "Mean E~2~:E~3~ ± RSD (%)", "Min. E~2~:E~3~", "Median E~2~:E~3~", "Max. E~2~:E~3~"))
```


The wet season was most heavily sampled at the six monitoring sites in the Leech WSA, where Rack samplers more doubled the number of samples that would have been collected through Grab sampling alone. 

\  

##### Seasonal changes in NOM character

To evaluate if (and when) molecular character of synoptic stream samples shifted, DOC concentrations (as NPOC) were compared to specific absorbance coefficients at 254 nm (SAC~254~, used by the spectro::lyser to estimate DOC). DOC concentration was well correlated with SAC~254~ during the wet season (R^2^ = 0.896) and is the transition from dry to wet seasons (first-flush event, R^2^ = 0.916), but the relationship between DOC concentration and SAC~254~ was weak during the dry season (R^2^ = 0.396). The separation of wet and dry season samples suggests that aqueous NOM in the GVSWA had more aromatic character (more humic-like, allochthonous NOM) during the wet-season than during the dry-season. NOM in  the dry season had comparable concentrations to the wet season but the absorbance at 254 nm was not as strongly related, leading to weaker NOM spectral absorbance relationships (Figure \@ref(fig:seasonalDOCest)).
...
These results suggests that spectral estimates of DOC should be evaluated carefully and that a season-specific site-based calibration should be conducted if DOC is to be estimated by UV-Vis absorbance. Wet-season NOM character caused positive bias in absorbance-based DOC estimates and dry-season sample characteristics lead to negative bias in UV-based DOC estimates. These results indicate a seasonal shift in NOM character from more aliphatic NOM in the dry season to more aromatic NOM in the wet season (autochthonous to allochthonous). 

\  

#### Spatiotemporal patterns & event-based sampling at the six Leech monitoring sites

Isolating DOC results to the six monitoring sites during only the wet season allowed for further comparison of event-based Vertical Rack to standard synoptic Grab samples (Table \@ref(tab:wetDOCcountInstallsites)). Across the six sites, event-based Rack samples contained higher DOC concentrations on average than Grab samples (6.8 mg/L compared to 5.2 mg/L). Surprisingly, a slightly greater variance was captured through Grab sampling (53% RSD) compared to Rack sampling (41% RSD). This indicates that stormflow sustained greater concentrations of NOM compared to lower concentrations with greater relative variance in inter-event flows.   

\  

```{r wetDOCcountInstallsites}
read_csv("R-outputs_UBC-forWater-MSc_HMc/tables/Ch3_wet-InstallSamples-DOC-summary.csv", col_names = TRUE) %>%
...
Leech-head was the only site where the maximum DOC concentration was obtained by Grab sampling (11.6 mg/L), rather than Rack (max 10.6 mg/L). At all other monitoring sites, event-based samples collected by Vertical Racks had higher mean DOC compared to Grab samples (Table \@ref(tab:wetDOCcountInstallsites)). Weeks crk had the highest DOC overall, with slightly higher concentrations occurring during stream rise (10.3 mg/L mean) compared to between events (9.3 mg/L mean) and Rack sampling collected a greater representation of all concentrations (Figure \@ref(fig:wetSeasonDOC)). Interestingly, Rack sampling at Weeks crk captured both the highest and lowest DOC concentrations (minimum of 3.8 mg/L, maximum of 19.1 mg/L).  West Leech showed the greatest difference between DOC collected as Grab samples (4.2	± 1.9) and event-based Rack samples (6.7 ± 2.2 mg/L). This suggests that hydrologic pulses in the West Leech sub-basin create considerably different DOC transport than non-stormflow in that sub-basin (Table \@ref(tab:wetDOCcountInstallsites), Figure \@ref(fig:wetSeasonDOC)). 

\  

```{r wetDOCinstallSites}
read_csv("R-outputs_UBC-forWater-MSc_HMc/tables/Ch3_wet-site-InstallSamples-DOC-summary.csv", col_names = TRUE) %>%
