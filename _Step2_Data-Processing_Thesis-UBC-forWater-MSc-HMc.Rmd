...
a <- sixfilter_physiographic %>%
  ...
  theme(legend.position = "top",
        text = element_text(size = 11)) +
  labs(y = expression(Monitoring~site~drainage~area~(km^2)), x = "DOC (mg/L)" )
...
b <- sixfilter_physiographic %>%
  ...
  theme(legend.position = "none",
        text = element_text(size = 11)) +
  labs(y = expression(Sub-basin~drainage~area~(km^2)), x = "DOC (mg/L)" )
...
area_DOC1 <- sixfilter_physiographic %>% 
  filter(site != "Tunnel") %>% 
  filter(E2E3 > 3.5) %>% 
...
area_DOC2 <- sixfilter_physiographic %>% 
  filter(site == "Tunnel") %>% 
  filter(E2E3 > 3.5) %>% 
  ggplot(aes(x = drainage_area_km2, y = NPOC_ppm)) +
  theme_bw() +
...
cowplot::plot_grid(area_DOC1, area_DOC2, nrow = 1, rel_widths = c(10,1))
