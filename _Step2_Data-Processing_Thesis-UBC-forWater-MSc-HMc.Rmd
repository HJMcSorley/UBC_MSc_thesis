...
# arrange as combo
cowplot::plot_grid(a,b,c,d, ncol = 1, align = "v", labels = "AUTO",
                   rel_heights = c(1.25,1,1,1))

# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch1_boxplot_allParams-allSites-allSamples.png", width = 6.5, height = 10, units = "in")


```

table: DOC by site Mean/Min/Max  
...
Boxplot: LWSA monitoring sites DOC 
```{r}
...
## return here - nested catchment DOC 
...
Weeks_grab$site <- fct_recode(Weeks_grab$site, 'Weeks (HW Grabs)' = "Weeks")

#2
ChrisCrk_grab <- synopticfilter %>% 
  select(site, trip, sample_type, sample, DateTime_sampled, sampleStage_cm, analysis, NPOC_ppm, SAC254_Abs.m, SUVA, pseudo_SUVA, E2E3, rain_season, HoldTime_days, QAQC_flag, site_type) %>% 
  filter(site == "ChrisCrk" & sample_type == "Grab")
ChrisCrk_grab$site <- fct_recode(ChrisCrk_grab$site, 'ChrisCrk (HW Grabs)' = "ChrisCrk")

#3
LeechHead_grab <- synopticfilter %>% 
  select(site, trip, sample_type, sample, DateTime_sampled, sampleStage_cm, analysis, NPOC_ppm, SAC254_Abs.m, SUVA, pseudo_SUVA, E2E3, rain_season, HoldTime_days, QAQC_flag, site_type) %>% 
  filter(site == "LeechHead" & sample_type == "Grab")
LeechHead_grab$site <- fct_recode(LeechHead_grab$site, 'LeechHead (US Grabs)' = "LeechHead")

#4
CraggCrk_grab <- synopticfilter %>% 
  select(site, trip, sample_type, sample, DateTime_sampled, sampleStage_cm, analysis, NPOC_ppm, SAC254_Abs.m, SUVA, pseudo_SUVA, E2E3, rain_season, HoldTime_days, QAQC_flag, site_type) %>% 
  filter(site == "CraggCrk" & sample_type == "Grab")
CraggCrk_grab$site <- fct_recode(CraggCrk_grab$site, 'CraggCrk (US Grabs)' = "CraggCrk")

#5
WestLeech_grab <- synopticfilter %>% 
  select(site, trip, sample_type, sample, DateTime_sampled, sampleStage_cm, analysis, NPOC_ppm, SAC254_Abs.m, SUVA, pseudo_SUVA, E2E3, rain_season, HoldTime_days, QAQC_flag, site_type) %>% 
  filter(site == "WestLeech" & sample_type == "Grab")
WestLeech_grab$site <- fct_recode(WestLeech_grab$site, 'WestLeech (US Grabs)' = "WestLeech")
...
pseudo_synopticfilter <- full_join(synopticfilter, pseudo) %>% 
  mutate(site = factor(site, levels = c('Weeks', 'ChrisCrk', 
                                        'Weeks (HW Grabs)', 
                                        'ChrisCrk (HW Grabs)', 
                                        'LeechHead (US Grabs)', 'LeechHead',
                                        'Jarvis-crk', 'Lazar-crk', 
                                        'CraggCrk', 'CraggCrk (US Grabs)', 
                                        'WestLeech', 'WestLeech (US Grabs)',
                                        'Leech-Beach', 'Tunnel', 
                                        'Rithet-crk', 'Judge-crk', 'Deception') ))

# rename
pseudo_synopticfilter$site <- fct_recode(pseudo_synopticfilter$site, 
                                         'LeechHead (DS Rack + Grabs)' = 'LeechHead')
...
a <- pseudo_synopticfilter %>% 
  filter(QAQC_flag == "OK") %>% 
  filter(site == "Weeks_grab" | site == "ChrisCrk_grab" | site == "LeechHead") %>% 
  ggplot(aes(x = site, y = NPOC_ppm, fill = site)) +
  geom_boxplot(alpha = 0.5, outlier.shape = 4) +
  #scale_fill_viridis(discrete = TRUE)+
  scale_fill_manual(values = c("Weeks_grab" = "#440154FF",
                               "ChrisCrk_grab" =  "#482677FF",
                               "LeechHead" = "#404788FF")) +
  geom_jitter(aes(shape = sample_type), alpha = 0.6) +
  scale_shape_manual(values = c("Grab" = 21, "Rack" = 24))+ #circle, triangle  
  theme_bw() +
...
b <- pseudo_synopticfilter %>% 
  ...
  geom_boxplot(alpha = 0.5, outlier.shape = 4) +
  #scale_fill_viridis(discrete = TRUE)+
  scale_fill_manual(values = c('Jarvis-crk' = "#1F968BFF",
                               'Lazar-crk' =  "#29AF7FFF",
                               "CraggCrk" = "#55C667FF")) +
  geom_jitter(aes(shape = sample_type), alpha = 0.6) +
  scale_shape_manual(values = c("Grab" = 21, "Rack" = 24))+ #circle, triangle  
...
c <- pseudo_synopticfilter %>% 
  filter(QAQC_flag == "OK") %>% 
  filter(site == 'LeechHead_grab' | site == 'CraggCrk_grab' | site == 'WestLeech_grab' | site == "Tunnel") %>% 
  ggplot(aes(x = site, y = NPOC_ppm, fill = site)) +
  geom_boxplot(alpha = 0.5, outlier.shape = 4) +
  #scale_fill_viridis(discrete = TRUE)+
  scale_fill_manual(values = c("LeechHead_grab" = "#404788FF", 
                               "CraggCrk_grab" = "#55C667FF",
                               "WestLeech_grab" = "#95D055FF",
                               "Tunnel" = "#FDE725FF")) +
  geom_jitter(aes(shape = sample_type), alpha = 0.6) +
  scale_shape_manual(values = c("Grab" = 21, "Rack" = 24))+ #circle, triangle  
  theme_bw() +
...
abc <- cowplot::plot_grid(ab, c, ncol = 1, rel_heights = c(1,1.5), 
...
box & ridge plots of G/R six-sites  
...
sixfilter %>%
  ...
  ggplot(aes(x = site, y = NPOC_ppm, fill = site)) +
  geom_boxplot(alpha = 0.4)+
  geom_jitter(alpha = 0.4, shape = 21) +
  ...
  theme(legend.position = "none",
        ...
        axis.text.x = element_text(angle = 30, hjust = 1)) 
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/DOC_seasonal-subbasin_boxplots_2.png", width = 5, height = 4, units = "in")
...
RF_plot_relimp_RSE <- randomForest::importance(RF_NPOC_surface, type = 2) %>% 
  as.data.frame() %>% 
  rownames_to_column() %>% 
  select(predictor = rowname,
