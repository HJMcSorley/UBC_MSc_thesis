...
DOCNPOC <- synopticfilter %>% 
  ...
  geom_point(aes(fill = rain_seasons, shape = rain_seasons), size = 2, alpha = 0.8)+
...
cowplot::ggdraw() +
  ...
  draw_plot(plot = DOCNPOC,
            ...
            scale = 1) # Inset scale (1 = none)
#save image
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch2_DOC-SAC254_seasonal_with-Inset.png",
  ...
  bind_rows(
    synopticfilter %>%
      ...
      summarize(sample_type = "all",
                ...
                DOCmax = max(NPOC_ppm, na.rm = T)) %>% 
      ungroup()) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/Ch2_wet-InstallSamples-DOC-summary.csv")
...
sixfilter %>%
  ...
  summarize(count = n(), 
            ...
            DOCmedian = median(NPOC_ppm, na.rm = T), 
            DOCmax = max(NPOC_ppm, na.rm = T)) %>%
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/Ch2_wet-site-InstallSamples-DOC-summary.csv")
...
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch3_RFplot_all_DOC.png", 
       width = 6, height = 6, units = "in")
...
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch3_RFplot_all_E2E3.png", 
       width = 6, height = 6, units = "in")
...
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch3_RFplot_all_SAC254.png", 
       width = 6, height = 6, units = "in")
...
###### appendix plots individual group predictors
...
dynamicVariables_DOC <- sixfilter_physiographic %>% 
  filter(!is.na(NPOC_ppm)) %>%
  select(c(NPOC_ppm, # predict SAC254
    sampleStage_normalized, 
    antecedent_30day_rain, # relatively more important than 7-day -- correlated with stage tho 
    antecedent_7day_temp # no real diff in VIM for 7 vs 30 day
    )) # %>%     
...
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/appendix_RFplot_dynamic_DOC.png", 
       width = 6, height = 6, units = "in")
...
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/appendix_RFplot_static_DOC.png", 
       width = 6, height = 6, units = "in")
...
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/appendix_RFplot_dynamic_E2E3.png", 
       width = 6, height = 6, units = "in")
...
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/appendix_RFplot_static_E2E3.png", 
       width = 6, height = 6, units = "in")
...
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/appendix_RFplot_dynamic_SAC254.png", 
       width = 6, height = 6, units = "in")
...
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/appendix_RFplot_static_SAC254.png", 
       width = 6, height = 6, units = "in")
...
# ----- all together
## DOC
cowplot::plot_grid(RFplot_dynamic_DOC + theme(legend.position = "none"), 
                   RFplot_static_DOC + theme(legend.position = "none"), 
                   ncol = 1)

cowplot::plot_grid(RFplot_dynamic_E2E3 + theme(legend.position = "none"), 
                   RFplot_static_E2E3 + theme(legend.position = "none"), 
                   ncol = 1)

cowplot::plot_grid(RFplot_dynamic_SAC254 + theme(legend.position = "none"), 
                   RFplot_static_SAC254 + theme(legend.position = "none"), 
                   ncol = 1)

```

## plots: predictors to predictants: one to one
...
c <- sixfilter_physiographic %>% 
  ggplot(aes(x = slope_median_degrees, y = NPOC_ppm)) + 
  theme_bw() +
  ...
  theme(legend.position = "top",
        text = element_text(size = 11)) +
  labs(x = "Slope, median (degrees)", y = "DOC (mg/L)")
# save
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch4_RF_E2E3-predictor-2-stage.png",
       width = 4, height = 4, units = "in")
...
stage_samples %>% 
  ...
  facet_wrap(~site, ncol = 1, scales = "free_y", strip.position = "right") +
  labs(y = "River Stage (cm)", x = "", caption = "Events 10 & 11")+
...
e10temp_DOC <- hyst_df %>%
  filter(event_ID == "10") %>% 
  filter(!is.na(DateTime_sampled)) %>% 
  ggplot(aes(x = antecedent_14day_temp, y = NPOC_ppm, colour = site)) + 
  ...
  theme(legend.position = "none", strip.background.x = element_blank(), strip.text = element_blank(), 
        text = element_text(size = 11)) # +
 # labs(x = "antecedent 3-day temp (°C)", y = "DOC (mg/L)") + ggtitle("A) Event 10")
...
e11temp_DOC <- hyst_df %>%
  ...
  #labs(x = "antecedent 3-day temp (°C)", y = "") + ggtitle("B) Event 11")
...
# side by side
#hystPlot_DOC_3temp <-
hystPlot_1011_SAC <- cowplot::plot_grid(e10temp_SAC254, e11temp_SAC254, ncol = 2)
...
# side by side
hystPlot_1011_E2E3 <-cowplot::plot_grid(e10E2E3, e11E2E3, ncol = 2)
...
# side by side
#hystPlot_DOC_3temp <-
hystPlot_1011_E2E3 <- cowplot::plot_grid(e10temp_E2E3, e11temp_E2E3, ncol = 2)
