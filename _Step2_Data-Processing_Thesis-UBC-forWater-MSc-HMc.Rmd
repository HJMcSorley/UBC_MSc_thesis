...
bind_rows(a, b) %>% 
  write_csv(path = "R-outputs_UBC-forWater-MSc_HMc/tables/Ch2_DOC-Synoptic-summary.csv", col_names = T)


# --- only grab samples
# synoptic DOC concs by site
a <- synopticfilter %>%
  filter(QAQC_flag == "OK") %>% 
...
# tag totals summary onto site summary
bind_rows(a, b) %>% 
  write_csv(path = "R-outputs_UBC-forWater-MSc_HMc/tables/Ch2_NOM-Synoptic-summary.csv", col_names = T)
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch2_DOC-boxplot_up-down_metcompar.png", width = 6, height = 6, units = "in")
...
insetSAC <- synopticfilter %>% 
  ...
  geom_jitter(aes(fill = rain_seasons, shape = rain_seasons), size = 2, alpha = 0.8) +
  scale_fill_manual(values = c("wet" = forWater_colours2[["SkyBlue"]], 
                               "dry" = forWater_colours2[["MyOrange"]], 
                               "first flush" = cbPalette[["grey"]]))+
  scale_shape_manual(values = c("wet"=23, "first flush"=22, "dry"=21))+
