...
mean_rainplot <- LWSA_meanWx %>% 
  ...
  annotate("text", label = "Project start", angle = 90, colour = brewer.pal(8, "Dark2")[8],
           x = as_date("2018-10-23")-20, y = 100)

# plot LWSA snow
mean_snowplot <- LWSA_meanWx %>% 
  mutate(date = lubridate::as_date(DateTime)) %>% 
  group_by(date) %>% 
  dplyr::summarise(daily_Snowmean = mean(SnowDepth_mean, na.rm = TRUE)) %>% 
  ungroup()  %>% 
  ggplot(aes(x = date, y = daily_Snowmean)) +
  geom_col(colour = "grey60") +
  labs(x = "", y = 'Snow (m/day)') +
...
# save plot
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Wx_LWSA-means.png", width = 6, height = 6, units = "in")
```
...
a <- precip_data %>%
  ...
  dplyr::rename("year" = Year,
                ...
                "max. snow depth, mean (m)" = max_snow_m,
                "mean air temp. (°C)" = annual_mean_temp, 
                "stdev air temp. (±°C)" = temp_sd,
                "min. temp., mean (°C)" = annual_min_temp2,
                "max. temp., mean (°C)" = annual_max_temp2) %>% 
...
b <- precip_data %>%
  ...
  dplyr::rename("year" = JanFebYr,
                "mean annual rain (mm)" = mean_rain_mm,
                "stdev rain. (±mm)" = sd_rain_mm,
                "max. snow depth, mean (m)" = mean_max_snow_m,
                "mean air temp. (°C)" = annual_mean_temp, 
                "stdev air temp. (±°C)" = temp_sd,
                "min. temp., mean (°C)" = annual_min_temp2,
                "max. temp., mean (°C)" = annual_max_temp2) 
...
cowplot::plot_grid(a, b, d, c, align = "v", ncol = 1, rel_heights = c(2, 1.6, 1.6, 1.6))  
# save
...
HT_tests_summary <- HT_Wilcoxon_tests %>% 
  ...
  select(Set, "Days held" = lapse, 
         ...
         "sig. diff (SAC254)" = sig.diff_SAC254_Abs.m, 
         "p-value (SAC254)" = p.value_SAC254_Abs.m,
         
         "SUVA254 change (%)" = percent_change_SUVA,
         "sig. diff (SUVA254)" = sig.diff_SUVA, 
         "p-value (SUVA254)" = p.value_SUVA,
         
         "E2:E3 change (%)" = percent_change_E2E3,
         "sig. diff (E2:E3)" = 'sig.diff_E2:E3', 
...
n_SynopticGrabs_UV_OK <- synopticfilter %>% 
  dplyr::filter(sample_type == "Grab", QAQC_flag == "OK", !is.na(pseudo_SUVA)) %>%
  #group_by(site = forcats::fct_explicit_na(site)) %>% # to see which sites are included
  summarise(grab_sample_count = n()) %>% 
  filter(grab_sample_count > 2) %>% pull()
# how many grab samples were grabbed (<2 per site)?
n_Grabs_UV_OK <- sampleresults %>% 
  dplyr::filter(sample_type == "Grab", analysis == "DOC", QAQC_flag == "OK", !is.na(pseudo_SUVA)) %>%
  summarise(grab_sample_count = n()) %>%  pull()
# how many of each sample_type are there at the 6 main sites?
...
n_totalSamples_UV_OK <- sampleresults %>% 
  dplyr::filter(sample_type == "Rack" | sample_type == "Grab",
                QAQC_flag == "OK", analysis == "DOC", !is.na(pseudo_SUVA)) %>%
  summarise(sample_count = n()) %>% pull()
...
# plot stage, rain events, samples (by sample type) all sites - no weather
stage_samples %>% 
  ggplot(aes(x = DateTime))+
  # start of rain events
  geom_vline(colour = forWater_colours1["MainBlue"], linetype = "twodash", 
             xintercept = c(event_starts), size = 0.5, show.legend = TRUE)+ 
  # river stage
...
# save 
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Wx-stage-subbasins_megaplot_sampletype.png",
...
# save megaplot
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Wx-stage_subbasins_season-megaplot2.png", 
...
synopticfilter %>% 
  ...
  labs(#caption = glue::glue("DOC concentrations in grab samples across 13 sites over 16 months (", {n_SynopticGrabs}, " samples)"), 
    x = "", y = "DOC (mg/L)")
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/DOC_syn_13sites_boxplots.png", width = 6.5, height = 4, units = "in")
```
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch1_DOC-boxplot_allSites-allSamples.png", width = 6.5, height = 4, units = "in")
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch1_boxplot_allParams-allSites-allSamples.png", width = 5, height = 8, units = "in")
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch1_ridgeplot_allParams-allSites-allSamples.png")
```
...
bind_rows(a, b) %>% 
  write_csv(path = "R-outputs_UBC-forWater-MSc_HMc/tables/DOC-Synoptic-grab_summary.csv", col_names = T)


# --- same but not JUST Grabs, all samples
# synoptic grab samples DOC concs by site
a <- synopticfilter %>%
  filter(QAQC_flag == "OK") %>% 
...
# tag totals summary onto site summary
bind_rows(a, b) %>% 
  write_csv(path = "R-outputs_UBC-forWater-MSc_HMc/tables/CH1_DOC-Synoptic-summary.csv", col_names = T)
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch1_DOC-boxplot_up-down_metcompar.png", width = 6, height = 6, units = "in")
...
synopticfilter %>%
  ...
  scale_x_datetime(date_labels = "%Y %b %d", date_breaks = "2 months", date_minor_breaks = "1 months") 
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/CH2_DOC-loess-trend.png", width = 4, height = 4, units = "in")
...
d <- synopticfilter %>%
  ...
  scale_x_datetime(date_labels = "%Y %b %d", date_breaks = "2 months", date_minor_breaks = "1 months")
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/CH2_E2E3-loess-trend.png", width = 4, height = 4, units = "in")
...
DOCNPOC <- synopticfilter %>% 
  ...
  theme(text = element_text(size = 11),
        legend.position = "top")
#save image
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/CH2_DOC-CDOM_seasonal.png",
       height = 5, width = 5, units = "in")

# ---
# SAC254 vs NPOC by season
insetSAC <- synopticfilter %>% 
  filter(QAQC_flag == "OK") %>% 
  dplyr::filter(analysis == "DOC", 
                sample_type == "Grab" | sample_type == "Rack") %>% 
  ggplot(aes(x = NPOC_ppm, y = SAC254_Abs.m)) +
  geom_jitter(aes(fill = rain_seasons, shape = rain_seasons), size = 1, alpha = 0.8) +
  scale_fill_manual(values = c("wet" = forWater_colours2[["SkyBlue"]], 
                               "dry" = forWater_colours2[["MyOrange"]], 
                               "first flush" = cbPalette[["grey"]]))+
  scale_shape_manual(values = c("wet"=23, "first flush"=22, "dry"=21))+
  ...
  theme(text = element_text(size = 10),
        legend.position = "none")
#save image
#ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch2_SAC254-DOC_seasonal_1-1.png", height = 3.5, width = 5, units = "in") 


# inset the SAC254 plot into the NPOC DOCest plot
cowplot::ggdraw() +
  draw_plot(DOCNPOC) +
  draw_plot(plot = insetSAC,
...
#save image
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/CH2_DOC-CDOM_seasonal_with-Inset.png",
       height = 5, width = 5, units = "in")
  ...
  bind_rows(
    synopticfilter %>%
      ...
      summarize(rain_season = "all",
                ...
                DOCmax = max(NPOC_ppm, na.rm = T)) %>% 
      ungroup()) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/CH2_seasonal-samples-DOC-summary.csv")

# in only the LWSA monitoring sites...
# how many samples were collected seasonally?
sixfilter %>%
  filter(QAQC_flag == "OK") %>% 
  filter(sample_type == "Grab" | sample_type == "Rack") %>% 
  ...
  bind_rows(
    synopticfilter %>%
      ...
      summarize(sample_type = "all",
                ...
                DOCmax = max(NPOC_ppm, na.rm = T)) %>% 
      ungroup()) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/CH2_seasonal-InstallSamples-DOC-summary.csv")
...
label <- c(wet = "Wet season", dry = "Dry season")

synopticfilter %>%
  filter(QAQC_flag == "OK") %>% 
  filter(sample_type == "Grab" | sample_type == "Rack") %>% 
  ggplot(aes(y = NPOC_ppm, x = site, fill = site)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_viridis(discrete = TRUE) +
  theme_bw()+
  theme(legend.position = "none",
...
sixfilter %>%
  ...
  labs(x = "", y = "DOC (mg/L)")
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/CH2_subbasin_GvsR_WETseason-boxplot.png",
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/CH2_subbasin_GvsR_WETseason-ridgeplot.png",
...
## DOC Boxplot: LWSA monitoring sites DOC 
```{r}
...
sixfilter %>%
  ...
  theme(legend.position = "top",
        ...
        axis.text.x = element_text(angle = 30, hjust = 1)) 
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/CH3_DOC-seasonal-subbasin_boxplots.png", width = 5, height = 4, units = "in")

```

seasonal sample count and DOC summary
```{r}
# how many samples were collected seasonally?
sixfilter %>%
  filter(QAQC_flag == "OK") %>% 
...
add sample counts to events summary table
```{r}
...
eventsample_table <- right_join(events_table,
  ...
  summarise("Samples collected" = n(),
            ...
            "mean SAC254" = mean(SAC254_Abs.m, na.rm = T),
  ...
  # rename things
  select(-"Storm number") %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/CH3_events-n-samples.csv")
...
# group
cowplot::plot_grid(d,u,s,e, nrow = 1, rel_widths = c(1,1,1,1.075))
# save
...
stage_response %>% 
  ...
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/response_stage-summary.csv")
...
# get daily mean rain
antecedent_7day_Wx <- LWSA_meanWx %>% 
  mutate(Date = as_date(DateTime)) %>% 
  group_by(Date) %>% 
  ...
  mutate(antecedent_7day_rain = as.double(zoo::rollsum(daily_rain, k = 7, align = "right", fill = NA)),
...
sixfilter_RF <- sixfilter %>%
  ...
  left_join(., antecedent_7day_Wx, by = c("Date_sampled" = "Date")) %>% ungroup() %>%  
...
sixfilter_physiographic <- left_join(sixfilter_RF, physiographic, by = c("site")) %>%
  ...
  select(c(site, DateTime_sampled, HoldTime_days, NPOC_ppm, rain_season, sampleStage_normalized, antecedent_7day_rain, antecedent_7day_temp, latitude, longitude, strahler_order, drainage_area_km2, elevation_masl, forest_percent, wetland_percent, openWater_percent, geo_WarkGneiss_percent, geo_metamorphic_percent, geo_ArgilliteMetagreywacke_percent, geo_Metagreywacke_percent, geo_metasedimentary_percent, geo_ChertArgilliteVolcanic_percent, geo_metaSedimentaryVolcanic_percent, geo_MetchosinVolcanics_percent, geo_SookeGabbro_percent, geo_igneous_percent, slope_med_degrees, slope_max_degrees, logging_percent_FROMnineteeneightyTOtwentyeleven = "logging_percent_nineteeneighty-twentyeleven")) 
# remove features that seems to disrupt the algorithm by overshadowing 

# remove unnecessary variables for RF
RF_sixPhysiog <- sixfilter_physiographic %>% 
  select(-c(site, DateTime_sampled, rain_season)) %>% 
  # add a column of random values as as QA -- this should not be found to be important
  mutate(QA_randomNumbers = sample(200, size = nrow(.), replace = TRUE)) #,
         #QA_randomNumbers_doubled = QA_randomNumbers*2) # add a correlated variable to check

# check for correlations
# view(cor(RF_sixPhysiog))


## filter for cross-correlation which biases the importance ranking
# subset for subsurface attributes (geological / parent material)
RF_sixPhysiog_subsurface <- RF_sixPhysiog %>% 
  select(c(NPOC_ppm, antecedent_7day_rain, geo_metamorphic_percent, geo_metasedimentary_percent, geo_metaSedimentaryVolcanic_percent, geo_igneous_percent, latitude, longitude, QA_randomNumbers))
# check for correlations
# view(cor(RF_sixPhysiog_subsurface))
 
 
# subset for surface attributes (land-cover attributes)
RF_sixPhysiog_select <- RF_sixPhysiog %>% 
  select(-c(HoldTime_days,
           antecedent_7day_rain, # strongly correlated to stage 
            strahler_order, geo_WarkGneiss_percent, geo_ArgilliteMetagreywacke_percent, geo_ChertArgilliteVolcanic_percent, geo_Metagreywacke_percent, geo_MetchosinVolcanics_percent, geo_SookeGabbro_percent,  latitude, longitude))
# check for correlations
# view(cor(RF_sixPhysiog_surface))


# ---
## Run RF

# fNPOC_ppm is dependent variable
...
RF_NPOC <- randomForest::randomForest(NPOC_ppm ~ ., 
                                      data = RF_sixPhysiog_select, 
...
# plot relative importance of MSE_percent
RF_plot_relimp_MSE <- randomForest::importance(RF_NPOC, type = 1) %>% 
  ...
  mutate(predictor = fct_recode(predictor,
                                ...
                                "metamorphic parent material" = "geo_metamorphic_percent",
                                "metasedimentary parent material" = "geo_metasedimentary_percent",
                                "metasedimentary-igneous parent material" = "geo_metaSedimentaryVolcanic_percent",
                                "igneous parent material"  = "geo_igneous_percent",
                                "median basin slope" = "slope_med_degrees",
                                "maximum basin slope" = "slope_max_degrees",
                                "logging history (1980-2011)" = "logging_percent_FROMnineteeneightyTOtwentyeleven", 
                                "QA-QC (random numbers)" = "QA_randomNumbers" ),
  ...
  geom_bar(stat = "identity", position = "dodge", color = "black", 
           fill= forWater_colours2["DeepBlue"]) +
  theme_bw() +
  labs(y = "variable importance (%)", x = "", size = 11, 
       caption = "Variable importance for predicting DOC, measured by type 1 (mean square errors, \n ie., mean decrease in accuracy when variable was omitted).")
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/RF_NPOC-type1-relimp_MSE.png", 
       width = 5, height = 4, units = "in")

## relative importance of features in predicting DOC
# type 2 = mean decrease in node impurity
# plot relative importance of RSS_percent -- more sensitive to correlated values
RF_plot_relimp_RSE <- randomForest::importance(RF_NPOC, type = 2) %>% 
  as.data.frame() %>% 
  rownames_to_column() %>% 
  select(predictor = rowname,
  ...
  coord_flip()+
  geom_bar(stat = "identity", aes(), position="dodge", color = "black", 
           fill = forWater_colours2["DeepBlue"]) +
  theme_bw() +
  labs(y = "variable importance (%)", x = "", size = 11, 
       caption = 
         "Variable importance for predicting DOC, measured by type 2 (residual sum of squares, \n ie., mean decrease in node impurity when variable was omitted).")
# save this one too 
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/RF_NPOC-type2-relimp_RSS.png",
       width = 5, height = 4, units = "in")


# randomForestExplainer to describe results of RF model
# create a descriptive html report 
# this is comutationally expensive -- hash out when done
# RF_NPOC_explained <- randomForestExplainer::explain_forest(RF_NPOC, interactions = TRUE, data = RF_sixPhysiog)
# report saved in root directory

```


### random forests with only wet season data
```{r}
# remove unnecessary variables for RF
RF_sixPhysiog_wet <- sixfilter_physiographic %>% 
  filter(rain_season == "wet") %>% 
  select(-c(site, DateTime_sampled, rain_season)) %>% 
  # add a column of random values as as QA -- this should not be found to be important
  mutate(QA_randomNumbers = sample(200, size = nrow(.), replace = TRUE)) %>% 
...
# mtry = (length(sixfilter_physiographic)-1)/3 = number of features/3 ~= 10 (or = num.features)
# mtry : at each node, mtry variables are selected at random and are searched through for best split then the largest tree possible is grown (unpruned)
RF_NPOC_wet <- randomForest::randomForest(NPOC_ppm ~ ., 
                                      data = RF_sixPhysiog_wet, 
                                      na.action = randomForest::na.roughfix,
                                      mtry = ceiling(length(data)/2), # no. of features
                                      ntree = 5000,  # ntree default = 500 (but Breiman says "Don't be stingy")
                                      importance = TRUE,
...
# type 1 = mean decrease in accuracy
# plot relative importance of MSE_percent
RF_plot_relimp_MSE <- randomForest::importance(RF_NPOC_wet, type = 1) %>% 
  as.data.frame() %>%  # rather than matrix
  rownames_to_column() %>% 
  select(predictor = rowname,
         MSE = "%IncMSE") %>% # percent increase in mean square error
  ...
  labs(y = "variable importance (%)", x = "", size = 11, 
       caption = 
         "Variable importance for predicting DOC during the wet season (only), measured by type 1 \n (mean square errors, ie., mean decrease in accuracy when variable was omitted).")
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/RF_NPOC_type1-wet-relimp_MSE.png", 
       width = 5, height = 4, units = "in")

# type 2 = mean decrease in node impurity
# plot relative importance of RSS_percent -- more sensitive to correlated values
RF_plot_relimp_RSE <- randomForest::importance(RF_NPOC_wet, type = 2) %>% 
  as.data.frame() %>% 
  rownames_to_column() %>% 
  select(predictor = rowname,
         RSS = IncNodePurity) %>% # residual sum of squares
  ...
  labs(y = "variable importance (%)", x = "", size = 11, 
       caption = 
         "Variable importance for predicting DOC during wet season (only), measured by type 2 \n (residual sum of squares, ie., mean decrease in node impurity when variable was omitted).")
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/RF_NPOC_type2-wet-relimp_RSS.png", 
       width = 5, height = 4, units = "in")

```

### random forests with only dry season data
```{r}
# remove unnecessary variables for RF
RF_sixPhysiog_dry <- sixfilter_physiographic %>% 
...
# mtry = (length(sixfilter_physiographic)-1)/3 = number of features/3 ~= 10 (or = num.features)
# mtry : at each node, mtry variables are selected at random and are searched through for best split then the largest tree possible is grown (unpruned)
RF_NPOC_dry <- randomForest::randomForest(NPOC_ppm ~ ., 
                                      data = RF_sixPhysiog_dry, 
                                      na.action = randomForest::na.roughfix,
                                      mtry = ceiling(length(data)/2), # no. of features
                                      ntree = 5000,  # ntree default = 500 (but Breiman says "Don't be stingy")
                                      importance = TRUE,
...
# type 1 = mean decrease in accuracy
# plot relative importance of MSE_percent
RF_plot_relimp_MSE <- randomForest::importance(RF_NPOC_dry, type = 1) %>% 
  as.data.frame() %>%  # rather than matrix
  rownames_to_column() %>% 
  select(predictor = rowname,
         MSE = "%IncMSE") %>% # percent increase in mean square error
  ...
  labs(y = "variable importance (%)", x = "", size = 11, 
       caption = 
         "Variable importance for predicting DOC during the dry season (only), measured by type 1 \n (mean square errors, ie., mean decrease in accuracy when variable was omitted).")
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/RF_NPOC_type1-dry-relimp_MSE.png", 
       width = 5, height = 4, units = "in")

# type 2 = mean decrease in node impurity
# plot relative importance of RSS_percent -- more sensitive to correlated values
RF_plot_relimp_RSE <- randomForest::importance(RF_NPOC_dry, type = 2) %>% 
  as.data.frame() %>% 
  rownames_to_column() %>% 
  select(predictor = rowname,
         RSS = IncNodePurity) %>% # residual sum of squares
  ...
  labs(y = "variable importance (%)", x = "", size = 11, 
       caption = 
         "Variable importance for predicting DOC during dry season (only), measured by type 2 \n (residual sum of squares, ie., mean decrease in node impurity when variable was omitted).")
# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/RF_NPOC_type2-dry-relimp_RSS.png", 
       width = 5, height = 4, units = "in")
