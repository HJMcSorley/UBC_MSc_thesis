...
mean_rainplot <- LWSA_meanWx %>% 
  ...
  ungroup()  %>% 
  ggplot(aes(x = date, y = daily_rn)) +
  geom_col(colour = forWater_colours2["DeepBlue"]) +
  scale_y_reverse() +
  ...
  gghighlight::gghighlight(date >= "2018-10-23",
                           unhighlighted_params = list(colour = forWater_colours2["Gray"])) +
...
mean_snowplot <- LWSA_meanWx %>% 
  ...
  ungroup()  %>% 
  filter(daily_SnowMax > 0) %>% 
  ggplot(aes(x = date, y = daily_SnowMax)) +
  #geom_col(colour = forWater_colours1["MainBlue"]) + # Bill said make it a line plot
  geom_line(colour = forWater_colours1["MainBlue"]) + 
  labs(x = "", y = 'Cumulative Snow (m)') +
  theme_bw() +
  theme(text = element_text(size = 11)) +
  theme(legend.position = "none")+
  scale_x_date(date_breaks = "2 months", date_minor_breaks = "1 months", labels = NULL)+ 
...
## RAIN EVENTS
```{r}
# Bill suggested determining what percent of total rainfall was in each Event
rain_proj_total <- LWSA_meanWx_dat %>% 
  filter(dplyr::between(DateTime, project_start, project_end) ) %>% 
  mutate(year = lubridate::year(DateTime)) %>% 
  #group_by(year) %>% 
  summarise(project_total_rain = sum(Rn15_mean, na.rm = TRUE)) %>% 
  pull()

# get Event variables right
majEvents_summary <- events %>%
  mutate(duration = difftime(EndDate, StartDate, units = "hours"),
         ...
         percent_total_rain = (rain/rain_proj_total)*100 ) %>% 
  mutate(ID = as_factor(ID))

# make a table
events_table <- majEvents_summary %>% 
  mutate(duration_days = round(as.numeric(duration_days), 1),
         percent_total_rain = paste(round(percent_total_rain, 1), '%')) %>% 
  select('Storm number' = stormnum,
         ...
         'Rainfall (mm)' = rain,
         'Percent of total rain for the study period' = percent_total_rain
         ) %>%
...
# compare air and water temps (linear relations?)
Hobo %>%
  #filter(rain_season == "dry") %>% 
  #filter(rain_season == "wet") %>%
  select(DateTime, TidbiT_location, site, Date, rain_season, Temp_C_fullwet) %>% 
  pivot_wider(names_from = TidbiT_location,
              values_from = Temp_C_fullwet) %>% 
  ggplot(aes(x = air, y = water))+
  geom_point(alpha = 0.5)+
  scale_color_brewer(palette = "Dark2")+ 
  theme_bw()+
  facet_wrap(~site, ncol = 2, scales = "free", strip.position = "top") +
  labs(x = "Air Temperature (°C)", y = "Water Temperature (°C)")+
  theme(text = element_text(size = 12))

# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/TidbiTs_air-water.png", 
...
# already defined: 'project_start' and 'project_end'
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/HoldTime_airTemp_megaplot.png", 
...
subasin_snow_plot <- LWSA_meanWx %>% 
  ...
  #geom_col(aes(colour = Snow), colour = "grey40") + + # Bill said make it a line plot
  geom_line(aes(colour = Snow), colour = forWater_colours1["DarkGrey"]) + 
  labs(x = "", y = "m") +
  theme_bw() +
...
subasin_meantemp_plot <- LWSA_meanWx %>% 
  ...
  filter(date >= "2018-10-24", date <= "2020-02-19") %>% 
  ggplot(aes(x = date, y = daily_Tmean)) +
  geom_line(aes(colour = Temp), colour = forWater_colours1["MyOrange"]) +
  geom_hline(yintercept = 0,
             linetype = "dotted") +
  labs(x = "", y = "°C /day") +
  theme_bw() +
...
subbasin_meanrain_plot <- LWSA_meanWx %>% 
  ...
  filter(date >= "2018-10-24", date <= "2020-02-19") %>% 
  ggplot(aes(x = date, y = daily_rn)) +
  geom_col(aes(colour = Rain), colour = forWater_colours2["DeepBlue"]) +
  scale_y_reverse() +
  labs(x = "", y = "mm /day") +
  theme_bw() + 
  scale_x_date(date_breaks = "2 months",
...
season_meanrain_plot <- subbasin_meanrain_plot +
  ...
  #geom_vline(colour = forWater_colours1["MyOrange"], linetype = "solid",  # final draft edits, remove first flush
...
# create a second megaplot with sample points included
# stack snow, temp, precip and stage with cowplot::plot_grid
cowplot::plot_grid(season_meantemp_plot, season_meanrain_plot, season_snow_plot, season_stage_sample_plot, ncol = 1, axis = "l", align = "v", rel_heights = c(1,1,1,7.2)) 

# save megaplot
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Wx-stage_subbasins_season-megaplot2.png", width = 6.5, height = 7.25, units = "in")
```
...
### limited Tunnel-Rithet-Judge

also, for comparing Tunnel to Rithet and Judge, limit the comarison to only overlapping periods
```{r}
Judge_dates <- synopticfilter %>%
  filter(site == "Judge-crk") %>% 
  select(Date_collected)

# 
synopticfilter %>%
  filter(site == "Judge-crk" | site == "Tunnel" | site == "Rithet-crk") %>% 
  filter(Date_collected == Judge_dates[[1,1]] | Date_collected == Judge_dates[[2,1]] | Date_collected == Judge_dates[[3,1]] | Date_collected == Judge_dates[[4,1]] | Date_collected == Judge_dates[[5,1]] | Date_collected == Judge_dates[[6,1]]) %>% 
  filter(QAQC_flag == "OK") %>% 
  filter(sample_type == "Grab" & sample == "Grab" | sample == "Treatability") %>%
  group_by(site, description) %>% 
  summarize(count = n(), 
            DOCmean = mean(NPOC_ppm, na.rm = T), 
            DOCsd = sd(NPOC_ppm, na.rm = T),
            ...
            DOCmedian = median(NPOC_ppm, na.rm = T), 
            DOCmax = max(NPOC_ppm, na.rm = T)) %>% 
write_csv(path = "R-outputs_UBC-forWater-MSc_HMc/tables/Ch3_DOC-Synoptic-summary_Tunnel-Rithet-Judge.csv", col_names = T)
...
```{r}
# daily mean temp and DOC
wut <- LWSA_meanWx %>% 
  mutate(Date = as_date(DateTime)) %>% 
  right_join(synopticfilter, by = c('Date', 'rain_season') ) %>% 
  group_by(Date, site, rain_season) %>% 
  summarise(mean_T = mean(Temp_mean, na.rm = TRUE),
            mean_DOC = mean(NPOC_ppm, na.rm = TRUE)) %>%
  ungroup() 

# monthly means
wutwut <- wut %>%
  mutate(month = lubridate::month(Date)) %>% 
  group_by(month, site, rain_season) %>% 
  summarise(mean_T = mean(mean_T, na.rm = TRUE),
            mean_DOC = mean(mean_DOC, na.rm = TRUE)) %>%
  ungroup() 


# plot DOC and mean temp
# wrap by season
...
wutwut %>% 
  #filter(rain_season == "wet") %>% 
  ggplot(aes(x = mean_T, y = mean_DOC))+ #, colour = site
  geom_point()+
  theme_bw()+
  #facet_wrap(~site, nrow = 2, scales = "free_y")+
  theme(legend.position = "none")+
  geom_smooth(method = "lm", colour = forWater_colours1["AccentBlue"])+
  stat_poly_eq(formula = y ~ x, 
              aes(#label = ..rr.label..), 
              label = paste0("atop(",..eq.label.., ",", ..rr.label.., ")")),
              parse = TRUE, rr.digits = 4) +
  labs(x = "monthly mean air temperature (°C)", y = "monthly mean DOC (mg/L) by site" )+
  facet_wrap(~rain_season, scales = "free_x",
             labeller = labeller(rain_season = label))

# save plot 
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch3_DOCvsTemp.png", width = 5.5, height = 3.5, units = "in")
...
#### [1:1] scatter 
```{r}
# how many samples were collected by season for which both NPOC and SAC254 data were available and OK?
synopticfilter %>%
  filter(sample_type == "Grab" | sample_type == "Rack",
...
char_wet <- sixfilter %>%
  ...
  bind_rows(
    sixfilter %>%
      ...
      ungroup()  ) %>% 
  mutate(mean_RSD_SAC = paste0(round(SACmean, 2), " ± ", round(SACRSD, 0), "%")) %>% 
  select(site, sample_type, char_count, mean_RSD_SAC, SACmin, SACmedian, SACmax) %>% 
  dplyr::arrange(site)
...
label1 <- c(Grab = "Wet season Grab samples", 
            Rack = "Wet season Rack samples")
label2 <- c(Grab = "Wet season Grab samples", 
            Rack = "Wet season Rack samples")
...
a <- sixfilter %>%
  ...
  labs(y = "", x = "DOC (mg/L)", subtitle = "A) NOM Quantity")
...
b <- sixfilter %>%
  ...
  labs(y = "", x = expression(SAC[254]~(m^-1)), subtitle = "B) NOM Quality" )
...
eventsample_table <- right_join(events_table,
  ...
  # rename things
  select(-c("Storm number", mean_pseudo254, "Duration (days)", "Percent of total rain for the study period")) %>%
  # rearrange
  select("Major event no.", "Start Date", "Rainfall (mm)", "Intensity (mm/24-hr)", "Samples collected", "mean DOC (mg/L)", "max_DOC", "mean_SAC254", "mean E2E3" ) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/CH4_events-n-samples.csv")


# do it again and see if max DOC was related to the ranking of storm intensity??
right_join( (events_table %>% 
               rename(intensity = "Intensity (mm/24-hr)",
                      event = 'Major event no.') %>% 
               arrange(intensity) %>% 
               mutate(intensity_rank = row_number(desc(intensity)),
            ...
            ( sixfilter %>% 
                ...
                summarise(sample_n = n(),
                          mean_DOC = mean(NPOC_ppm, na.rm = T),
                          max_DOC = max(NPOC_ppm, na.rm = T),
                          mean_SAC254 = mean(SAC254_Abs.m, na.rm = T),
                          mean_E2E3 = mean(E2E3, na.rm = T), 
                          mean_pseudo254 = mean(pseudo254, na.rm = T)
                          #mean_pseudoSUVA = mean(pseudo_SUVA, na.rm = T)
                ) %>%  
                mutate(mean_pseudo254 = as.character(mean_pseudo254)) %>% 
                ...
                mutate(mean_SAC254 = as.numeric(mean_SAC254)) %>%
                arrange(max_DOC) %>% 
               mutate(max_DOC_rank = row_number(desc(max_DOC))) %>% 
                arrange(event_ID)) ,
            # attach to event table
            by = c("event" = "event_ID" )) %>% 
  # drop things
  select(-c("Storm number", "Duration (days)", "Percent of total rain for the study period", mean_pseudo254)) %>% 
  arrange(max_DOC_rank)
  
# peak DOC occurred in low intensity storms early in the wet season
# least concentrated DOC occurred in the winter cold months with high intensity storms
...
# extract legend to add to top of combined variable plot
...
c <- sixfilter %>%
  ...
  guides(fill = FALSE,
         shape = guide_legend(override.aes = c("Grab" = 21, "Rack" = 24), size = 1.5))+
  labs(x = "", y = expression(E[2]:E[3]), 
       subtitle = "C")


# plot grid -- add legend to top plot
A <- cowplot::plot_grid(legend,
                        a + theme(legend.position = "none"),
                        ncol = 1, rel_heights = c(1,8))

# stack the three analyte seasonal plots
cowplot::plot_grid(A, B, c, ncol = 1, axis = "tb", 
                   rel_widths = c(1,1,1),
                   rel_heights = c(1.15, 1, 1.25))
# save
...
antecedent_Wx <- LWSA_meanWx %>% 
  ...
  summarise(daily_rain = sum(Rn15_mean, na.rm = TRUE),
            daily_mean_T = mean(Temp_mean, na.rm = TRUE)) %>%  
  ungroup() %>% 
  arrange(Date) %>% 
...
# write a table summarizing VIM
RF_DOC_allChrc_RelImp1_MSE %>% 
  mutate(MSE_percent = round(MSE_percent, 2)) %>% 
  select(predictor, variable_group, MSE_percent) %>% 
  arrange(-MSE_percent) %>% 
  rename("Predictor" = predictor,
         "Variable Group" = variable_group, 
         "Relative Importance (%)" = MSE_percent ) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/Appendix-Ch4_RF_DOC.csv")
...
# save legend for individual plots
RF_group_legend <- ggpubr::get_legend(RFplot_all_DOC)
...
RF_E2E3_allChrc_RelImp1_MSE %>% 
  ...
  rename("Predictor" = predictor,
         "Variable Group" = variable_group, 
         "Relative Importance (%)" = MSE_percent ) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/Appendix-Ch4_RF_E2E3.csv")
...
RF_SAC254_allChrc_RelImp1_MSE %>% 
  ...
  rename("Predictor" = predictor,
         "Variable Group" = variable_group, 
         "Relative Importance (%)" = MSE_percent ) %>% 
  write_csv("R-outputs_UBC-forWater-MSc_HMc/tables/Appendix-Ch4_RF_SAC254.csv")
...
staticVariables_DOC <- sixfilter_physiographic %>% 
  filter(!is.na(NPOC_ppm)) %>%
  select(c(NPOC_ppm, # predict DOC
           #geo_LeechRiverFormation_percent,
           #wetland_percent,
           #COLL_O.HFP_percent,
           geo_metamorphic_percent,
           geo_igneous_percent,
...
RFsep_DOC <- cowplot::plot_grid(RFplot_dynamic_DOC, RFplot_static_DOC, 
                                ncol= 1, align = "v",
                                rel_heights = c(1, 1.6))

# add legend (RF_group_legend)
D <- cowplot::plot_grid(RF_group_legend, RFsep_DOC, ncol = 1, rel_heights = c(1, 4.5))
...
# DOC dynamic and static adjacent to eachother
RFsep_E2E3 <- cowplot::plot_grid(RFplot_dynamic_E2E3, RFplot_static_E2E3, 
                                 ncol= 1, align = "v",
                                 rel_heights = c(1, 1.6))

# add legend (RF_group_legend)
E <- cowplot::plot_grid(RF_group_legend, RFsep_E2E3, ncol = 1, rel_heights = c(1, 5))

# save it
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch4_RFplot_sep-E2E3.png", width = 6, height = 6, units = "in")
...
# check for correlations
# view(cor(staticVariables_SAC254))
...
# DOC dynamic and static adjacent to eachother
RFsep_SAC254 <- cowplot::plot_grid(RFplot_dynamic_SAC254, RFplot_static_SAC254, 
                                   ncol= 1, align = "v",
                                   rel_heights = c(1, 1.6))
# add legend
S <- cowplot::plot_grid(RF_group_legend, RFsep_SAC254, ncol = 1, rel_heights = c(1, 5))

# save it
 ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch4_RFplot_sep-SAC254.png", width = 6, height = 6, units = "in")
...
# DOC-temp seasonal for appendix
temp_DOC_A <- sixfilter_physiographic %>% 
  mutate(rain_season = factor(rain_season, levels = c("wet", "dry"))) %>% 
  ggplot(aes(x = antecedent_7day_temp, y = NPOC_ppm)) + 
  theme_bw() +
  geom_jitter(aes(fill = site), shape = 21, size = 2) +
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Appendix-Ch4_RF_DOCtemp.png",
       width = 4, height = 4, units = "in")
...
rain_DOC <- sixfilter_physiographic %>% 
  ...
  theme(legend.position = "top",
        text = element_text(size = 11)) +
  facet_wrap(~site, scales = "free_y", strip.position = "right") +
  labs(x = "", y = "DOC (mg/L)" )
...
cowplot::plot_grid(rain_DOC, rain_SAC254, rain_E2E3,
                   ncol = 1, align = "v", 
                   rel_heights = c(2, 1.3, 1.3))

# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch4_RF_rain-bysite.png",
       width = 6, height = 7, units = "in")
...
all sites together
```{r}
train_DOC <- sixfilter_physiographic %>% 
  ggplot(aes(x = antecedent_30day_rain, y = NPOC_ppm)) + 
  theme_bw() +
  geom_point(aes(fill = site), shape = 21, size = 2) +
  geom_smooth(colour = "black", size = 0.7) +
...
train_SAC254 <- sixfilter_physiographic %>% 
  ggplot(aes(x = antecedent_30day_rain, y = SAC254_Abs.m)) + 
  theme_bw() +
  geom_jitter(aes(fill = site), shape = 21, size = 2) +
  geom_smooth(colour = "black", size = 0.7) +
...
train_E2E3 <- sixfilter_physiographic %>% 
  filter(E2E3 > 2.5) %>% 
  ggplot(aes(x = antecedent_30day_rain, y = E2E3)) + 
  theme_bw() +
  geom_jitter(aes(fill = site), shape = 21, size = 2) +
...
# together
cowplot::plot_grid(train_DOC, train_SAC254, train_E2E3,
                   ncol = 1, align = "v", 
                   rel_heights = c(2, 1.3, 1.3))
...
#### seasonal metamorphic
```{r}
# split by seasons because there were different patterns in NOM by season and the condition-based processes differ between wet and dry seasons

metamorphicS_DOC <- sixfilter_physiographic %>% 
  ggplot(aes(x = geo_metamorphic_percent, y = NPOC_ppm)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
...
metamorphicS_SAC254 <- sixfilter_physiographic %>% 
  ggplot(aes(x = geo_metamorphic_percent, y = SAC254_Abs.m)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
...
metamorphicS_E2E3 <- sixfilter_physiographic %>% 
  ggplot(aes(x = geo_metamorphic_percent, y = E2E3)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
...
# together
cowplot::plot_grid(metamorphicS_DOC, metamorphicS_SAC254, metamorphicS_E2E3,
                   ncol = 1, align = "v", rel_heights = c(2, 1.3, 1.3))

# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch4_RF_metamorphic-seasonal.png",
       width = 4, height = 6, units = "in")
...
#### Leech Rv Formation
```{r}
LRF_DOC <- sixfilter_physiographic %>% 
...
LRF_SAC254 <- sixfilter_physiographic %>% 
  ggplot(aes(x = geo_LeechRiverFormation_percent, y = SAC254_Abs.m)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
...
LRF_E2E3 <- sixfilter_physiographic %>% 
  ggplot(aes(x = geo_LeechRiverFormation_percent, y = E2E3)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
  theme(legend.position = "none",
        text = element_text(size = 11)) +
  labs(x = "Leech River Formation, metasedimentary parent material (%)", 
       y = expression(E[2]:E[3]) )

# together
cowplot::plot_grid(LRF_DOC, LRF_SAC254, LRF_E2E3,
                   ncol = 1, align = "v", rel_heights = c(2, 1.3, 1.3))

# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch4_RF_LRF-parentmaterial.png",
       width = 4, height = 6, units = "in")
...
"COLL_O.HFP_percent"   * similar to metamorphic (wark gneiss) pattern -- present in all sites at ~50% (same amount in Chris and Cragg)              
"TILL_DU.HFP_percent"                
"TILL_O.HFP_percent"
"FLUV_DU.HFP_percent"  * fairly linear -- present in weeks (& therefore leech-head and tunnel basins also)
"GLFL_DU.HFP_percent"                
"UNDO_T.M_percent"     * fairly linear, similar to 'FLUV_DU.HFP_percent'

Parent/Development	Weeks-crk	Chris-crk	Leech-head	Cragg-crk	West-Leech	Leech-Tunnel
COLL / O.HFP	42.2	48.1	44.3	48.1	50.0	47.9
FLUV / DU.HFP	3.9	0.0	2.2	0.0	0.0	0.5
GLFL / DU.HFP	0.5	0.0	0.6	6.9	0.0	2.2
TILL / DU.HFP	26.6	51.9	36.1	31.8	1.8	19.2
TILL / O.HFP	17.8	0.0	12.9	1.0	48.2	25.6
UNDO / T.M	6.4	0.0	3.6	0.0	0.0	0.8


```{r}
# COLL_O.HFP_percent
# similar to metamorphic (wark gneiss) pattern
# 
COLL_O.HFP_percent_DOC <- sixfilter_physiographic %>% 
  ggplot(aes(x = COLL_O.HFP_percent, y = NPOC_ppm)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
...
COLL_O.HFP_percent_SAC254 <- sixfilter_physiographic %>% 
  ggplot(aes(x = COLL_O.HFP_percent, y = SAC254_Abs.m)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
...
COLL_O.HFP_percent_E2E3 <- sixfilter_physiographic %>% 
  ggplot(aes(x = COLL_O.HFP_percent, y = E2E3)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
  theme(legend.position = "none",
        text = element_text(size = 11)) +
  labs(x = "COLL_O.HFP_percent (%)", y = expression(E[2]:E[3]) )

# together
collHFP <- cowplot::plot_grid(COLL_O.HFP_percent_DOC, COLL_O.HFP_percent_SAC254, COLL_O.HFP_percent_E2E3,
                   ncol = 1, align = "v", rel_heights = c(2, 1.3, 1.3))

# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch4_RF_soil_COLL_O-HFP.png", width = 4, height = 6, units = "in")

# ---
# FLUV_DU.HFP_percent
FLUV_DU.HFP_percent_DOC <- sixfilter_physiographic %>% 
  ggplot(aes(x = FLUV_DU.HFP_percent, y = NPOC_ppm)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
...
FLUV_DU.HFP_percent_SAC254 <- sixfilter_physiographic %>% 
  ggplot(aes(x = FLUV_DU.HFP_percent, y = SAC254_Abs.m)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
...
FLUV_DU.HFP_percent_E2E3 <- sixfilter_physiographic %>% 
  ggplot(aes(x = FLUV_DU.HFP_percent, y = E2E3)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
  theme(legend.position = "none",
        text = element_text(size = 11)) +
  labs(x = "FLUV_DU.HFP_percent (%)", y = expression(E[2]:E[3]) )

# together
fluvDUHFP <- cowplot::plot_grid(FLUV_DU.HFP_percent_DOC, FLUV_DU.HFP_percent_SAC254, FLUV_DU.HFP_percent_E2E3,
                   ncol = 1, align = "v", rel_heights = c(2, 1.3, 1.3))

# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch4_RF_soil_FLUV_DU-HFP.png", width = 4, height = 6, units = "in")



# ---
# UNDO_T.M_percent
UNDO_T.M_percent_DOC <- sixfilter_physiographic %>% 
  ggplot(aes(x = UNDO_T.M_percent, y = NPOC_ppm)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
...
UNDO_T.M_percent_SAC254 <- sixfilter_physiographic %>% 
  ggplot(aes(x = UNDO_T.M_percent, y = SAC254_Abs.m)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
...
UNDO_T.M_percent_E2E3 <- sixfilter_physiographic %>% 
  ggplot(aes(x = UNDO_T.M_percent, y = E2E3)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
  theme(legend.position = "none",
        text = element_text(size = 11)) +
  labs(x = "UNDO_T.M_percent (%)", y = expression(E[2]:E[3]) )

# together
undoTM <- cowplot::plot_grid(UNDO_T.M_percent_DOC, UNDO_T.M_percent_SAC254, UNDO_T.M_percent_E2E3,
                   ncol = 1, align = "v", rel_heights = c(2, 1.3, 1.3))

# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch4_RF_soil_UNDO-TM.png", width = 4, height = 6, units = "in")
...
slope_SAC254 <- sixfilter_physiographic %>% 
  ggplot(aes(x = slope_mean_degrees, y = SAC254_Abs.m)) + 
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5,
               varwidth = TRUE) +
  scale_fill_brewer(palette="Set2") +
...
slope_E2E3 <- sixfilter_physiographic %>% 
  ggplot(aes(x = slope_mean_degrees, y = E2E3)) + 
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5,
               varwidth = TRUE) +
  scale_fill_brewer(palette="Set2") +
...
logging_E2E3 <- sixfilter_physiographic %>% 
  ...
  theme(legend.position = "none",
        text = element_text(size = 11)) +
  labs(x = "Area harvested 1980 to 2011 (%)", y = expression(E[2]:E[3]) )


# together
a <- cowplot::plot_grid(logging_DOC + theme(legend.position = "none"), 
                        logging_SAC254, logging_E2E3,
                   ncol = 1, align = "v", 
                   rel_heights = c(1,1,1)) #2, 1.3, 1.3

# extract legend to add to top of combined variable plot
legend <- cowplot::get_legend(logging_DOC)


# .
# ... join plots of logging history and tree age
# .
...
tree_E2E3 <- sixfilter_physiographic %>% 
  ...
  theme(legend.position = "none",
        text = element_text(size = 11)) +
  labs(x = "Tree age, average (years)", y = "" )

# together
b <- cowplot::plot_grid(tree_DOC, tree_SAC254, tree_E2E3,
                   ncol = 1, align = "v", rel_heights = c(1,1,1))

# join variable plots
c <- cowplot::plot_grid(a, b, 
                   ncol = 2, align = "h",
                   rel_widths = c(1.025, 1))

# add legend 
cowplot::plot_grid(legend, c, 
                   ncol = 1, 
                   rel_widths = c(1,1),
                   rel_heights = c(1,8))

# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch4_RF_logging-tree-age.png", width = 6.5, height = 6, units = "in")
...
a <- sixfilter_physiographic %>%
  filter(E2E3 > 3.5) %>% 
  ggplot(aes(y = drainage_area_km2, x = NPOC_ppm)) +
  theme_bw() +
  ggridges::geom_density_ridges(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
  ...
  theme(legend.position = "top",
        text = element_text(size = 11)) +
  labs(y = expression(Drainage~area~(km^2)), x = "DOC (mg/L)", fill = "Site:" )
...
b <- sixfilter_physiographic %>%
  ...
  labs(y = "", 
       x = "DOC (mg/L)",
       title = "", subtitle = "nested catchments only")

# put the sub-basin plot adjacent to all sites
DOC_areas <- cowplot::plot_grid(a + theme(legend.position = "none"), b, nrow= 1)
...
aa <- sixfilter_physiographic %>%
  filter(E2E3 > 3.5) %>% 
  ggplot(aes(y = drainage_area_km2, x = SAC254_Abs.m)) +
  theme_bw() +
  ggridges::geom_density_ridges(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
  ...
  theme(legend.position = "none",
        text = element_text(size = 11)) +
  labs(y = expression(Drainage~area~(km^2)), 
       x = expression(SAC[254]~(m^1)) )
...
# put the sub-basin plot adjacent to all sites
SAc254_areas <- cowplot::plot_grid(aa, bb, nrow= 1)
...
aaa <- sixfilter_physiographic %>%
  filter(E2E3 > 3.5) %>% 
  ggplot(aes(y = drainage_area_km2, x = E2E3)) +
  theme_bw() +
  ggridges::geom_density_ridges(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
  ...
  theme(legend.position = "none",
        text = element_text(size = 11)) +
  labs(y = expression(Drainage~area~(km^2)), 
       x = expression(E[2]:E[3]), caption = "A" )
...
bbb <- sixfilter_physiographic %>%
  ...
  theme(legend.position = "none",
        text = element_text(size = 11)) +
  labs(y = "", 
       x = expression(E[2]:E[3]),
       title = "", subtitle = "nested catchments only", caption = "B")

# put the sub-basin plot adjacent to all sites
E2E3_areas <- cowplot::plot_grid(aaa, bbb, nrow= 1)

# --- 
# all together
sans <- cowplot::plot_grid(DOC_areas, SAc254_areas, E2E3_areas,
                   ncol = 1, rel_heights = c(1,1,1.1))

# add legend
cowplot::plot_grid(a_legend, sans, ncol = 1, rel_heights = c(1,8))

# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch4_RF_drainage-area_ridges.png",
       width = 6, height = 6, units = "in")
...
aa <- sixfilter_physiographic %>%
  ...
  theme_bw() +
  ggridges::geom_density_ridges(aes(fill = site), alpha = 0.5) +
  scale_fill_manual(values = brewer.pal(6, "Set2")[[6]]) + # yellow, 6th shade
  theme_bw() +
  lims(x = c(0,20),
       y = c(95.25, 95.75)) +
  theme(legend.position = "none",
        text = element_text(size = 11), 
        axis.text.x = element_blank()) +
  labs(x = "", y = expression( (km^2) ) ) # no labels, stack on top
...
# join
area_DOC1 <- cowplot::plot_grid(aa, a, ncol = 1, rel_heights = c(2,7), align = "v")
# add legend
area_DOC <- cowplot::plot_grid(a_legend, area_DOC1, ncol = 1, rel_heights = c(1,12))
...
b <- sixfilter_physiographic %>%
  ...
  theme(legend.position = "none",
        text = element_text(size = 11)) +
  labs(y = expression(Drainage~area~(km^2)), x = expression(SAC[254]~(m^-1)) )
...
# join
area_SAC254 <- cowplot::plot_grid(bb, b, ncol = 1, rel_heights = c(2,7), align = "v")
...
area_DOC <- sixfilter_physiographic %>% 
  filter(E2E3 > 3.5) %>% 
  ggplot(aes(x = drainage_area_km2, y = NPOC_ppm)) +
  theme_bw() +
  geom_boxplot(aes(fill = site), alpha = 0.5) +
  scale_fill_brewer(palette="Set2") +
  theme_bw() +
...
wut <- LWSA_meanWx %>% 
  mutate(Date = as_date(DateTime)) %>% 
  right_join(sixfilter, by = c("Date", 'rain_season') ) %>% 
  group_by(Date, site, rain_season) %>% 
  summarise(mean_T = mean(Temp_mean, na.rm = TRUE),
            mean_DOC = mean(NPOC_ppm, na.rm = TRUE)) %>%
  ungroup() 
...
just curious --- plot stage samples with values of DOC on points
```{r}
# first wet season
stage_samples %>% 
  filter(Date >= "2018-10-24", Date <= "2019-02-01") %>%  
  ggplot(aes(x = DateTime))+
  geom_line(aes(y = corr_stage_cm), colour = forWater_colours2["DeepBlue"], size = 0.6)+ 
  theme_bw()+
  geom_point(aes(y = sampleStage_cm), shape = 21, fill = "white", na.rm = TRUE)+
  
  geom_text(aes(y = sampleStage_cm, label = NPOC_ppm))+
...
# second wet season -- there are some interesting things in here
stage_samples %>% 
  filter(Date >= "2019-10-10", Date <= "2019-12-17") %>%  
  ggplot(aes(x = DateTime))+
  geom_line(aes(y = corr_stage_cm), colour = forWater_colours2["DeepBlue"], size = 0.6)+ 
  theme_bw()+
  geom_point(aes(y = sampleStage_cm), shape = 21, fill = "white", na.rm = TRUE)+
...
# big plot with all max and min and lines for subset
...
subbset_Wilcoxon_tests %>% 
  group_by(Parameter) %>% 
  summarise(mean = mean(p.value, na.rm = TRUE))
...
treatdat_full <- sampleresults %>% 
  filter(sample == "Treatability") %>% 
  full_join(treatdat_UW, by = c("Date", "site")) %>% 
  mutate(site = factor(site)) %>%  # -- new experimetnal (October 10, 2020)
  select(site, source, NPOC_ppm, SAC254_Abs.m, E2E3, SUVA, # mine
         # from U.W:
         source, Sample_ID, pH, Collection,
         `UV254_cm-1`, DOC_ppm, Zeta_Potential_mV, 
         THMs, TCM, TBM, DBCM, HAAs, DCAA, DBAA, TCAA, MCAA, MBAA, 
         Free_Chlorine_Final_mgL, day, Date) %>% 
  rename(UV254_cm = `UV254_cm-1`) %>% 
  mutate(UV254_m = UV254_cm*100,
         SUVA_UW = UV254_m/DOC_ppm) %>% 
  select(-c(UV254_cm, DBAA, MBAA, TBM)) %>% # drop DBP-FP below detection limits (NAs) 
  filter(source == "02") %>%
  tidyr::pivot_longer(names_to = "DBP", cols = c(THMs:MCAA),
                      values_to = "DBPfp_ugL") %>% 
  mutate(DBP = factor(DBP) ) #, levels = c("THMs", "TCM", "BDCM", "HAAs", "DCAA", "TCAA")))
...
# plot E2E3 --- not useful.
d <- treatdat_full %>% 
  ggplot(aes(x = E2E3, y = DBPfp_ugL))+
  geom_point(aes(colour = site,
                 shape = Collection),
             size = 2.5)+
  scale_colour_brewer(palette="Set2") +
  ...
  theme(text = element_text(size = 11),
        legend.position = "right",
        axis.text.y = element_blank())+
  labs(x = expression(atop(E[2]:E[3])), 
       y = "DBP-FP",
       shape = "Collection:")+
  guides(#colour = ,
    shape = guide_legend(nrow = 2))
...
cowplot::plot_grid(a, b, c, 
                   ncol = 3, align = "h", rel_widths = c(1.25, 1, 1.9)) 
# save
...
treatdat_UW %>% 
  filter(source == "02") %>%
  rename(SAC254_cm = 'UV254_cm-1') %>% 
  mutate(SAC254_m = SAC254_cm*100, 
         SUVA254 = SAC254_m/DOC_ppm) %>%
  select(SAC254_m, DOC_ppm, SUVA254, THMs, BDCM, TCM, HAAs, DCAA, TCAA) %>% 
  #select(-c(source, Sample_ID, day, site, Date, Collection, TBM, DBAA, DBCM, MCAA, MBAA) ) %>% 
  #select(-c(pH, Turbidity_NTU, Zeta_Potential_mV, Free_Chlorine_Final_mgL, SAC254_cm)) %>% 
  rename(SAC254 = SAC254_m, 
         DOC = DOC_ppm, 
         SUVA254 = SUVA254) %>% 
  cor() %>% 
  write.table(file = "R-outputs_UBC-forWater-MSc_HMc/tables/Appendix_DBPcorrelations.csv")
...
treatdat_long %>% 
  rename(UV254_cm = `UV254_cm-1`) %>% 
  mutate(UV254_m = UV254_cm*100 ) %>% 
  mutate(SUVA254_m = (UV254_m/DOC_ppm) ) %>% 
  select(UV254_m, DOC_ppm, SUVA254_m, DBPfp_ugL) %>%
  cor()


# is E2E3 correlated with SUVA ? -- for aromaticity 
wut <- synopticfilter %>% 
  filter(between(E2E3, 3,10) ) %>% 
  filter(QAQC_flag == "OK") %>% 
  filter(sample_type == "Grab" | sample_type == "Rack") %>% 
  ggplot(aes(x = E2E3, y = SUVA)) +  
  geom_point(aes(fill = sample_type), shape = 21, size = 1.5) +
  theme_bw() +
  scale_fill_brewer(palette="Set2") +
  theme(legend.position = "right",
        text = element_text(size = 11)) +
  labs(x = expression(E[2]:E[3]), y = expression(SUVA[254]), fill = "Sample type:") +
  stat_poly_eq(formula = y ~ x, 
               aes(label =  (..rr.label..)), 
               parse = TRUE, rr.digits = 4)
# nope

# is SAC254 correlated with SUVA ? -- for aromaticity 
wut <- synopticfilter %>% 
  filter(SUVA<6) %>% 
  filter(QAQC_flag == "OK") %>% 
  filter(sample_type == "Grab" | sample_type == "Rack") %>% 
  ggplot(aes(x = SAC254_Abs.m, y = SUVA)) +  
  geom_point(aes(fill = sample_type), shape = 21, size = 1.5) +
  theme_bw() +
  scale_fill_brewer(palette="Set2") +
  theme(legend.position = "right",
        text = element_text(size = 11)) +
  labs(x = expression(SAC[254]), y = expression(SUVA[254]), fill = "Sample type:") +
  stat_poly_eq(formula = y ~ x, 
               aes(label =  (..rr.label..)), 
               parse = TRUE, rr.digits = 4)
# nope

# is E2E3 correlated with zetapotential ? 
wut <- treatdat_full %>% 
  filter(between(E2E3, 3, 10) ) %>% # drop outliers
  ggplot(aes(x = E2E3, y = Zeta_Potential_mV)) +  
  geom_point(aes(colour = site,
                 shape = Collection),
             size = 2.5) +
  theme_bw() +
  ...
  theme(legend.position = "right",
        text = element_text(size = 11)) +
  labs(x = expression(E[2]:E[3]), y = "Zeta Potential (mV)" , colour = "Sampling site:") +
  stat_poly_eq(formula = y ~ x, 
               aes(label =  (..rr.label..)), 
               parse = TRUE, rr.digits = 4)
# yeah, kind of!! R^2 = 0.5297

# how about E2E3 with free chlorine?
wut <- treatdat_full %>% 
  filter(between(E2E3, 3, 10) ) %>% # drop outliers
  ggplot(aes(x = E2E3, y = Free_Chlorine_Final_mgL)) +  
  geom_point(aes(colour = site,
                 shape = Collection),
             size = 2.5) +
  theme_bw() +
  ...
  theme(legend.position = "right",
        text = element_text(size = 11)) +
  labs(x = expression(E[2]:E[3]), y = "Free Chlorine, Final (mg/L)" , colour = "Sampling site:") +
  stat_poly_eq(formula = y ~ x, 
               aes(label =  (..rr.label..)), 
               parse = TRUE, rr.digits = 4)
# Nope
...
# is SUVA254 correlated with zetapotential ? 
wut <- treatdat_full %>% 
  ggplot(aes(x = SUVA_UW, y = Zeta_Potential_mV)) +  
  geom_point(aes(colour = site,
                 shape = Collection),
             size = 2.5) +
  theme_bw() +
  ...
  theme(legend.position = "right",
        text = element_text(size = 11)) +
  labs(x = expression(SUVA[254]), y = "Zeta Potential (mV)" , colour = "Sampling site:") +
  stat_poly_eq(formula = y ~ x, 
               aes(label =  (..rr.label..)), 
               parse = TRUE, rr.digits = 4)
...
coincident_peaks <- sixfilter %>%
  ...
  filter(rain_season == "wet",   # remove baseflow samples (no stage peaks)
         ...
         QAQC_flag == "OK",      # remove samples with extended hold-time  
         sample != "R1", sample != "R2", sample != "R3", sample != "R4", sample != "R5",# no reps
         !is.na(sampleStage_cm), !is.na(NPOC_ppm), NPOC_ppm != 0) %>%  # drop NAs
