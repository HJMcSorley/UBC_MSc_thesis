...
conc_seas <- synopticfilter %>%
  ...
  select(rain_season, count, mean_RSD, DOCmin, DOCmedian, DOCmax) %>% 
...
sixfilter %>%
  ...
  bind_rows(
    synopticfilter %>%
      ...
      filter(sample_type == "Grab" | sample_type == "Rack") %>% 
      summarize(sample_type = "all",
                count = count(!is.na(SAC254_Abs.m)), 
                SACmean = mean(SAC254_Abs.m, na.rm = T), 
                SACsd = sd(SAC254_Abs.m, na.rm = T), 
                RSD = round((SACsd/SACmean)*100, 0),
                SACmin = min(SAC254_Abs.m, na.rm = T), 
...
# save
ggsave("R-outputs_UBC-forWater-MSc_HMc/figures/Ch3_subbasin_GvsR_WETseason-ridgeplot.png", width = 5, height = 4, unit = "in") 
