...
fig_cap = "\\label{fig:megaplot} *Weather and stream response at six locations across the Leech Water Supply Area with points indicating stage and time of sample collection.*"
...
As the Leech River Tunnel will be the point of diversion for future inter-basin transfers from Leech water supply area (LWSA) to the Sooke Reservoir basin, the Tunnel (site-6) is the effective outlet of the LWSA where runoff from each nested catchment is integrated. Similarly, Weeks Main Creek and Chris Creek (*'Weeks'* & *'ChrisCrk'*, sites 1 & 2) are integrated at (*'LeechHead'*, site-3), which is just below the headwaters' confluence. Table \@ref(tab:subbasinDOCtunnelint) summarizes the range and variability[^5] in DOC within each site and among all sites, with categorical groupings of sub-basins.

[^5]:   Relative standard deviation (RSD) reflects how values range around the mean within a group; it is a ratio of the absolute standard deviation relative to the mean. And standard deviation is the square root of the variance. 
...
[^6]:Levene's test is used to check for homogeneity of variance (homoscedasticity), it's an alternative to the Bartlett’s test that's less sensitive to departures from normality in the data.
