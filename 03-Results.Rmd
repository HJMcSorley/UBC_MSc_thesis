...
To determine whether DOC minimum and maximum concentrations occurred at the same time across the LWSA synchrony tests were run on sample results. The R package Synchrony was used to find the proportion of common minima and maxima time-stamped DOC values between pairs of sites. Table \@ref(tab:synchDOCpeaks) summarizes the findings, where proportion of 1 indicates perfect temporal synchrony of DOC peaks between sites, and significance was based on Monte Carlo randomizations (type 2 with 999 randomizations without replacement).

```{r synchDOCpeaks}
read_csv("R-outputs_UBC-forWater-MSc_HMc/tables/DOC-stats_synchrony-peaks.csv", col_names = TRUE) %>% 
