...
```{r subsetting samples by campaign and sample counts}
...
```{r metals and DOC (NPOC), echo = FALSE}
...
```{r sample_type ridge plots, include=FALSE, echo = FALSE}
...
```{r seasonal DOC table and ridge plot, fig.width=8, fig.height=6, echo = FALSE}
...
```{r seasonal DOC table 2, echo = FALSE}
# summary table
sixfilter %>% 
  dplyr::filter(analysis == "DOC", 
                sample_type == "Grab" | sample_type == "Rack") %>% 
...
```{r seasonal Grab DOC table 2-B, echo = FALSE}
...
```{r DOC vs CDOM at 6 sites, echo = FALSE}
